...
def _analytic_scheme(case: Case) -> int:
    ...
    try:
        ...
        _unsupported(
            "geometry.file", str(case.geometry.file),
...
def _validate_native_slice(case: Case) -> None:
    if case.run.workflow != "profile":
        _unsupported(
            "run.workflow", case.run.workflow, "profile",
            "Use workflow = 'profile' for this first native execution route.",
        )
    if case.geometry.format != "analytic":
        _unsupported(
            "geometry.format", case.geometry.format, "analytic",
            "Use format = 'analytic'; native VMEC and Boozer readers will be added without a compatibility conversion.",
        )
    _analytic_scheme(case)
    if case.physics.magnetic_drifts != "dkes":
        _unsupported(
            "physics.magnetic_drifts", case.physics.magnetic_drifts, "dkes",
            "Set magnetic_drifts = 'dkes' for the supported local analytic route.",
        )
    if case.physics.phi1 != "off":
        _unsupported(
            "physics.phi1", case.physics.phi1, "off",
            "Set phi1 = 'off' for this route.",
        )
    if case.electric_field.mode != "prescribed":
        _unsupported(
            "electric_field.mode", case.electric_field.mode, "prescribed",
    ...
    if case.parallel.strategy == "batch" or case.parallel.shard:
        _unsupported(
            "parallel", {"strategy": case.parallel.strategy, "shard": case.parallel.shard},
    ...
    if any(value <= 0.0 for value in case.geometry.surfaces) or any(
        right <= left for left, right in zip(case.geometry.surfaces, case.geometry.surfaces[1:])
    ):
        _unsupported(
            "geometry.surfaces", case.geometry.surfaces,
...
def _profile_matrix(case: Case, attribute: str) -> np.ndarray:
    values = np.asarray([getattr(species, attribute) for species in case.species], dtype=np.float64)
...
def _profile_gradients(values: np.ndarray, r_hat: np.ndarray) -> np.ndarray:
    ...
    return np.asarray(np.gradient(values, r_hat, axis=0, edge_order=edge_order), dtype=np.float64)


def _geometry_context(case: Case, grids, surface_index: int):
    ...
    r_n = math.sqrt(psi_n)
    if scheme == 1:
        psi_a_hat, a_hat = 0.15596, 0.5585
    elif scheme == 2:
        a_hat, psi_a_hat = 0.5585, 0.5585**2 / 2.0
    elif scheme == 3:
        a_hat, psi_a_hat = 0.5400, 0.5400**2 / 2.0
    else:
        psi_a_hat, a_hat = -0.384935, 0.5109
    geometry = FluxSurfaceGeometry.from_scheme(
        scheme, theta=grids.theta, zeta=grids.zeta
    )
    return geometry, RadialCoordinates(psi_a_hat=psi_a_hat, a_hat=a_hat, r_n=r_n)
...
def _make_operator(
    ...
    *,
    ...
    geometry, radial = _geometry_context(case, grids, surface_index)
    z_s = jnp.asarray([species.charge for species in case.species], dtype=jnp.float64)
    m_hat = jnp.asarray(
        [species.mass_amu * _ATOMIC_MASS_KG / _PROTON_MASS_KG for species in case.species],
        dtype=jnp.float64,
    )
    ...
    if case.physics.collisions == "pitch_angle_scattering":
        pas = make_pitch_angle_scattering_v3_operator(
            x=grids.x, z_s=z_s, m_hats=m_hat, n_hats=n_surface,
            t_hats=t_surface, nu_n=DEFAULT_NU_N, n_xi_for_x=grids.n_xi_for_x,
            n_xi=grids.n_xi,
        )
        constraint_scheme = 2
    else:
        fp = make_fokker_planck_v3_operator(
            x=np.asarray(grids.x), x_weights=np.asarray(grids.x_weights),
            ddx=np.asarray(grids.ddx), d2dx2=np.asarray(grids.d2dx2), x_grid_k=0.0,
            z_s=np.asarray(z_s), m_hats=np.asarray(m_hat), n_hats=np.asarray(n_surface),
            t_hats=np.asarray(t_surface), nu_n=DEFAULT_NU_N, krook=0.0,
            n_xi=grids.n_xi, nl=grids.n_l, n_xi_for_x=np.asarray(grids.n_xi_for_x),
            strict_parity=len(case.species) > 1,
        )
        constraint_scheme = 1

    er_hat = float(case.electric_field.value_kV_m)
    ...
    op = KineticOperator(
        n_species=len(case.species), n_x=grids.n_x, n_xi=grids.n_xi,
        n_theta=grids.n_theta, n_zeta=grids.n_zeta, rhs_mode=1,
        constraint_scheme=constraint_scheme, point_at_x0=False,
        use_dkes_exb=True, with_exb=er_hat != 0.0,
        with_er_xidot=False, with_er_xdot=False,
        x=grids.x, x_weights=grids.x_weights, ddx=grids.ddx,
        ddtheta=grids.ddtheta, ddzeta=grids.ddzeta,
        theta_weights=grids.theta_weights, zeta_weights=grids.zeta_weights,
        ...
        b_hat=geometry.b_hat, db_hat_dtheta=geometry.db_hat_dtheta,
        db_hat_dzeta=geometry.db_hat_dzeta, d_hat=geometry.d_hat,
        ...
        z_s=z_s, m_hat=m_hat, t_hat=t_surface, n_hat=n_surface,
        dn_hat_dpsi_hat=dn_dpsi, dt_hat_dpsi_hat=dt_dpsi,
...
def run_case(case: Case, *, out: str | Path | None = None, emit=None) -> Result:
    ...
    scheme = _analytic_scheme(case)
    a_hat = {1: 0.5585, 2: 0.5585, 3: 0.5400, 4: 0.5109}[scheme]
    r_hat = a_hat * r_n
    ...
    for index, surface in enumerate(surfaces):
        ...
        op, _grids, _geometry, radial = _make_operator(
            case, surface_index=index, n_hat=n_hat, t_hat=t_hat,
            dn_dr_hat=dn_dr_hat, dt_dr_hat=dt_dr_hat,
        )
        solve_start = time.perf_counter()
        solved = solve(
            op, op.rhs(), method=_route_name(case.solver.method),
            ...
            * _total_host_memory_bytes() / (1024**3),
        )
    ...
    output_path = (
        Path(out).expanduser().resolve() if out is not None
    ...
    arrays = {
        ...
        "electric_field_kV_m": np.full((surfaces.size,), case.electric_field.value_kV_m),
    ...
    dimensions = {
        "surface": ("surface",), "r_N": ("surface",),
        "species": ("species",), "charge_e": ("species",), "mass_amu": ("species",),
        ...
        "primal_residual": ("surface",), "solver_iterations": ("surface",),
    ...
    metadata = {
        ...
        "normalization": {
            "density_m3": 1.0e20, "temperature_keV": 1.0,
            "mass_kg": _PROTON_MASS_KG,
        },
        "geometry_sha256": _sha256(case.geometry.file),
        "dkx_version": __version__, "python_version": platform.python_version(),
        "jax_version": jax.__version__, "jaxlib_version": jaxlib.__version__,
        "platform": platform.platform(), "precision": case.run.precision,
    ...
    result = Result(
        case_id=case.case_id, case_name=case.name, workflow=case.run.workflow,
        arrays=arrays, dimensions=dimensions, metadata=metadata,
        output_path=output_path, _runtime={"operator": retained_operator},
    )
    if out is not None:
