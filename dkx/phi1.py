...
from dkx.inputs import (  # noqa: E402
    RawNamelist,
    SfincsInput,
    load_sfincs_input,
    sfincs_input_from_raw,
)
from dkx.solve import SolveResult, solve  # noqa: E402
...
def operator_from_input(
    ...
    LINEAR solve (no Newton) routed through :func:`dkx.run.run_profile`,
    not this Newton entry point; ``quasineutralityOption`` other than 1/2 raises
    from :func:`dkx.drift_kinetic.kinetic_operator_from_namelist`.
    """
    ...
    *,
    ...
    Args:
        inp: a deck (``SfincsInput`` / ``RawNamelist`` / path) or a prebuilt
            canonical :class:`~dkx.drift_kinetic.KineticOperator` with
            ``include_phi1=True``.
