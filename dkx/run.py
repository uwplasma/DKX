...
class ProfileRun:
    ...
    Attributes:
        ...
        moments: the full RHSMode=1 per-species diagnostic table keyed by
            sfincsOutput.h5 names (:func:`dkx.moments.rhsmode1_moments`
...
def _raw_with_validated_overrides(inp: SfincsInput) -> RawNamelist:
    ...
    xDot/xiDot Er terms), which :func:`dkx.inputs._validate` applies to
    ...
    *,
    ...
    Args:
        ...
        solver_trace_path: optional JSON sidecar path; when set, a versioned
            :class:`dkx.solver_trace.SolverTrace` is written from
            the shared multi-RHS :class:`dkx.solve.SolveResult`.
    ...
    if rhs_mode == 3 and d11_bounds_supported(op):
        ...
        # (JAX-only diagnostic; see dkx.variational).
    ...
    if out_path is not None:
        ...
        if os.environ.get("DKX_WRITE_SOLVER_DIAGNOSTICS", "").strip().lower() in {
            "1", "true", "yes", "on",
        }:
...
def profile_moments_from_operator(
    ...
    *,
    ...
    Thin functional wrapper over :func:`dkx.moments.rhsmode1_moments`
    ...
    :func:`dkx.solve.solve` — the implicit solve.
    ...
    Args:
        ...
        ntv_kernel_tz: optional NTV geometric kernel ``(T, Z)``
            (:func:`dkx.moments.ntv_kernel`); when given, the ``NTV``
...
def run_profile(
    ...
    *,
    ...
    Args:
        ...
        solver_trace_path: optional JSON sidecar path; when set, a versioned
            :class:`dkx.solver_trace.SolverTrace` is written from
            the single-RHS :class:`dkx.solve.SolveResult`.
        emit: per-line stdout sink for the Fortran-parity print blocks
            (``print`` reproduces the v3 console flow); ``None`` silences it.
    ...
    if op.include_phi1:
        ...
        # Newton solve in dkx.phi1 wraps solve() as its inner linear step.
        import jax.numpy as jnp  # noqa: PLC0415

        from dkx.phi1 import solve_phi1_history  # noqa: PLC0415
        ...
        newton_tol = _env_tol("DKX_PHI1_NEWTON_TOL", 1.0e-12)
        gmres_tol = _env_tol("DKX_PHI1_GMRES_TOL", 1.0e-12)
...
def run_from_namelist(
    ...
    *,
    ...
    This is the Python equivalent of the ``dkx write-output`` CLI flow:
    ...
    Args:
        ...
        solver_trace_path: optional JSON sidecar path for a versioned
            :class:`dkx.solver_trace.SolverTrace`.
        emit: per-line stdout sink for the Fortran-parity print blocks;
