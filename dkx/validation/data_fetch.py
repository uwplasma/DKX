...
def data_cache_root() -> Path:
    ...
    override = os.environ.get("DKX_DATA_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    xdg = os.environ.get("XDG_CACHE_HOME", "").strip()
    base = Path(xdg).expanduser() if xdg else Path.home() / ".cache"
    return base / "dkx" / "data"
...
def ensure_external_equilibrium_data(*, quiet: bool = False) -> Path:
    ...
    if not archive.exists() or _sha256(archive) != manifest["archive_sha256"]:
        if os.environ.get("DKX_OFFLINE", "").strip() in {"1", "true", "True"}:
            raise FileNotFoundError(
                "External DKX equilibrium data is not cached and DKX_OFFLINE is set."
            )
        if not quiet:
            print(f"Downloading DKX external equilibrium data from {_release_asset_url(manifest)}")
        _download_archive(manifest, archive)

    with tempfile.TemporaryDirectory(prefix="dkx_data_", dir=str(data_cache_root())) as tmpdir:
        ...
        if not _verify_files(tmp_target, manifest):
            raise RuntimeError("External DKX equilibrium data verification failed after extraction.")
        for item in manifest["files"]:
            rel = Path(item["path"])
            dst = target / rel
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Fetch release-hosted DKX equilibrium fixtures.")
    ...
    if not args.quiet:
        print(f"DKX external data {manifest['version']} ready in {target}")
        for item in manifest["files"]:
