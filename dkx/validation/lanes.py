"""Completion policy for the research lanes.

A lane is a line of work with declared evidence and a push gate. These are
the checks that decide whether one is complete, and the error raised when a
lane claims completion it cannot show.

The first link in the validation chain: this module depends on nothing else
in the package.
"""
