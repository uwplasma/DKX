"""Schema and promotion policy for committed benchmark artifacts.

Every JSON artifact the repository commits as evidence must satisfy a schema
and a promotion policy before it can back a claim. These are the checks that
decide, plus the index that classifies an artifact by what it is evidence for.

Split out of ``artifacts.py``, which had grown to seven unrelated concerns in
one file. This group is a leaf: it reads payloads and returns error lists, and
shares no symbol with the analysis or figure code it used to sit beside.
"""
