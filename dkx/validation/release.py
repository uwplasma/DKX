...
# reproduction, but the production benchmark tier is DKX-owned by
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "when they exist locally. Not used by the default DKX benchmark tier."
        ),
    )
    parser.add_argument(
...
def _run_write_output(
    *,
    ...
    RHSMode selects the run kind and :func:`dkx.solve.solve` selects
...
def write_output_trace_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Profile a full dkx write-output run with JAX trace capture. "
    ...
    if heartbeat_interval_s > 0.0:
        heartbeat_thread = threading.Thread(target=_heartbeat, name="dkx_phase_log_heartbeat", daemon=True)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Run lightweight DKX release validation helpers.")
