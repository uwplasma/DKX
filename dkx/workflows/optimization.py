...
def find_ambipolar_roots(
    ...
    *,
    ...
    :mod:`dkx.ambipolar`: :math:`j_r(E_r)=\sum_s Z_s\Gamma_s(E_r)`.
...
class ScanPromotionSummary:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "claim_boundary": (
                "This audit evaluates completed dkx kinetic outputs. "
...
class CandidateScanPlan:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            "workflow": "dkx_optimization_candidate_scan_plan",
            ...
            "claim_boundary": (
                "This plan launches high-fidelity dkx scan-er runs for an "
...
class PromotionEvidencePlan:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            "workflow": "dkx_optimization_promotion_evidence_plan",
            "input_namelist": str(self.input_namelist),
            "out_dir": str(self.out_dir),
            "er_values": [float(value) for value in self.er_values],
...
def compare_fortran_promotion(
    dkx_payload: PromotionPayloadInput,
    ...
    *,
    ...
    """Compare a DKX promotion payload with a Fortran-v3-derived one."""

    return compare_promotion_pair(
        dkx_payload,
        fortran_v3_payload,
        reference_label=dkx_label,
...
def compare_optimization_promotions(
    ...
    *,
    ...
    dkx_fortran_payload: PromotionPayloadInput | None = None,
    tolerances: PromotionComparisonTolerances | None = None,
    require_gate_pass: bool = True,
    require_flux_objective: bool = True,
) -> dict[str, Any]:
    """Compare CPU/GPU promotions and optionally DKX against Fortran v3.

    When ``fortran_v3_payload`` is provided, the Fortran comparison uses
    ``dkx_fortran_payload`` as its JAX-side reference if supplied;
    ...
    if fortran_v3_payload is not None:
        jax_reference = (
            cpu_payload if dkx_fortran_payload is None else dkx_fortran_payload
        )
        comparisons["dkx_fortran_v3"] = compare_fortran_promotion(
            jax_reference,
            fortran_v3_payload,
            tolerances=tol,
            require_gate_pass=require_gate_pass,
    ...
    return {
        "workflow": "dkx_optimization_promotion_comparison",
        "status": "pass" if not failures else "fail",
        "failures": failures,
