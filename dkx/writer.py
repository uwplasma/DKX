...
only canonical objects (:mod:`dkx.inputs`,
:mod:`dkx.drift_kinetic`, :mod:`dkx.moments`,
:mod:`dkx.magnetic_geometry`) and writes the same datasets the legacy
...
see :mod:`dkx.moments` for the flux conventions.
"""
...
from dkx.constants import RadialCoordinates, nu_n_from_nu_prime
from dkx.drift_kinetic import KineticOperator, _resolve_equilibrium_path
...
def operator_containers(
    ...
    """The :mod:`dkx.moments` input containers of a canonical operator."""
    layout = StateLayout(
...
def output_format_from_suffix(path: str | Path) -> str:
    ...
    if suffix == ".npz":
        return "npz"
    raise ValueError(
        f"Unsupported dkx output suffix {suffix!r}. Use .h5/.hdf5, .nc/.netcdf, or .npz."
    )
...
# :func:`dkx.collisions.polynomial_interpolation_matrix_np`.
# ---------------------------------------------------------------------------
...
def _export_f_config(*, raw, grids: Grids, geom: FluxSurfaceGeometry) -> _ExportFConfig | None:
    ...
    if x_option == 0:
        ...
        from dkx.collisions import polynomial_interpolation_matrix_np  # noqa: PLC0415
    ...
    *,
    ...
    Args:
        ...
        solver_diagnostics: optional per-``whichRHS`` residual debug datasets
            ...
            when the ``DKX_WRITE_SOLVER_DIAGNOSTICS`` opt-in is active.
...
def write_profile_output(
    *,
    ...
    (:func:`dkx.moments.rhsmode1_moments`), NTV, and the classical
...
def write_run_solver_trace(
    *,
    ...
    Emits the same :class:`dkx.solver_trace.SolverTrace` schema
    the legacy ``--solver-trace`` path produces, populated from the canonical
    :class:`dkx.solve.SolveResult` (method, residual norms, convergence,
    ...
    """
    from dkx.solver_trace import SolverTrace, write_solver_trace_json  # noqa: PLC0415
