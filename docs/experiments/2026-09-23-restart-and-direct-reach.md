...
Status: step 5 (the restart test) is final, with its admission result on the
`Nxi = 20` ladder and at `(Nxi, Nx) = (40, 16)`, and its recommendation is the
DKX 2.7.0 restart default (see "The default", below). The full gap deck and
step 6a (MUMPS analysis only) were not run; they are listed under Follow-up.
...
- **Step 6a, MUMPS analysis only (`JOB=1`) on the gap deck.** Not run. The
  environment is ready and the script is written (`mumps_analysis.py` on the
  `perf/restart-and-direct-reach` branch, not merged because it has never
  run): PyMUMPS
...
- The two SOLVAX changes above, by their owner.

## The default

DKX 2.7.0 adopts the recommendation as `solve(restart=None)`, the default: five
cycles of 30 and two of 100, as before, then the longest restart up to 1,000
whose `2 x restart x total_size x 8`-byte basis fits a quarter of the memory
available at that point (or `krylov_memory_budget_gb`). The short cycles stay
first because of the per-step cost above: a deck that converges within them
never allocates the wide basis. The stall ladder's larger-budget rung uses the
same wide restart. On the gap deck that budget gives a restart near 1,000 on a
host with 40 GiB available, the run listed first under Follow-up.
