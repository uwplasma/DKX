...
solve uses it. The ``dkx[structured]`` extra is retained as a no-op
...
``dkx`` itself — frozen reference outputs ship with the test fixtures.
...
Use ``DKX_DATA_DIR=/path/to/cache`` to choose a different cache root. Use
``DKX_OFFLINE=1`` in CI or cluster jobs when a run must fail instead of
...
``dkx --plot``, and ``--out sfincsOutput.nc`` work without any extra plotting
