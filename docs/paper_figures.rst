...
.. figure:: _static/figures/paper_benchmarks/monoenergetic_icnts_w7x.png
   ...
   Monoenergetic ``D11``/``D31``/``D33`` against the ICNTS W7-X benchmark, across
   the 1/nu, plateau and Pfirsch-Schlueter regimes
   (``monoenergetic_icnts_w7x.py``).

.. figure:: _static/figures/paper_benchmarks/shaing_callen_convergence.png
   :alt: Bootstrap coefficient approaching the Shaing-Callen collisionless limit
   :width: 85%

   The bootstrap coefficient approaching its Shaing-Callen collisionless limit as
   collisionality falls (``shaing_callen_convergence.py``).

.. figure:: _static/figures/paper_benchmarks/w7x_ambipolar_er.png
   :alt: Ambipolar radial electric field and its roots on W7-X
   :width: 85%

   Radial current against electric field on W7-X, with every sampled evaluation
   and the resulting ion, unstable and electron roots
   (``w7x_ambipolar_er.py``).

.. figure:: _static/figures/paper_benchmarks/bootstrap_consistency_kinetic_loop.png
   :alt: Self-consistent bootstrap current loop between DKX and the equilibrium
   :width: 85%

   Bootstrap current iterated to consistency between the kinetic solve and the
   equilibrium (``bootstrap_consistency_kinetic_loop.py``).

.. figure:: _static/figures/paper_benchmarks/impurity_transport.png
   :alt: Impurity flux and screening against collisionality
   :width: 85%

   Impurity flux and the screening transition against collisionality
