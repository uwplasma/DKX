...
suite. The topics below are intentionally not release blockers; they are the
algorithmic targets for larger, more memory-limited, and more strongly parallel
research workloads.

Release decision on the in-house production preconditioner
----------------------------------------------------------
...
The release scope is therefore closed on correctness and documentation:
...
passes the same residual, runtime, and memory checks. Future work should avoid
...
Each topic below names the present implementation, the remaining blocker, the
next source-code touchpoints, and the check that must pass before the result can
be promoted into public performance claims.
...
promotion tests until a candidate implementation passes its release checks.
...
Until those checks pass, QI/device-QI material is not a release blocker and must
...
PAS byte budgets and fail-closed matrix-free checks prevent unsafe promotions.
The real geometry4 and HSX probes were residual-clean but did not improve
runtime or peak memory, so no default promotion is justified.
...
- The retired ``preconditioner_pas_matrix_free`` owner (deleted with the legacy pipeline) contained guarded
  ...
  byte budgets.
- PAS applicability and memory checks lived in the retired PAS policy owner
...
The 2026-05-17 clean ``office`` run closed the last unknown in this line of
work as a measured blocker, not a promotion. On two RTX A4000 GPUs, a fresh
checkout at ``8e34341`` completed the one-GPU hot solve in ``4.047 s`` but
timed out the two-GPU sharded child at the ``300 s`` cap. The checked artifact
...
- The sharded matrix-free operator path lived in the retired legacy operator
  owner (preserved on the research branch).
- ``docs/_static/transport_sharded_solve_gpu_1v2_failclosed_2026_05_17.json``
...
deterministic-output check, beat the warm one-device solve, and prove that the
...
must pass. The amortization check must also pass with predicted speedup and
...
- Communication-avoiding GMRES and domain-decomposition preconditioners provide
  the scaling model for this line of work: fewer global reductions help only
  when the local subdomain work and coarse correction amortize communication.
...
- `Lineax <https://docs.kidger.site/lineax/>`_ remains an optional benchmark
  study until real SFINCS operators pass residual/error checks; it should not
