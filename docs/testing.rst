...
`dkx` is validated with a layered testing strategy. The operator, solvers,
output writer, and public workflows are each exercised at multiple levels, so no
single benchmark carries the correctness claim on its own.
...
not as the public identity of the code. They show that the standalone `dkx`
implementation reproduces trusted neoclassical physics on the supported audited
scope.
...
- The Simakov-Helander high-collisionality audit must recompute the Appendix-B
  ...
  full analytic-limit requirement unmet unless the scan reaches the configured
  high-``nu`` threshold.
...
conversion consistency. These checks are cheap enough for CI and catch a common
...
For release documentation, the claim is that:
...
- keep ``tests/test_data_fetch.py`` as the unit check for manifest structure,
...
than hard-code a permanent collected-test count here. The active refactor line of
work records about ``91%`` measured package coverage, with ``95%`` meaningful
package coverage as the research-grade target and CI wall time kept below ten
minutes.
...
Python-floor collection, and the wheel-install job are each capped at ten
...
checks. A residual-improving device run is not true-device-QI promotion until
those checks pass.

The documentation build is part of the release discipline. A docs change that
breaks Sphinx or leaves pages internally inconsistent is a real regression.
...
4. **Keep backend equivalence cheap.** CPU/GPU and ``jit``/eager equivalence tests
   ...
   Production CPU/GPU and Fortran comparisons remain release or nightly checks,
   not every-commit CI checks.
5. **Require a test for every fixed bug.** Any bug found in profiling, solver selection,
...
6. **Raise CI thresholds in steps.** The CI fail-under gate is ``80%``. Move the
   ...
   release-data smoke, and the fast release checks to pass within the target
   wall-time budget.
7. **Keep size and runtime bounded.** New tests should use generated synthetic
   ...
   multi-megabyte tracked fixtures to increase coverage; the repo-size check and
...
is considered research-grade when the coverage floor, physics checks, output-key
coverage, CPU/GPU equivalence checks, release-data checks, docs build, and
...
For performance audits, keep benchmark instrumentation bounded:

- The suite runners default to ``--jax-profile-marks off`` so runtime drift is
  measured without profiler interference. Opt into ``on`` or ``full`` only for
  explicit profiling work, and leave per-mark device-memory sampling off unless
  you are doing targeted device-memory diagnosis.
- Kernel/XLA traces should go through
  ``python -m tools.release.release write-output-trace`` or the transport-trace
  helpers, not through always-on per-phase GPU memory polling.
- The runtime-drift audit prefers the solver's logged ``elapsed_s=...`` value
  when available, falling back to subprocess wall time only for archived
  artifacts that do not record it.
- The suite subprocesses leave ``JAX_COMPILATION_CACHE_DIR`` unset unless
  ``--jax-cache-dir`` is requested, so runtime drift is not polluted by
  persistent-cache write cost.
...
- For long RHSMode=1 profiling runs, keep the trace helper's phase log in the
  artifact bundle. Its default ``profile_write_output_trace_phases.json``
  sidecar makes it clear whether a nonzero wrapper status came from the solve
  itself or from profiler finalization after a valid output file was already
  written.
...
Publication-facing validation entries
-------------------------------------

The manuscript-oriented validation entries are tracked separately from the
general unit/regression suite. Each entry is one self-contained line of
validation work, with its own scripts, artifacts, tests, and acceptance
criteria; the JSON records and CLI commands keep the ``lanes`` spelling. The
tracked pieces are:
...
- ``tests/test_validation_manifest_schema.py`` enforces that every entry has explicit
  source-code anchors, protecting tests, and acceptance criteria.
- ``python -m tools.release.release check-gates`` and ``tests/test_release_gate_metadata.py`` add a
  CI-fast release check over the manifest's ``release_gate`` metadata. Each entry must be
  ``release_ready``, ``regression_scaffold``, ``bounded_proxy``, or
  ``closed_deferred``; no entry may remain ambiguous in the release manifest. The
  release checker also validates
  the record status/kind, the non-empty literature, claim, source, test, and
  acceptance-criteria lists, and all listed source, test, script, and artifact
  paths, including paths on ``closed_deferred`` entries. A deferred entry is
  therefore allowed to stay out of the tagged release claim, but it is not
  allowed to rot silently.
- ``docs/_static/research_lane_completion_2026_05_12.json`` records the active
  research and performance topics, evidence artifacts, completion estimates,
  acceptance criteria, and next actions for the checked research cycle.
  ...
  evidence-backed and that active topics record substantial measured progress
  before their completion estimate is increased. The policy is target-capped:
  if a topic has fewer percentage points remaining than the requested push target,
...
- ``python -m tools.release.release check-research-lanes`` and
  ``tests/test_research_lane_policy.py`` provide the offline check for deferred
  QI/device-QI evidence. That check covers provenance, fail-closed metadata, and
  evidence-backed completion percentages only; it is not a computed convergence
  bound. Historical or checked-in research artifacts therefore cannot
  silently overclaim GPU/device status.

The ``E_r`` trajectory-model sweep family is a release-facing validation entry:
...
- a fixed tokamak-like reference case with direct numerical assertions on zero-field
  agreement and finite-field model separation,
- a bounded stellarator-like branch case that keeps the fixed input and figure stable
...
The collisionality-figure generator (``generate_sfincs_paper_figs.py``) is not
trusted on the strength of existing figure files alone. Two real bugs were found
in its scan-input writer on this branch:
...
- and the fast-path override could place missing keys such as ``NL`` outside the
  namelist group, while also choosing ``Nzeta=3``, which is below the stencil
...
The audited full artifacts from that entry are checked in:
...
with the manifest's provenance and acceptance-criteria expectations instead of
relying on plots alone.

Mapped x-grid and QI integration smoke checks (retired)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
checks at the documented production grid.

The high-collisionality Simakov-Helander entry has a bounded normalization audit:
...
audited collisionality scans stop near ``nu'=10``.
...
- high-``nu`` range checks for threshold, point count, and decade span,
...
high-``nu`` range, provenance, and checked-in-artifact checks pass. Otherwise it
...
The executable high-``nu`` run-plan artifact is also labeled a run plan, not
...
checked artifact and the validation checks that keep the entry closed until
converged scan artifacts exist.

The W7-X high-``nu`` preconditioner performance figure is intentionally narrower
than a physics-validation entry. Its summary supports a single-point performance
...
The W7-X ambipolar literature entry is kept as a deferred artifact check, not as
a stable long-run figure generator. The retained package-level validation is
...
scan/solve tests. The check records explicit ``deferred_reasons``, provenance
...
numerical root checks pass and the W7-X provenance/source artifact is complete
and checked in.

Further reading
---------------

For benchmark and performance state, see :doc:`performance` and
:doc:`parallelism`. For the external validation story, see :doc:`fortran_comparison`.
For the manuscript-facing literature/figure entries, see :doc:`validation_matrix`.

Optional ecosystem research topics
----------------------------------

External JAX ecosystem libraries are not production dependencies. Lineax,
Equinox-wrapper, and JAXopt benchmark drivers are preserved as research-topic
material until they satisfy the accuracy, runtime, memory, differentiability, and
dependency-policy requirements. Stable tests instead exercise the retained in-tree
