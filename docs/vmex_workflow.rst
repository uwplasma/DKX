...
The public workflow is a Boozer-spectrum proxy transport-objective gradient
check. It is not a claim of full VMEC-boundary-to-SFINCS kinetic transport
gradients.
...
no-overclaim check, a no-optional-dependency Boozer-spectrum autodiff readiness
check, and the exact command for the optional proxy-gradient check.  It does
not import either optional backend.

The status JSON also contains ``no_solve_provenance_gate``.  It is a
machine-readable assertion that the workflow is still a proxy-gradient path:
...
``dkx.workflows.geometry_adapters`` check, so the skip-safe status path and the
...
``kinetic_solve_executed`` must both remain false in this workflow.
...
``backend_readiness_gate``.  That check is intentionally synthetic: it
evaluates a small Boozer spectrum through a ``dkx`` proxy transport objective,
checks the full spectral JAX gradient against centered finite differences, and
checks a JVP against the gradient dot product.  It is a local
...
Differentiated in this workflow:
...
- no gradient through the SFINCS kinetic transport solve in this workflow,
...
- ``kinetic_transport_scalar_contract.no_overclaim_gate.status`` must be
  ``"pass"`` in default CI.  It fails if the proxy workflow claims a kinetic
  solve, requires optional geometry packages in default CI, drops a required
  stage, or promotes the full kinetic scalar while deferred stages remain.

Checks
------

Run the VMEC/Boozer workflow and adapter checks:

.. code-block:: bash

   python -m pytest tests/test_vmex_workflow.py tests/test_jax_geometry_adapters.py -q

These tests also include a no-solve invariant check that the normalized Boozer
...
they satisfy the documented accuracy, runtime, memory, and dependency
requirements.

The optional VMEC/Boozer integration tests use ``pytest.importorskip`` or a
skip-status payload. Missing optional packages therefore record skipped tests,
not a failed default installation.

In a file-backed optional run, the written ``workflow-summary.json`` must also
show ``no_solve_provenance_gate.status == "pass"``.  For that path the check
...
future kinetic-scalar workflow cannot silently lose required stages or
boundaries.

Promotion rule
--------------

This workflow is complete enough for documented research use when:
...
- the proxy-gradient check writes a summary JSON with
...
geometry path and a transport-objective gradient check. Until that exists, keep
