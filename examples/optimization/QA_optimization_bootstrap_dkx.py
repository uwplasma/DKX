"""Finite-beta QA optimization that minimizes the DKX kinetic bootstrap current.

This is ``vmex/examples/optimization/QA_optimization_bootstrap.py`` with one
...
for max_mode, max_nfev, n_spline in zip(MAX_MODES, MAX_NFEV, N_CURRENT_SPLINE):
    print(f"\n===== QA kinetic-bootstrap stage, max_mode = {max_mode} =====")
...
# Save results
input_path = final_input.to_indata("input.QA_bootstrap_dkx_optimized")
wout_path = vj.write_wout("wout_QA_bootstrap_dkx_optimized.nc", final_equilibrium.wout)
print(f"wrote {input_path}\nwrote {wout_path}")

# Plot results
monitor.save("QA_bootstrap_dkx_objectives.csv")
monitor.plot("QA_bootstrap_dkx_objectives.png")
plot_kinetic_bootstrap_current("QA_bootstrap_dkx_current.png", final_equilibrium,
                               kinetic, redl=bootstrap)
print("wrote QA_bootstrap_dkx_current.png")
