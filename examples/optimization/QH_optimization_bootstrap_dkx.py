"""Finite-beta QH optimization that minimizes the DKX kinetic bootstrap current.

This is ``vmex/examples/optimization/QH_optimization_bootstrap.py`` with one
...
# Reported, not optimized: the analytic estimate of the quantity being minimized.
...
objective_function_terms = [
    (qs, 0.0, 1.0), (kinetic, 0.0, KINETIC_WEIGHT),
...
report = opt.EquilibriumReporter(
    ("QS", qs.total, ".4e"), ("j_boot_dkx", kinetic.total, ".4e"),
...
for max_mode, max_nfev, n_spline in zip(MAX_MODES, MAX_NFEV, N_CURRENT_SPLINE):
    print(f"\n===== QH kinetic-bootstrap stage, max_mode = {max_mode} =====")
...
# Save results
input_path = final_input.to_indata("input.QH_bootstrap_dkx_optimized")
wout_path = vj.write_wout("wout_QH_bootstrap_dkx_optimized.nc", final_equilibrium.wout)
print(f"wrote {input_path}\nwrote {wout_path}")

# Plot results
monitor.save("QH_bootstrap_dkx_objectives.csv")
monitor.plot("QH_bootstrap_dkx_objectives.png")
plot_kinetic_bootstrap_current("QH_bootstrap_dkx_current.png", final_equilibrium,
                               kinetic, redl=bootstrap)
print("wrote QH_bootstrap_dkx_current.png")
