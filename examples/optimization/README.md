...
- QA nfp=2 neoclassical optimization proxies with explicit `dkx`
  high-fidelity promotion gates from completed `scan-er` outputs
- editable VMEX-style QA optimization with an optional bootstrap-current
  objective
...
- `qa_nfp2_dkx_objectives.py` — fast JAX proxy lane for adding
  ...
  designs still need real `dkx scan-er` outputs before kinetic
  validation or publication claims.
- `qa_nfp2_bootstrap_current_comparison.py` — focused teaching and README
  figure generated from the real VMEX QA optimization output. It
  ...
  `vmex` result directory. This panel is an equilibrium diagnostic until
  promoted with completed `dkx scan-er` outputs.
- `QA_optimization_bootstrap_current.py` — editable `vmex`-style QA
  optimization script with all knobs at the top. It is intentionally close to
  the VMEX QA optimization example, uses `MAX_MODE=3` for
...
```bash
python examples/optimization/qa_nfp2_dkx_objectives.py --objective balanced --steps 120 --out-dir runs/qa_candidate01/proxy --stem candidate01_proxy
python examples/optimization/launch_dkx_candidate_scan.py --proxy-summary runs/qa_candidate01/proxy/candidate01_proxy.json --input runs/qa_candidate01/input_r0p50.namelist --out-dir runs/qa_candidate01/scan_cpu/r0p50 --er-min -3 --er-max 3 --n-er 7 --jobs 4
...
python examples/optimization/evaluate_dkx_promotion_scan.py --scan-dir runs/qa_candidate01/scan_cpu/r0p50 --out-dir runs/qa_candidate01/audit --stem candidate01_r0p50_cpu --require-electron-root
...
python examples/optimization/evaluate_dkx_promotion_scan.py --scan-dir runs/qa_candidate01/scan_gpu/r0p50 --out-dir runs/qa_candidate01/audit --stem candidate01_r0p50_gpu --require-electron-root
python examples/optimization/compare_dkx_promotion_runs.py --cpu runs/qa_candidate01/audit/candidate01_r0p50_cpu.json --gpu runs/qa_candidate01/audit/candidate01_r0p50_gpu.json --out-dir runs/qa_candidate01/audit --stem candidate01_r0p50_comparison
```
