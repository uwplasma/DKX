#!/usr/bin/env python
"""Evaluate high-fidelity dkx promotion gates for an optimization scan.

Use this script after a proxy optimization has selected a candidate geometry and
you have run an ``dkx scan-er`` over the candidate.  The script reads the
...
from dkx.io import write_sfincs_h5  # noqa: E402
from dkx.workflows.optimization import evaluate_sfincs_scan_promotion  # noqa: E402


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--scan-dir", type=Path, help="Completed dkx scan-er directory.")
    parser.add_argument("--out-dir", type=Path, default=Path.cwd(), help="Directory for JSON/PNG/PDF outputs.")
    parser.add_argument("--stem", default="qa_nfp2_dkx_promotion_scan", help="Output file stem.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.json:
        ...
        print("dkx optimization promotion scan complete")
