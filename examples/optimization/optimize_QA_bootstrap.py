...
SIGNGS = int(eq_shaped.wout.signgs)  # handedness of the equilibrium (-1 for VMEX)
...
def kinetic_moments(booz, x0=None, recycle=None):
    ...
    # The template has psiAHat > 0; convert the Boozer route's handedness so
    # <j.B> carries the sign of the VMEC-file route and of Redl.
    op = convert_boozer_route_handedness(
        op, signgs=SIGNGS, g_hat=booz["bvco_b"][0], iota=booz["iota_b"][0],
        i_hat=booz["buco_b"][0])
