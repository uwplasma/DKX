#!/usr/bin/env python
"""QA nfp=2 optimization lane with dkx neoclassical objectives.
...
1. A cheap, differentiable Boozer-spectrum proxy objective is used inside the
   optimizer loop.
2. Accepted designs are promoted to high-fidelity ``dkx`` kinetic gates
...
from dkx.workflows.geometry_adapters import boozer_bhat_from_spectrum  # noqa: E402
from dkx.workflows.optimization import (  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--stem", default="qa_nfp2_dkx_optimization_lane", help="Output file stem.")
...
def _promotion_plan() -> dict[str, Any]:
    return {
        "claim_boundary": (
            ...
            "accepted designs to pass high-fidelity dkx kinetic gates."
        ),
        "required_high_fidelity_gates": [
            "same-profile DKX Er scan over each selected radius",
...
def main(argv: list[str] | None = None) -> int:
    ...
    result["workflow"] = "qa_nfp2_dkx_neoclassical_optimization_proxy"
    result["nfp"] = 2
    ...
    if args.json:
        ...
        print("QA nfp=2 dkx optimization proxy complete")
