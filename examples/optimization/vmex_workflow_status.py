...
``examples/autodiff/vmex_to_boozer_sfincs_pipeline.py`` gate. It does not
import ``vmex`` or ``booz_xform_jax``. It reports whether those optional
...
def build_status(
    *,
    ...
    return {
        ...
        "commands": {
            "preflight": _command(["python", "examples/optimization/vmex_workflow_status.py", "--json"]),
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--strict",
        action="store_true",
        help="Return nonzero when vmex or booz_xform_jax is unavailable.",
    )
    return parser
