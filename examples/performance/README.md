...
- `benchmark_sharded_solve_scaling.py` — benchmark single-case CPU/GPU sharded RHSMode=1 solves. For hot-solve scaling checks, use `--inner-warmup-solves 1 --sample-timeout-s 300 --rhs1-precond theta_schwarz --schwarz-coarse-levels 2`; add `--deterministic-output-probe` to record a bounded 1-vs-N residual/digest gate.
...
- `benchmark_transport_parallel_scaling.py` — benchmark transport-worker scaling on CPU or GPU (`--backend cpu|gpu`). The release-grade multi-GPU transport-worker path uses `--backend gpu` on `transport_parallel_2min.input.namelist`.
