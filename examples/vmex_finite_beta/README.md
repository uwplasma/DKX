...
The script runs the bundled `input.nfp2_QA_finite_beta` deck with `vmex`,
writes a VMEC-style `wout` file, uses that file directly in `dkx`
...
This is a primal finite-beta transport example. It uses `vmex` to generate
VMEC-style equilibrium data, then hands a `wout` file to `dkx`; it does
not differentiate through VMEX setup, file I/O, scheme-5 geometry evaluation,
...
an optimizer. It evaluates the Redl bootstrap-current formula with `vmex` on
the reactor-scale Landreman-Paul QA example and can optionally run `dkx`
on the same surfaces to compare
`FSABjHatOverRootFSAB2 * e n_bar sqrt(2 T_bar / m_bar)` against
`<J.B>/sqrt(<B^2>)` from the Redl fit:

```bash
python examples/vmex_finite_beta/compare_landreman_paul_qa_bootstrap_redl.py --skip-sfincs
python examples/vmex_finite_beta/compare_landreman_paul_qa_bootstrap_redl.py \
...
to compare a whole-radius `dkx` RHSMode=1 scan with the Redl bootstrap
formula. For a user-facing QA or QH `dkx` versus Redl plot that does not
...
`vmex` for the Redl algebra, but it does not need a SFINCS Fortran v3
...
runtime/non-autodiff lane (`DKX_IMPLICIT_SOLVE=0`) with the direct-tail
...
For a user-facing `dkx` versus Redl-only plot, add `--jax-vs-redl` (alias
...
`--verbose-sfincs` or set `DKX_EXAMPLE_VERBOSE=1` for production-grid
...
If `vmex` is installed from a source checkout, point the example at it:

```bash
export DKX_VMEX_ROOT=/path/to/vmex
```

Outputs are written under `examples/vmex_finite_beta/output/` by default and
include `finite_beta_vmex_sfincs_bootstrap_er.png`,
