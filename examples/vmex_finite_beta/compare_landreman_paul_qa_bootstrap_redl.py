#!/usr/bin/env python
"""Compare DKX and Redl bootstrap current for Landreman-Paul QA.
...
current formula through ``vmex``, optionally runs ``dkx`` at the same
surfaces, and plots the normalized bootstrap-current profiles side by side.

The plotted comparison uses

* Redl: ``<J.B> / sqrt(<B.B>)`` from ``vmex.redl_bootstrap_jdotb``.
* DKX: ``FSABjHatOverRootFSAB2`` converted to SI using the standard
  ``e n_bar sqrt(2 T_bar / m_bar)`` scale.

Run a fast Redl-only plot from the ``dkx`` repository root:

    python examples/vmex_finite_beta/compare_landreman_paul_qa_bootstrap_redl.py --skip-sfincs

Run the bounded DKX comparison with radial coverage and numerical
...
from dkx.magnetic_geometry import read_vmec_wout  # noqa: E402
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "<vmex-root>/examples/data/input.LandremanPaul2021_QA_reactorScale_lowres."
        ),
    )
    parser.add_argument(
        "--wout",
        ...
        help=(
            ...
            "<vmex-root>/examples/data/wout_LandremanPaul2021_QA_reactorScale_lowres_reference.nc."
        ),
    )
    ...
    parser.add_argument("--er", type=float, default=0.0, help="DKX Er value used for each surface.")
    parser.add_argument("--nu-n", type=float, default=8.31565e-3, help="DKX normalized collisionality.")
    parser.add_argument("--run-sfincs", action="store_true", help="Run missing DKX points.")
    parser.add_argument("--skip-sfincs", action="store_true", help="Do not run DKX; plot Redl and existing outputs only.")
    parser.add_argument("--skip-existing", action="store_true", default=True, help="Reuse existing sfincsOutput.h5 files.")
    parser.add_argument("--force", action="store_true", help="Overwrite and rerun DKX outputs.")
    ...
    parser.add_argument(
        ...
        choices=(0, 1),
        default=0,
        help="DKX collisionOperator: 0=full Fokker-Planck, 1=pitch-angle scattering.",
    )
...
def _require_vmex(root: Path):
    root = root.expanduser().resolve()
    if (root / "vmex" / "__init__.py").exists() and str(root) not in sys.path:
        sys.path.insert(0, str(root))
    try:
        import vmex as vj
        from vmex.finite_beta import redl_bootstrap_geometry_from_state
        from vmex.redl_bootstrap import polynomial_profile_and_derivative, redl_bootstrap_jdotb
        from vmex.wout import state_from_wout
    except ImportError as exc:  # pragma: no cover - optional dependency path
        raise SystemExit(
            "This example requires vmex. Pass --vmex-root or set DKX_VMEX_ROOT."
        ) from exc
    return {
        "vj": vj,
...
def _resolve_default_paths(args: argparse.Namespace) -> tuple[Path, Path]:
    root = args.vmex_root.expanduser().resolve()
...
def _sfincs_template(
    *,
    ...
    return f"""! Generated by examples/vmex_finite_beta/compare_landreman_paul_qa_bootstrap_redl.py
&general
  RHSMode = 1
    ...
    *,
    ...
    for idx, r_n in enumerate(np.asarray(redl["r_n"], dtype=float)):
        ...
        if output_path.exists() and not bool(args.force) and bool(args.skip_existing):
            should_run = False
        if should_run:
            print(f"Running DKX r_N={float(r_n):.4g} -> {output_path}", flush=True)
...
def _run_convergence_errorbars(
    *,
    ...
    real_args = _scan_args_with_resolution(
        args,
        out_dir=args.out_dir / "dkx_points_real_space_refined",
        ntheta=real_ntheta,
        nzeta=real_nzeta,
        nxi=int(args.nxi),
        nl=int(args.nl),
    ...
    velocity_args = _scan_args_with_resolution(
        args,
        out_dir=args.out_dir / "dkx_points_velocity_space_refined",
        ntheta=int(args.ntheta),
        nzeta=int(args.nzeta),
        nxi=velocity_nxi,
        nl=velocity_nl,
...
def _write_summary(
    *,
    ...
    payload = {
        "workflow": "landreman_paul_qa_dkx_redl_bootstrap_current_comparison",
        "claim_boundary": (
            "This diagnostic compares a DKX RHSMode=1 kinetic solve to the "
            "Redl/Sauter analytic bootstrap-current fit. It is a normalization and trend "
            "diagnostic, not a Redl-parity claim: DKX uses the supplied nu_n, "
        ...
        "normalization": {
            ...
            "redl_geometry_B_convention": (
                "vmex and SIMSOPT evaluate Redl trapped-particle geometry with the "
        ...
        "collisionality_contract": {
            ...
            "note": (
                "RHSMode=1 DKX uses the input nu_n together with dimensional "
...
def _write_plot(*, payload: dict[str, Any], png_path: Path, pdf_path: Path) -> None:
    ...
    if np.any(np.isfinite(sfincs)):
        if np.any(np.isfinite(errorbars)):
            axes[0].errorbar(
                ...
                lw=2.2,
                capsize=3.0,
                label="dkx (baseline +/- refinement)",
            )
        else:
            axes[0].plot(s, sfincs, "s--", color="#c0392b", lw=2.2, label="dkx")
    else:
        axes[0].text(0.5, 0.5, "Run with --run-sfincs\nto add DKX points", ha="center", va="center", transform=axes[0].transAxes)
    ...
    if np.any(np.isfinite(rel)):
        ...
        axes[1].text(0.5, 0.5, "No DKX outputs loaded", ha="center", va="center", transform=axes[1].transAxes)
...
def main() -> int:
    ...
    if args.json:
        ...
        print(f"Compared DKX and Redl at {compared} surface(s).")
        if compared == 0:
            print("Run again with --run-sfincs to generate missing DKX points.")
    return 0
