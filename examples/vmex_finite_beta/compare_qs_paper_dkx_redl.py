#!/usr/bin/env python
"""Paper-backed DKX / Redl comparison with an optional Fortran overlay.
...
DKX RHSMode=1 solves at the same normalized toroidal fluxes. If the
...
The script deliberately makes one plot only: DKX points, the Redl
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--zenodo-root",
        type=Path,
        default=Path(os.environ.get("DKX_QS_ZENODO_ROOT", DEFAULT_ZENODO_ROOT)),
    ...
    parser.add_argument(
        ...
        help="Comma-separated psi_N=s surfaces for DKX solves, or 'all' for every archived psiN_* deck.",
    )
    parser.add_argument(
        "--quick",
        action="store_true",
    ...
    parser.add_argument(
        "--solve-method",
        default=None,
        help="Optional DKX RHSMode=1 solve method, e.g. sparse_pc_gmres for large production-sized runs.",
    )
    parser.add_argument("--force", action="store_true", help="Rerun existing DKX outputs.")
    parser.add_argument(
        "--verbose-sfincs",
        action="store_true",
        help=(
            "Forward DKX phase/progress logging while running each kinetic solve. "
    ...
    parser.add_argument(
        "--keep-going",
        action="store_true",
        help="Record failed DKX points and continue the radial scan instead of stopping at the first failure.",
    )
    parser.add_argument("--skip-sfincs", action="store_true", help="Only evaluate Redl and any existing DKX outputs.")
    parser.add_argument(
        "--hide-fortran",
        "--jax-vs-redl",
        "--dkx-vs-redl",
        ...
        action="store_true",
        help=(
            "Plot only DKX and Redl. This mode does not load or require "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Set the DKX grid to the unique archived SFINCS Fortran v3 "
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Fail before plotting if DKX and the archived SFINCS Fortran "
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("outputs/qs_paper_dkx_redl"))
    parser.add_argument(
        "--fig-dir",
        type=Path,
        default=Path("docs/_static/figures/vmex_finite_beta"),
        help="Directory for PNG/PDF/JSON figure artifacts.",
    )
    parser.add_argument("--stem", default="qs_paper_dkx_redl_comparison")
    parser.add_argument(
        "--from-summary-json",
        type=Path,
        default=None,
...
def _require_vmex(root: Path) -> dict[str, Any]:
    root = root.expanduser().resolve()
    if (root / "vmex" / "__init__.py").exists() and str(root) not in sys.path:
        sys.path.insert(0, str(root))
    try:
        from vmex.redl_bootstrap import trapped_fraction_from_modb_sqrtg, redl_bootstrap_jdotb
    except ImportError as exc:  # pragma: no cover - optional dependency path
        raise SystemExit("This example requires vmex. Pass --vmex-root or set DKX_VMEX_ROOT.") from exc
    return {
...
def _solver_diagnostics_from_h5(h5: h5py.File) -> dict[str, Any]:
    """Return solver diagnostics written by dkx output generation."""
...
def _configure_benchmark_solver_defaults() -> None:
    ...
    """
    os.environ.setdefault("DKX_IMPLICIT_SOLVE", "0")
    os.environ.setdefault("DKX_RHSMODE1_FORTRAN_REDUCED_PC_BACKEND", "global")
    os.environ.setdefault("DKX_RHSMODE1_FORTRAN_REDUCED_DIRECT_CONSTRAINT_TAIL", "1")
    os.environ.setdefault(
        "DKX_RHSMODE1_FORTRAN_REDUCED_DIRECT_TAIL_PRECONDITIONER",
        "active_global_field_split_schur",
    )
    os.environ.setdefault("DKX_RHSMODE1_FORTRAN_REDUCED_DIRECT_TAIL_STRUCTURED_PC_REQUIRED", "1")
    os.environ.setdefault("DKX_RHSMODE1_FORTRAN_REDUCED_DIRECT_TAIL_PC_MAX_MB", "2048")
    os.environ.setdefault("DKX_RHS1_FULL_CSR_ACTIVE_ADAPTIVE_RESIDUAL_BASIS", "1")
...
def _run_or_read_dkx(
    *,
    ...
    metadata_path = run_dir / "dkx_run_metadata.json"
...
def _apply_matching_fortran_resolution(args: argparse.Namespace, selected_fortran_rows: list[dict[str, Any]]) -> None:
    ...
    A public DKX-vs-Fortran plot should not silently compare reduced JAX
...
def _resolution_comparison(
    *,
    ...
    return {
        ...
        "dkx_resolution_label": _resolution_label(jax_resolution),
        ...
        "claim": (
            "DKX and SFINCS Fortran v3 are compared on the same radial surfaces and resolution."
            if same_resolution
            else (
                "No SFINCS Fortran v3 overlay was loaded; this payload is a DKX-vs-Redl diagnostic."
                if not selected_fortran_rows
...
def _enforce_same_resolution_requirement(args: argparse.Namespace, comparison: dict[str, Any]) -> None:
    ...
    if not bool(comparison.get("same_resolution_on_compared_surfaces")):
        jax_label = comparison.get("dkx_resolution_label")
        fortran_label = comparison.get("sfincs_fortran_v3_resolution_label") or "mixed/unknown"
        raise SystemExit(
            "Refusing mixed-resolution DKX/SFINCS Fortran v3 figure: "
...
def _run_convergence_errorbars(
    *,
    ...
    return {
        "enabled": True,
        "definition": (
            "Error bars are the pointwise maximum absolute change in the DKX "
...
def _plot(payload: dict[str, Any], *, png_path: Path, pdf_path: Path) -> None:
    ...
    sfincs = [row for row in payload["dkx"] if row["status"] == "ok"]
    ...
    if metrics.get("max_jax_relative_difference_vs_redl") is not None:
        ...
        if metrics.get("dkx_elapsed_s_sum") is not None:
            lines.append(f"JAX solve wall = {metrics['dkx_elapsed_s_sum']:.1f} s")
    ...
    if hide_fortran:
        labels = ["DKX"]
        colors = ["#c43b2f"]
        performance_keys = ["dkx"]
    else:
        labels = ["DKX", "Fortran v3"]
        colors = ["#c43b2f", "#2d2d2d"]
        performance_keys = ["dkx", "sfincs_fortran_v3"]
...
def main(argv: list[str] | None = None) -> int:
    ...
    vmec = _require_vmex(args.vmex_root)
    ...
    payload = {
        "workflow": (
            "qs_paper_dkx_redl_bootstrap_current"
            if args.hide_fortran
            else "qs_paper_dkx_fortran_v3_redl_bootstrap_current"
        ),
        "paper": {
        ...
        "claim_boundary": (
            (
                f"{scan_kind} DKX-vs-Redl diagnostic; no SFINCS Fortran v3 "
                "executable or archived Fortran sfincsOutput.h5 overlay is used."
            )
            if args.hide_fortran
            else (
                f"{scan_kind} same-resolution DKX/SFINCS Fortran v3 diagnostic; "
            ...
            if same_resolution_fortran
                ...
                "not a DKX/SFINCS Fortran v3 same-resolution parity claim because the plotted grids differ."
            )
        ),
        ...
        "performance": {
            "runtime_total_s": {
                "dkx": float(sum(elapsed_values)) if elapsed_values else None,
                "sfincs_fortran_v3": (
                    float(sum(selected_fortran_elapsed_values)) if selected_fortran_elapsed_values else None
                ),
                "dkx_points": int(len(elapsed_values)),
            ...
            "memory_peak_mb": {
                "dkx": float(max(jax_memory_values)) if jax_memory_values else None,
                "sfincs_fortran_v3": (
                    float(max(selected_fortran_memory_values)) if selected_fortran_memory_values else None
                ),
                "dkx_points": int(len(jax_memory_values)),
        ...
        "metrics": {
            ...
            "dkx_elapsed_s_sum": float(sum(elapsed_values)) if elapsed_values else None,
            "dkx_elapsed_s_mean": float(np.mean(elapsed_values)) if elapsed_values else None,
            "dkx_elapsed_s_min": float(np.min(elapsed_values)) if elapsed_values else None,
            "dkx_elapsed_s_max": float(np.max(elapsed_values)) if elapsed_values else None,
            "dkx_solver_memory_mb_max": float(np.max(jax_memory_values)) if jax_memory_values else None,
