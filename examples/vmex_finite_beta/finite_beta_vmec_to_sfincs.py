#!/usr/bin/env python
"""Finite-beta VMEX to dkx bootstrap-current example.

This script is intentionally a single runnable example:

1. Read a finite-beta VMEC input deck.
2. Run VMEX and write a VMEC-style ``wout`` file.
3. Use that ``wout`` directly in a geometryScheme=5 dkx input.
...
``vmex`` is used to generate VMEC-style equilibrium data, but this script
hands a ``wout`` file to ``dkx`` and runs non-differentiated kinetic
...
try:
    ...
    raise SystemExit("This example requires matplotlib. Install dkx normally first.") from exc

from dkx.ambipolar import radial_current_from_output  # noqa: E402
...
def _require_vmex():
    ...
    env_path = (
        os.environ.get("DKX_VMEX_ROOT", "").strip()
        or os.environ.get("DKX_VMEX_PATH", "").strip()
    )
    if env_path:
        candidates.append(Path(env_path).expanduser())
    candidates.extend(
        [
            _REPO_ROOT.parent / "vmex",
            _REPO_ROOT.parent.parent / "vmex",
            Path.home() / "local" / "vmex",
        ]
    )
    for candidate in candidates:
        package_init = candidate / "vmex" / "__init__.py"
    ...
    try:
        ...
        raise SystemExit(
            "This example requires vmex. Install it with `pip install vmex` "
            "or set DKX_VMEX_ROOT=/path/to/vmex."
        ) from exc
    return vj
...
def _read_scalar_from_wout(path: Path, name: str) -> float | None:
    ...
    except ImportError:  # pragma: no cover - dependency is required by dkx
        return None
    try:
...
def _fill_wout_scalar_metadata(path: Path, *, run: Any, vj: Any) -> dict[str, float]:
    """Fill VMEC scalar metadata required by dkx if VMEX left it zero.

    Some VMEX development wout writers can emit valid mode/profile data with
    zero ``Aminor_p``/``Rmajor_p`` summary scalars. dkx uses these scalars
    ...
    try:
        from netCDF4 import Dataset
    except ImportError as exc:  # pragma: no cover - dependency is required by dkx
        raise SystemExit("This example requires netCDF4, which is a normal dkx dependency.") from exc
    ...
    if rcos.ndim != 2 or rcos.shape[0] < 1 or rcos.shape[1] < 1:
        raise RuntimeError("Could not infer Rmajor_p from VMEX state.Rcos.")
...
def run_vmex_to_wout(
    *,
    ...
    wout_path = out_dir / "wout_nfp2_QA_finite_beta_vmex.nc"
    if bool(skip_existing) and wout_path.exists():
        print(f"  VMEX wout: reusing {wout_path}", flush=True)
...
def _sfincs_template(
    *,
    ...
    return f"""! Generated by examples/vmex_finite_beta/finite_beta_vmec_to_sfincs.py
&general
  RHSMode = 1
...
def _run_or_load_sfincs_record(
    *,
    ...
    if skip_existing and output_h5.exists():
        ...
        t0 = time.perf_counter()
        print(f"{label} r_N={r_n:g}, Er={er:g}: running dkx", flush=True)
        write_output(
            input_path,
            output_h5,
            overwrite=True,
...
def _finite_beta_workflow_contract() -> dict[str, Any]:
    return {
        "workflow": "finite_beta_vmex_to_dkx_bootstrap_er_profile",
        "contract_version": 1,
        "differentiability": {
            "vmex_fixed_boundary_run": "primal_setup_not_differentiated_by_this_example",
...
def plot_summary(
    *,
    ...
    ax.set_title(rf"VMEX $|\hat B|$ contour at $r_N={representative_r_n:.2f}$")
    ...
    fig.suptitle("VMEX to dkx finite-beta bootstrap-current pipeline", fontsize=14, y=0.985)
...
def build_summary(
    *,
    ...
    return {
        "metadata": {
            "kind": "finite_beta_vmex_to_dkx_bootstrap_er",
            ...
            "normalization": (
                ...
                "the normalized quantities written by dkx."
            ),
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--stem", default="finite_beta_vmex_sfincs_bootstrap_er")
    parser.add_argument(
        "--quick",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("Finite-beta VMEX to dkx pipeline")
    ...
    wout_path, vmec_summary = run_vmex_to_wout(
