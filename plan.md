...
| 2026-08-28 | P2.4/P2.7 native Boozer reader | `p2/native-boozer-reader` | Added direct native ``Case`` execution from physical Boozer ``.bc`` inputs without a namelist adapter or user-facing SFINCS geometry number. The dedicated reader auto-detects scheme-11 cosine-only and scheme-12 asymmetric columns from one file read, parses once, and reuses immutable surface tables and one shape-stable phase-space grid. Checked scheme-11/12 detection and a two-surface asymmetric kinetic oracle pass; at a scientifically matched ``1e-10`` linear tolerance, particle flux, heat flux, and parallel current agree with the established scheme-12 route at ``2e-12`` relative tolerance. The inherited ``1e-8`` tolerance produced bit-identical operators but a 0.3206% observable difference from iterative conditioning, so the oracle was tightened rather than weakened. On the M4/24 GiB host with Python 3.11.15/JAX 0.9.2 and an initially empty external compilation cache, the checked two-surface asymmetric profile measured 8.265 s cold and 2.957 s warm inside ``dkx.run``, 1155743744 B cold and 1054703616 B warm maximum RSS, and 1.01e-13 maximum primal residual; its native NetCDF result is 47476 B. Isolated artifacts are 570570 B (wheel) and 920262 B (sdist), both pass ``twine check``. An out-of-checkout wheel install resolved DKX from ``site-packages`` under JAX 0.10.2, completed the same profile at 2.24e-15 maximum residual, and round-tripped a 47477 B NetCDF result. | Require every hosted scientific/package/docs gate before promotion. Independent SFINCS/MONKES/YANCC validation remains a separate P8.7 gate. |

## 22. Current next action

Complete the dedicated native Boozer vertical slice without converting a
physical ``Case`` back into a SFINCS deck. Require automatic checked-column
convention detection, one read/parse with immutable surface reuse, shape-stable
grid reuse, exact parity against the established kinetic kernel, M4 cold/warm
time and peak RSS, isolated artifacts, installed-wheel smoke, and all hosted
scientific/package/docs gates. Then begin pinned independent tokamak and two-
stellarator validation against SFINCS/MONKES/YANCC; do not promote beyond the
