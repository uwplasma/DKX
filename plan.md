...
**Step 3, one week: preconditioner reuse economics across a sweep.** The recycling half of this step has been run and stopped: over a ten-point `E_r` sweep, recycling cuts iterations by 1.18 to 1.28 times and does not reduce wall time at all, because the orthogonalizations cost more than the iterations they save (`docs/experiments/2026-09-07-recycling-and-preconditioner-reuse.md`). It is not why the Krylov route loses.

The same ablation found the lever the hypothesis missed. Holding the preconditioner fixed across the sweep costs 9 per cent more iterations and 2.4 times less wall time, because rebuilding it at every point costs more than the extra iterations. Reuse is nearly free while the operator stays nearby, degrades steeply as it does not, and fails outright far away: built at `E_r = 0` it costs 1.18× the iterations at `E_r = 9`, 2.3× at 20, 14× at 40, and does not converge at 80. That failure is reported rather than silent, `converged = False` with the original residual at 0.35 against a requested 1e-10, so a stale preconditioner costs time and is caught.

The work is therefore a refresh policy, not a constant, and it is the economics the reuse contract in section 5.1 already specifies: compare the rebuild cost against the expected extra iterations over the remaining horizon. Track the iteration count itself as the diagnostic and refresh when it climbs; a fixed distance threshold in `E_r` is the wrong mechanism because the useful window depends on the deck. Measured on an A4000 as well: holding the preconditioner fixed is 2.8 times faster on one species and 2.6 on two, against 2.4 on CPU, and the two-species case behaves like the one-species case. Two corrections come from that run. The iteration price of a fixed preconditioner is much higher on GPU, 110 against 401 over eight points, and still worth paying because rebuilding dominates. And recycling is *counterproductive* combined with a fixed preconditioner, 6.21 seconds against 10.63, while it helps when the preconditioner is rebuilt anyway; the policy should therefore hold the preconditioner and not recycle against it, which is the opposite of the pairing this step first proposed. Production-scale decks remain the gap before this becomes a default.

Admission test: a sweep and a root solve on at least one full-FP and one multispecies deck, total wall time and per-point iterations against the rebuild-every-point baseline, all points meeting the original-equation tolerance, and a stale-preconditioner case that is refused rather than accepted. Kill: no wall-time gain at fixed accepted accuracy once the refresh policy is charged.
...
What remains is the admission this step was written for and cannot be met without hardware: the campaign that showed the six decks completing ran on a 36-core, 62 GB machine, and the step asks for them within the office GPU's 16 GB with the original residual at or below 1e-10. That is the measurement to run when the accelerator is reachable.
