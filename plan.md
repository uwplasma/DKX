...
- Remaining risks: the remaining real CPU offenders are still HSX PAS DKES/full and the geometry11 PAS full-trajectory case, but the current A/B evidence says the next win is not another Krylov-method flip; the hottest remaining path is split between RHSMode=1 solve time and pre-solve/output-geometry assembly.
- Next actions: profile `sfincs_jax_output_dict` and related pre-solve geometry/diagnostic setup on the HSX/geometry11 offenders at finer granularity, then target the actual hotspot rather than adding more solve-method heuristics.
