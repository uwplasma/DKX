...
**Source.** 63 production Python files, 51,230 lines: 28 files and 6,230 lines above the section 6.2 budgets. 15 files exceed 1,200 lines and 7 exceed 1,800. Source still lives in repository-root `dkx/`. `import dkx` imports the JAX backend, mutates seven environment variables, creates `~/.cache/dkx/jax_compilation_cache`, and costs 0.46 s against 0.04 s for a bare interpreter.
...
**Dependencies.** `requires-python` is still `>=3.10` with a `tomli` marker and a Python-3.10 CI floor job, against the accepted 3.11 decision. `rich`, `myst-parser`, and `furo` are absent. The `structured` extra is a declared no-op.
