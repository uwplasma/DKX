...
The source audit also found a redundant principal-inverse application in SOLVAX's bordered preconditioner. [SOLVAX #101](https://github.com/uwplasma/SOLVAX/pull/101) reuses its already-computed transformed border, leaving the mathematical preconditioner unchanged. It is a draft pending complete DKX integration and post-change timing; finish that admission and recalibrate application cost before tuning a host refresh heuristic around the old cost.

Proceed in this order, within the existing step:
