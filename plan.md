...
Complete local, package, and hosted review of the native physical radial-flux
conversion and matched SFINCS certificate, then merge it. The independent
referee closes the unit conversion and one fixed-field PAS/DKES parity gate;
it does not make the speed/zeta matrix converged. Regenerate any promoted
whole-profile absolute-flux result before README figures or downstream
absolute-flux objectives. Continuous branch-event localization, experiment,
full-FP ambipolar comparison, Phi1, and a second stellarator family remain
separate gates.
