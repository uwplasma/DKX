...
| Fokker–Planck, iterations run away | The speed resolution, not the Legendre one and not the dropped collision coupling (`2026-09-19-nx-drives-the-iteration-growth.md`) | The triangle after a stall (#250); step 2 below addresses the conditioning the speed grid carries | Original residual; same answer as `coarse` where both converge |
...
So the work is to equilibrate the pinned chain before factoring it, not to retain more of the operator in it. Scaling the whole system consistently is a similarity transform and cannot change the spectrum of `A M⁻¹`; scaling inside the factorization changes how accurately the factors represent the inverse they claim to be, which is the mechanism the direct route already demonstrated — MKL PARDISO went from nine perturbed pivots and a relative residual of 9.4e-2 to `1.2e-10` on exactly this deck once equilibrated, and SOLVAX 0.24.0 carries `equilibrate`. That mechanism is measured. Against the pinned chain its own generators produce, the block-Thomas factors lose accuracy one speed at a time: at `Nx = 10` the backward error runs 2.1e-16 on the lowest-speed ion chain to 7.9e-8 on the highest-speed electron one, and at `Nx = 16` the top seven electron chains run 2.2e-10 to 3.2e-7, about a factor of three per speed point added. The chains are not ill-conditioned — assembled outright the first is 169 at `Nx = 10` and 652 at `Nx = 16` — they are badly scaled, in the same direction as the speed derivative, and the light species is worst because its streaming coefficients carry the `sqrt(T/m)` its collision diagonal does not. So equilibrate the chain blocks before factoring them, and read the worst subsystem's backward error and the iteration count together.
Admission: at `(Nxi, Nx) = (40, 16)` on the gap deck, at most half of `coarse`'s 7,167 iterations with an apply no more than 1.2× `coarse`; the same answer for `FSABjHat` to the eight digits the three preconditioner arms already agree on.
Kill: equilibrating the chain takes the worst subsystem's backward error to round-off and does not reduce iterations by 1.5×. Then the factors' accuracy is not what costs the iterations, the growth is in the eigenvector conditioning instead, and the next move is a spectral measurement of `A M⁻¹` against `Nx` rather than another preconditioner.
