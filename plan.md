...
| 2026-08-28 | P8.7 bounded independent MDKE validation | `p8/independent-validation` | Added a checksummed machine-readable zero-field PAS/DKES rung for a DSHAPE tokamak, NCSX, and W7-X EIM. The audit maps physical ``nu/v`` through ``nuDHat(x0)``, uses the local rather than LCFS radius in the Beidler references, corrects the raw ``B0`` scale, and records the handedness map. All four coefficients agree within 5.52%: maximum errors are 5.51% DSHAPE versus live YANCC, 1.66% NCSX versus live YANCC, and 5.52% W7-X EIM versus the pinned MONKES row; maximum ``D33*`` error is 0.065%. The artifact pins DKX/YANCC/MONKES/SFINCS commits, source and compact-output checksums, reference residuals, equations, grids, host, timing, RSS, and exclusions. Matched empty-cache M4 processes measured DKX cold/warm/peak RSS as 3.636/0.586 s/642826240 B (DSHAPE), 5.711/1.651 s/4780326912 B (NCSX), and 13.326/9.875 s/1027932160 B (W7-X). Local review passed the external-input audit, 99 focused validation/release/package tests, scoped Ruff/diff checks, warning-clean 46-page docs, 572540 B wheel, 923409 B sdist, ``twine check``, and an isolated wheel normalization smoke. This rung is explicitly not full-Fokker--Planck, finite-``Er``, ambipolar, experimental, or cross-code performance validation. | Require all hosted scientific/package/docs gates; then add matched full-kinetic SFINCS profile evidence without substituting unmatched equations. |

## 22. Current next action

Complete hosted review of the bounded P8.7 zero-field monoenergetic rung. Do
not widen its claim beyond the matched PAS/DKES equation. The next independent
slice should reproduce a full-kinetic physical-profile case against the pinned
SFINCS checkout, with matching collision operator, trajectories, coordinates,
profiles, tolerances, and convergence; finite-``Er``/ambipolar validation and
production dense-surface branch-event behavior remain later gates.
