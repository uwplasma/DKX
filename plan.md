...
Complete local, package, and hosted review of the native ambipolar launch
preflight. Then run the smallest two-surface admitted-grid root probe before
the five-surface campaign. A matched theta19/zeta37 diagnostic already shows
DKX/SFINCS agreement within 0.269% for flux and `1.3e-5` relative for current,
with both codes moving current by about 0.96% from theta15; this rules out a
simple current normalization mismatch but does not admit high-zeta profile
current. Keep bootstrap/current convergence separate from the transport-root
campaign. Continuous branch-event localization, experiment, full-FP
