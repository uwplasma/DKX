...
The work is therefore a refresh policy, not a constant, and it is the economics the reuse contract in section 5.1 already specifies: compare the rebuild cost against the expected extra iterations over the remaining horizon. Track the iteration count itself as the diagnostic and refresh when it climbs; a fixed distance threshold in `E_r` is the wrong mechanism because the useful window depends on the deck. Extend the measurement to a multispecies deck, where the preconditioner is larger and rebuilding costs more, before choosing the rule.
