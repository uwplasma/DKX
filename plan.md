...
Kill: the backward error of the coarse factors does not grow with `Nx`, or equilibrating the chain does not reduce iterations by 1.5×. Then the growth is in the eigenvector conditioning rather than the factorization, and the next move is a spectral measurement of `A M⁻¹` against `Nx` rather than another preconditioner.
