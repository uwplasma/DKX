...
1. **Measure coverage, then delete rather than test.** The gate is 95% line and branch; the last measurement was 90.6% and 78.5%. Section 7.1 already forbids reaching the number with shallow tests, so the uncovered-line report is the input to a dead-code sweep, not to a test-writing exercise. This is the one action that moves both the coverage gate and the file/line budget, which is why it comes first. Note that measurement itself has been awkward: two `pytest-xdist` runs died with workers exiting and the master hanging at 96%, so the reliable route is a sequential run on the office host.
