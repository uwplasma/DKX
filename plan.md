...
Status: P2 native execution and Result vertical slices in progress; native VMEC profiles are merged and native ambipolar profiles are the active slice
...
| 2026-08-28 | P8.2/P8.4 adaptive ambipolar evidence start | `p8/adaptive-ambipolar-evidence` | Added a deterministic bounded midpoint hierarchy that retains every kinetic evaluation with reason/level and every rung's search/total counts, root count, root movement, requested-observable movement, final bracket width, and convergence flag. Synthetic same-sign endpoints expose two hidden crossings after refinement; no-root, exact-root, exhaustion, deterministic-order, evaluation-bound, and memory-preflight gates pass. Result/NetCDF/summary/plot/docs distinguish `resolved`, `refinement_exhausted`, and `no_bracket_observed` while warning that finite sampling cannot exclude hidden even crossings. On the M4/24 GiB host with Python 3.11.15/JAX 0.9.2 and empty external caches, the opt-in checked two-surface case retained 14 of a conservative 153 evaluations per surface, reproduced roots `[-1.796875, -1.8359375]` kV/m, resolved after one midpoint rung with zero root/observable movement and 0.0390625 kV/m brackets, and measured 11.512 s cold, 6.576 s warm, 1656619008 B process peak, and 2.40e-15 maximum primal residual. A matched opt-out run retained 11 evaluations and measured 8.579 s cold/5.508 s warm/1774665728 B process peak; the 34.2% cold and 19.4% warm opt-in cost buys three retained midpoint evaluations per surface and does not affect the default route. Final wheel/sdist artifacts are 563956 B/910426 B; an isolated wheel run resolved both roots and round-tripped a 151746 B NetCDF result. Warning-clean 46-page docs, 131 focused native/batch/scan/source/package tests, and Ruff/diff checks pass. | Review the bounded opt-in slice and run hosted CI. After merge, implement P8.3 branch identity and creation/loss/merger/crossing events without hiding alternative roots. |

## 22. Current next action

Review the P8.2/P8.4 adaptive missed-root and convergence-evidence slice. Verify
the opt-in work/memory preflight, retained rung data, synthetic hidden-crossing
oracle, real kinetic roots, warning-clean docs, artifacts, and hosted coverage
without weakening the finite-sampling disclaimer. After merge, implement P8.3
branch identity and radial creation/loss/merger/crossing events as a separate
slice while keeping every alternative root visible.
