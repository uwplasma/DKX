...
**Step 2, one week: retain more of the collision coupling in the preconditioner.** This is where the measured mass is. `M` keeps only `mat[s, s, l, x, x]`, the self-species, speed-diagonal part of the collision operator, which is Fortran's `preconditioner_species=1` with `preconditioner_x=1`. SFINCS's own namelist offers weaker simplifications on the same axis, `preconditioner_x = 3` keeping the tridiagonal in speed and `preconditioner_x = 4` keeping the diagonal and superdiagonal ([the v3 `preconditionerOptions` block](https://github.com/landreman/sfincs/issues/1)), so the intermediate points exist upstream and are worth taking in order. Restoring a banded rather than diagonal speed coupling keeps each `(species, x)` subsystem's block-Thomas structure in `L` while making the retained operator closer to `A`; restoring the cross-species blocks does not, and belongs in a coarse correction rather than the band.

Admission test: GCROT iterations and wall time against the current diagonal preconditioner on the full-FP decks at 2,804, 15,844 and 66,004 unknowns, CPU and A4000, byte-identical operators, idle machine, A/B/A/B, with the original-equation residual as the acceptance rather than the preconditioned norm. Report the preconditioner's build and apply cost separately from the iteration count, because a stronger `M` that costs more to factor than it saves is a loss.

Kill: less than a 1.5× reduction in iterations at the 15,844 deck, or a net wall-time loss once the build cost is charged, after one week. Record and stop.
