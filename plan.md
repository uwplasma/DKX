...
*Corrected, 2026-09-22 (#281):* the chain has been closed before. `examples/optimization/optimize_QA_bootstrap.py` carries a gradient VMEX → Boozer → DKX → `FSABjHat` and agrees with finite differences to about 7e-3 on its dominant coefficient. What does not exist is the `QA_optimization.py` mirror — `least_squares`, inspectable residual rows, a Redl row beside the DKX row — and a residual that certifies the DKX number. #281 measured the chain again: `jax.grad` against central finite differences agrees to 6.6e-7, 6.6e-4 and 2.4e-3 on `RBC(1,0)`, `RBC(0,1)` and `ZBS(-1,1)`, inside the 1% bar and spanning `m = 0` and `1` (at a step of 1e-4; at 1e-3 the `m = 1` differences drift by 7% and 38%, because the map is strongly nonlinear there). It also found two things every DKX bootstrap number is blocked on. **The sign:** DKX gives ⟨j·B⟩ = −1.613e6 A T/m² where Redl gives +1.608e6 on the same surface and profiles, equal in magnitude to 0.3% — itself suspicious, since pitch-angle scattering usually overestimates the bootstrap current by about 40%; a coordinate-handedness convention is the leading suspect (the Boozer iota comes out negative). **The residual:** the default fast route solves only the lowest Legendre blocks exactly and reports an original-equation relative residual of 70, so the example must request the full solve before its number certifies anything.
...
*Corrected, 2026-09-22 (#279):* the premise of this paragraph is wrong, and so is its conclusion. The 2.0–2.6× was measured in eager, operation-by-operation execution. Under `jax.jit` the same `jax.grad` of `FSABjHat` costs **1.00× (median) and 1.11× (minimum)** of its primal on the 16,230-unknown structured-direct deck (office host, nine repeats) — inside the 1.3× gate with no code change — because the existing implicit solve already follows the Paul et al. pattern and the unused derivative of the factorization is removed at compile time. The explanation recorded with the 2026-09-20 measurement, that reverse mode re-executes the forward pass, is withdrawn: the eager cost is the number of individually dispatched operations, 13,698 for one gradient against 5,783 for a plain primal, and the solve step alone is some 250 triangular solves dispatched one at a time. The eager primal is itself 3.5–4× slower than the compiled one, so making the route compile itself speeds ordinary runs as well as gradients. Compiled, the factorization is about 85% of the primal and the reverse pass over coefficient construction costs almost nothing. Unmeasured under `jit`: the recycled Krylov route and the ambipolar root's 9.72×. So the gradient's cost is not what gates this phase, and no Tier 1 route or closure is needed on that account; making DKX compile its own path is the first step. One anomaly is open: compiled, the primal with `differentiable=False` is slower than with it on, 0.53 s against 0.37 s median.
...
*Corrected, 2026-09-22 (#278):* the second sentence above is wrong. On point A **no DKX route converges either**: the recycled Krylov route ran 20,000 iterations to 2.5e-5 (`2026-09-19-sfincs-on-the-gap-deck.md`). What DKX has on that deck and SFINCS lacks is the exact assembled operator in 4,800 products — the input to a solve, not a solution. On the 66,004-unknown collaborator grid both codes complete and SFINCS is the faster: MUMPS through SFINCS in about 40 s to 8.3e-14, DKX's direct route in 846 s to 1.3e-14. The case where SFINCS fails and DKX converges does not exist yet. Producing it is the task of this step, not its premise, and the restart finding of workstream 1 below is the most direct route to it. What DKX can show today where the reference falls short is a different kind of result: SFINCS's hard-coded `1d-12` threshold on small matrix entries moves the HSX bootstrap current by 12–19%, and with the threshold at zero SFINCS and DKX agree to 7e-11 (`2026-09-13-sfincs-sparsify-threshold.md`, landreman/sfincs#27). The README may show that, and states the gap deck as open in both codes.
...
### Phase 2d: the 2026-09-22 program — optimization-ready DKX, and the restored DKX 3 contracts

The owner's direction of 2026-09-22, recorded in full because it redirects the next weeks: make DKX's **derivatives** and DKX itself **fast enough, trading memory for speed where that helps**, to get the optimization of Phase 2c running; ground the solver work in a **literature review** of preconditioners, MUMPS, SuperLU_DIST and other direct methods, and adjoint or implicit-adjoint derivatives, putting generic numerics in SOLVAX; build the **VMEX optimization example** with a kinetic loss (bootstrap current first, then `Le1`/`Li1`); make the **README a showcase** with figures, including where SFINCS does not converge and DKX does and why, and with analytic-limit proof tests; bring the **documentation** up to date; **slim the repository** (`validation/`, `tools/`, `examples/`); and re-read the 2026-08-30 authoritative plan for what the project drifted from. That re-reading is section 11.

Five workstreams, run in parallel with disjoint files, each ending in a PR the maintainer reviews:

1. **Literature review** (no code). Preconditioners for the full-Fokker–Planck DKE and what SFINCS, MONKES, yancc, NEO-2, DKES and KNOSOS do about speed coupling; GMRES on non-normal operators (field of values, pseudospectra) for the open `Nx` question; sparse direct at this scale (MUMPS with BLR and out-of-core, SuperLU_DIST 3D and GPU, PARDISO, STRUMPACK, cuDSS) against a 62 GiB node with two 16 GiB A4000s; adjoint methods for neoclassical optimization, above all Paul, Abel, Landreman and Dorland, *J. Plasma Phys.* 85 (2019), the adjoint SFINCS for the bootstrap current; and the exact definition and source of `Le1`/`Li1`. Deliverable: a ranked recommendation list with the SOLVAX/DKX split, folded into this plan.
2. **Derivative cost** (DKX solver path, SOLVAX primitives). Profile before changing: where the 2.0–2.6× of `jax.grad` over a primal and the 9.72× of an ambipolar root actually go, given that factor reuse is complete and the adjoint runs on the primal's factors. The target formulation is the adjoint pattern: `dJ/dp = ∂J/∂p + λᵀ(∂b/∂p − ∂A/∂p x)` with `Aᵀλ = ∂J/∂x`, one transposed solve on the primal's factors and one vector–Jacobian product of the residual, evaluated once at the converged state, keeping factors, state and coefficients alive across forward and backward. Also the sparse transposed-solve floor (`3.18e-8` on the 1,204-unknown test deck), for which a transpose-aware equilibration is the first candidate. Admission: gradient ≤ 1.3× its primal on the structured-direct deck with a Taylor/finite-difference agreement near `1e-6`, and no change to any primal answer. Kill: the floor above 1.3× is measured and explained.
3. **VMEX kinetic optimization** (`examples/08_vmex_optimization/`). The chain of Phase 2c, correctness first: adjoint against finite differences within 1% on at least three boundary coefficients spanning two mode numbers, then the optimization mirroring VMEX's `QA_optimization.py` with a DKX bootstrap-current row beside the Redl one. `Le1`/`Li1` wait for workstream 1 to pin their definition. This replaces the geometry proxy that section 6 says slot 08 holds.
4. **Repository slimming** (`validation/`, `tools/`, `examples/`). Every file classified by who depends on it — runtime, CI or release gate, test fixture, figure source, evidence of a recorded result, or dead — before anything moves; dead files deleted with the evidence; recorded results verified to live in `docs/experiments/` before a raw artifact goes; the example ladder of section 11.6. Deletion-dominated PRs state added and removed lines separately against rule 2.
5. **README, documentation and proof tests.** The showcase of rule 12's revision; the analytic-limit tests of section 11.3 that exist audited and the missing ones added; documentation brought to release 2.6.0 (factor reuse, the assembled direct route and its equilibration, MUMPS, per-column acceptance, the split convergence verdicts, the transposed-solve limitation).

The measured baseline this phase starts from is in sections 2c, 11 and 12. Its results are appended to section 12 as they land, and a workstream whose premise a measurement destroys keeps its text here and gains the measurement beside it.

**Phase 2d results at the pause of 2026-09-22.** All five workstreams stopped at the owner's request with their work pushed and a draft PR each; nothing of theirs is on `main`. What each established:

- **Workstream 1, literature review** (record `docs/experiments/2026-09-22-literature-review.md`; questions 1, 3 and 4 answered, 2 partial). Ranked findings:
  1. **The `Nx` wall is most likely GCROT restart stagnation, not speed coupling.** DKX restarts at 200 with 8 recycled vectors. Its ladder is cheap while a solve fits in one cycle (174 and 187 iterations at `Nx` = 10 and 11) and explodes when it does not (3,810 at 12); on the small deck one unrestarted cycle reaches about 2e-10 at about 240 iterations against 340 restarted. SFINCS restarts at 2,000; yancc at 150 inner steps inside GCROT. Every experiment of this program ran at restart 200, so the first action is to rerun the ladder at restart ≥ 1,000 — about 5 GB of vectors on the full deck — and log the true-residual floor, before any new preconditioner.
  2. **`‖A − M‖/‖A‖` is the wrong attribution metric** when `M⁻¹` is large in some directions. Measure `‖(A − M)M⁻¹‖`, the field of values of `AM⁻¹` and the count of outlying eigenvalues, which on the small deck tracks the iteration count. This is why "a closer `M` converged worse" is not paradoxical.
  3. **The growth of `cond(ddx)` with `Nx` is explained**: the speed grid carries an `e^{-x²}` weight, so conditioning scales as `e^{x_max²}`, which predicts the measured factor of 10⁶ between `Nx` = 10 and 16. A speed-diagonal preconditioner cannot see it; only a non-diagonal change of speed basis changes what it drops.
  4. **NEO-2's recipe**: factor the local collision part, iterate the integral part, deflate the few slow modes with Arnoldi. SFINCS's `preconditioner_x_min_L`, full speed coupling at low `l`, is a cheap test.
  5. **An exact MONKES-style direct solve in Legendre with dense blocks** is about 2×10¹⁴ flops and about 1 GiB of working memory on the gap deck — minutes — against 14 h and 190 GiB for the general sparse projection. Its risk is stability, given the growth factor of about 3×10⁹ already measured in the per-chain elimination.
  6. **The ambipolar root**: Paul et al. fold the fixed-ambipolarity derivative into one extra adjoint solve (their eq. E7), which DKX can take from the tangent solve it already computes at the root; predicting the root from the sensitivity and taking one or two Newton steps should bring 9.72× to about 1.3–2× a solve.
  7. **`Le1`/`Li1`, found with about 95% confidence**: E. Lascas Neto, R. Jorge, C. D. Beidler and J. Lion, *Electron root optimisation for stellarator reactor designs*, J. Plasma Phys. 91, E24 (2025), arXiv:2405.12058, where they are `L₁₁ᵉ` and `L₁₁ⁱ`: SFINCS `RHSMode = 2` `transportMatrix[0,0]` from one single-species run per species, pitch-angle scattering only, DKES trajectories, `Nx = 4` — DKX's exact structured-direct route. The paper's printed objective (its eq. 46) and its code use inverse mass factors, `√(m_e/m_i)` against `√(m_i/m_e)`; a derivation from SFINCS's normalization favours the code and was not checked independently, so a test pins the factor before the objective is used.
  The per-solver sparse-direct survey (MUMPS BLR and out-of-core, SuperLU_DIST, PARDISO, STRUMPACK, cuDSS, A4000 FP64 rate) was not finished and is open.
  *Completed later the same day* (addendum of the record): a full-rank factorization of the gap deck needs about 150 GiB, so memory and not time is the wall; only MUMPS with block low-rank compression and out-of-core storage, or a MONKES-style Legendre-pair block elimination, fit 62 GiB. The cheapest decisive step is a MUMPS analysis-only run (`JOB=1`) on the 633,604-unknown matrix, which reports exact factor size, flops and memory in minutes. At a scaled `κ` near 3e10, BLR as a preconditioner admits only `ε ≲ 1e-8`. cuDSS, SuperLU_DIST and JAX's `spsolve` are not viable here. The A4000's float64 rate is disputed — NVIDIA's whitepaper gives 1/64 of float32, while an earlier measurement on these cards found 0.9–12× rising with size — and is measured before any GPU factorization is planned.
- **Workstream 2, derivative cost** (draft #279, record `docs/experiments/2026-09-22-derivative-cost.md` on its branch): the profile, reported as the correction to Phase 2c above — compiled, the gradient is already 1.00–1.11× its primal. Next: make the structured route compile itself for eager callers such as the optimization workflow's `jax.value_and_grad`, then measure the Krylov route and the root under `jit`, then a transpose-aware equilibration for the `3.18e-8` sparse transposed-solve floor (in SOLVAX if a primitive is missing), then a finite-difference check on three parameters.
- **Workstream 3, VMEX kinetic optimization** (draft #281): the chain's gradient passes the 1% bar; the sign and residual blockers are reported as the correction to Phase 2c above. Timings on the office host at load about 40: one DKX evaluation 103 s cold (almost all compilation) and 1.0 s warm; one gradient including VMEX's equilibrium adjoint 133 s cold and 10.7 s warm. Next: resolve the sign against DKX's own VMEC-file route, add the DKX and Redl rows to the `QA_optimization.py` objective list so VMEX's exact Jacobian carries them, request the full solve, then write rung 08 with its figure, a bounded test and the record. `Le1`/`Li1` get a marked extension point until the mass factor is pinned. The WIP prototype `examples/08_vmex_optimization/_wip_chain_gradient_check.py` is removed before the PR leaves draft.
- **Workstream 4, repository slimming** (draft #280): 52 files with no user anywhere deleted — the upstream SFINCS Fortran test harness under `examples/sfincs_examples/` (37 per-deck `tests.py`, `testsCommon.py`, `runExamples.py`), 9 cluster job files, 2 profile samples, 2 orphaned benchmark scripts and a redundant `examples/.gitignore`. **`validation/` cannot all be deleted**: 75 of its 80 files are sealed, checksummed evidence that `test_validation.py` audits live, and the files record each other's paths, so the section 11.4 layout needs a resealing step, not a move. Renaming `sfincs_examples/` to `compatibility/` is blocked by paths hard-coded in `src/dkx/workflows/scans.py`, `tools/release/release.py`, a figure generator and tests. Found for other owners: the `production-benchmark-inputs.yml` workflow calls `scripts/create_production_benchmark_inputs.py`, which is not tracked, so that workflow is broken; `docs/figure_provenance.json` credits `optimize_QA_bootstrap.png` to the QH script; `generate_er_trajectory_sweep.py` points at a deleted `examples/upstream/` tree.
- **Workstream 5, README, docs and proof tests** (draft #278): a new proof test, `tests/test_physics_limits.py`, reproduces Spitzer and Härm's conductivity ratio γ_E(Z) (Phys. Rev. 89, 977, 1953, Table III) from the full linearized Fokker–Planck operator: 0.58193, 0.68577, 0.78533 and 0.92338 against 0.5816, 0.6833, 0.7849 and 0.9225 for `Z` = 1, 2, 4 and 16, within 0.36%, unchanged to five digits from `Nx` = 8 to 24, asserted at 0.5%. The audit found the conservation, Maxwellian, transport-limit, Shaing–Callen/Boozer–Gardner and variational-bound proofs already in the tree, and **Onsager symmetry tested only on the monoenergetic coefficients and loosely** (`D₁₃* = −D₃₁*` to 1e-2–0.15), with no thermal-transport-matrix test. The README is not yet edited; it stands at 150 lines and 935 words against the enforced 160 and 1,000. The workstream also caught the overclaim corrected in Phase 2c step 4.
...
3. **No stacks without tooling.** Stacks are allowed only with depth at most 3, auto-rebase tooling, and each PR independently mergeable ([Graphite](https://graphite.com/guides/stacked-diffs)). Otherwise finish and merge PR n before opening n+1; at most three open PRs per author. Branch lifetime at most 48 hours ([trunk-based development](https://trunkbaseddevelopment.com/)); incomplete features land behind a flag.
   *Superseded in part, 2026-09-22, by the owner:* the 2026-09-22 program (Phase 2d) runs five parallel workstreams by explicit direction, each on its own branch and worktree with disjoint files, so "at most three open PRs per author" does not bind it. The 48-hour branch lifetime and independent mergeability still do.
...
6. **Three documents, three jobs, hard caps.** `plan.md` holds phases, figures and criteria and never status prose (this document; deletions weekly). `docs/adr/NNNN-*.md` holds decisions, one page each, immutable, superseded by a new ADR ([adr.github.io](https://adr.github.io/)). `CHANGELOG.md` holds what shipped per release ([Wilson et al. 2017](https://journals.plos.org/ploscompbiol/article?id=10.1371/journal.pcbi.1005510)). Execution detail lives in PR descriptions; review reports are dated write-once files under `docs/reviews/`.
   *Superseded, 2026-09-22, by the owner:* `plan.md` is the handoff **and** the logbook. It carries status and history, and revisions append and supersede in place; "deletions weekly" no longer applies. Section 0 states the maintenance convention and section 12 holds the logbook. ADRs, `CHANGELOG.md` and PR descriptions keep their jobs.
...
12. **README budget enforced by a test**: at most 160 lines and 1,000 words; at most two Python blocks of at most 16 lines each, the first a quickstart that runs `dkx.run` and the second ending in a gradient; at least one BibTeX block; the hero, parity and cross-code figures; and none of "has not", "cannot", "does not yet", "is not converged". Hedges live in the docs quadrant Diátaxis assigns them ([diataxis.fr](https://diataxis.fr/)). The numbers are set just above what the page currently needs, because the repository's own contracts force both code blocks and the measured-results table: `test_readme_quickstart_runs` requires a self-contained quickstart that prints a flux and a route, and `test_benchmark_doc_claims` pins fourteen measured tokens and the capability families. A budget the page cannot meet is not a budget. Measured exemplars: Diffrax 82 lines / 315 words, Optimistix 83 / 324, simsopt 85 / 403, DESC 137 / 623, jax-cfd 151 / 693; DKX was 250 / 1,508 with no citation block and no `jax.grad` before the 2026-09-06 rewrite, and had regrown from 158 lines three times.
   *Revisited, 2026-09-22, by the owner:* the README is to be a VMEX-style showcase — short self-contained sections, compelling figures, a case where SFINCS does not converge and DKX does *with the reason*, and a table of analytic-limit proof tests. The README PR of Phase 2d either fits that inside this budget or raises the test's numbers by the minimum needed and states the old and new values. Rule 12's other clauses stand. Its banned-phrase list sits against section 11.5's requirement that a limitation appear beside its claim; scope a claim positively ("on the HSX-like gap deck, one host") rather than with a banned phrase.
...
## 11. DKX 3 standing contracts, restored from the 2026-08-30 plan

The *DKX 3 Research-Grade Consolidation and Release Plan* of 2026-08-30 was the authoritative plan until the reconciliation of 2026-09-06 (#190). Its full text, 1,819 lines at its last revision, is in the git history at commit `6c4614d7`. The reconciliation kept its reduction goals in section 6 but softened or dropped its hardest targets: the package layout, the rule that a file over 1,800 lines is split, the compact test tree, the validation registry, and the explicit list of proof tests. The repository then grew in exactly those directions. The owner re-read it on 2026-09-22 and asked what can be kept; the answer is most of it. The contracts below are restored as standing targets. Where a later measurement changed one, the change is stated beside it rather than the target being quietly dropped.

### 11.1 Drift audit, 2026-09-22, at `main` `91e0582c`

| Contract (2026-08-30 section) | Target | Measured |
| --- | --- | --- |
| Production Python files (6.2) | about 35 | 61 |
| Production lines (6.2) | at most 45,000 | 51,480 |
| Files over 1,800 lines, split or explicit exception (6.2) | none | 8: `solve.py` 3,418; `drift_kinetic.py` 2,672; `cli.py` 2,458; `magnetic_geometry.py` 2,373; `workflows/optimization.py` 2,120; `representative.py` 1,932; `multigrid.py` 1,901; `phase_space.py` 1,870 |
| Files over 1,200 lines, justification required (6.2) | justified | 15 |
| Test modules (7.2) | 8–12, parametrized | 185, in 498 tracked files under `tests/` |
| User examples (9.1) | at most ten, each `run.py` + `case.toml` | 194 tracked files under `examples/` |
| `validation/` (7.8) | `registry.toml`, `cases/`, `summaries/`, `README.md` and one audit command | 80 files, 668 KB: a `registry.toml` beside 51 SFINCS inputs and campaign JSONs |
| `tools/` (7.8, 0.1 rule 5) | common runners, no one-purpose scripts | 103 files, 2.1 MB in six subtrees |
| Fresh clone (2) | under 20 MiB | `.git` about 20 MB, tracked files about 18.6 MB |
| README (8.4; rule 12 of section 7) | about 250 lines (2026-08-30), 160 lines and 1,000 words (2026-09-06) | 150 lines |
| Coverage (7.1) | ratchet 80 → 90 → 95 → 98 → 100 on stable reachable code | the 95% goal of section 6 |
| CI coverage shards (7.9) | 4–6 | 13 |

### 11.2 Source architecture and size discipline

One domain level under `src/dkx`, named by physics or numerics, never a deep framework. The 2026-08-30 target layout, a target and not a mandate to create files:

```text
src/dkx/
├── api.py  case.py  result.py  runtime.py  cli.py
├── geometry/     model.py  readers.py
├── kinetics/     grids.py  collisions.py  operator.py  moments.py
├── solvers/      linear.py  structured.py  krylov.py  preconditioners.py  implicit.py
├── workflows/    surface.py  profile.py  ambipolar.py  monoenergetic.py  convergence.py  optimization.py
├── io/           native.py  sfincs.py  plotting.py
├── physics/      units.py  validity.py  reduced.py
└── validation/   registry.py  compare.py
```

Budgets are review triggers, not line-count games: about 35 production files and at most 45,000 lines; a file over 1,200 lines carries a written single-responsibility justification, and one over 1,800 is split or given an explicit exception in this section. No attempt-, date-, device- or campaign-named source files; no duplicate public orchestration paths; no generic linear algebra in DKX that SOLVAX can own; no checked-in raw output, profiler trace, equilibrium archive or benchmark state.

Refactor by vertical slice and **delete while extracting**: establish a behaviour and evidence baseline, introduce the new owner, route one complete workflow through it, compare results, true residuals, runtime and memory, then delete the old owner and its tests in the same change. Split policy, data models, construction and formatting away from an equation kernel before splitting the kernel itself. `import dkx` imports no JAX backend, mutates no environment variable, initializes no distributed runtime and creates no cache; the CLI calls `dkx.runtime.configure()` before heavy imports.

The eight files over 1,800 lines are the concrete debt. `solve.py` at 3,418 lines is the first: it holds the three solve routes, the stall ladder, the assembled direct route with its factor reuse, and the policy that chooses among them, which are the `solvers/` owners above.

### 11.3 Test and proof architecture

Converge toward 8–12 substantive, parametrized test modules (`test_math`, `test_physics`, `test_numerics`, `test_solvers`, `test_workflows`, `test_io_cli_plotting`, `test_validation`, `test_performance_contracts`, `test_packaging`, plus the API). One test file per campaign, PR or artifact is not a test tree. Remove dated result directories, exact source-file inventories that block legitimate refactoring, exact line ceilings, tests of historical roadmap wording, duplicate frozen-output tests, and large fixtures that can be generated analytically. Never remove a scientific assertion to make a deletion pass. A test that only executes a line to raise coverage is not acceptable.

These properties are tested directly; they are what "full proof tests" means for DKX, and the README's proof table is drawn from them.

- **Mathematics.** Fourier differentiation exact to roundoff on representable modes; periodic derivative matrices annihilate constants; finite-difference and upwind stencils reach their declared order on manufactured functions; speed-grid quadrature integrates the Maxwellian moments the operators use; VMEC half/full-mesh interpolation and Boozer conventions against independently constructed fields.
- **Linear algebra and solver structure.** Block-tridiagonal extraction reconstructs the operator exactly; structured forward and transposed solves match dense and sparse referees; multi-right-hand-side factor reuse returns the same answers as separate solves; preconditioners, warm starts, recycled subspaces and cached factors do not change the converged answer; true-residual acceptance is independent of solver-internal estimates; mixed-precision factors are admitted only with double-precision refinement and a final true residual.
- **Roots.** Analytic current functions with known roots: single and multiple sign changes, exact-grid roots, closely spaced roots, no-root cases, tangential roots that do not change sign, branch creation, loss, merger and crossing, deterministic branch identity, seeded-interval against global scope.
- **Collision operators, per declared model.** Maxwellian null modes; particle conservation; momentum and energy conservation where the model claims them; interspecies exchange antisymmetry; weighted self-adjointness where applicable; non-negative entropy production; the Lorentz/PAS limit; Rosenbluth-potential equations and boundaries; species relabelling and the identical-species limit.
- **Drift-kinetic operator.** Null modes; parity and periodicity of streaming, mirror and drift terms; equivalence of trajectory models at `E_r = 0` where theory requires it; zero-drive solutions; manufactured solutions per term and for full combinations; bordered constraints remove only the intended null space.
- **Analytic transport limits.** Onsager symmetry where it applies; positive-semidefinite entropy-production form; variational bound ordering and gap closure under refinement; **Spitzer conductivity** and the analytic tokamak limits; the **Shaing–Callen / Boozer–Gardner collisionless bootstrap limit**; the **Pfirsch–Schlüter** high-collisionality limit; classical flux and impurity screening; monoenergetic-to-thermal convolution against manufactured coefficient functions.
- **Convergence.** Observed order on manufactured solutions; convergence of the requested observables, not only of state norms; independent refinement per axis and joint-tail checks; residual separated from discretization error; deterministic failure when a tolerance is below the achievable floor.
- **Derivatives.** JVP/VJP adjoint identities for the operator and moment maps; implicit linear-solve gradients against finite differences or complex step; ambipolar-root gradients on a fixed branch, with branch events marked non-smooth rather than differentiated through; gradient independence from Krylov history; Taylor tests for every stable objective.

### 11.4 Validation registry and evidence

Replace the forest of campaign JSON files and one-purpose audit scripts with `validation/registry.toml`, `validation/cases/`, `validation/summaries/` and a short `validation/README.md`, audited by one command that checks checksums, schema, status, paths, tolerances and claims. Each entry names the capability, status, claim, model, domain, input, reference, command, observables, tolerances, source tests, artifact, external commit and limitations. Large raw outputs live in GitHub release assets; Git keeps only compact summaries CI needs. A scientific conclusion lives in `docs/experiments/` before the raw artifact behind it is pruned, and retrieval is verified first. External codes are compared equations-first: geometry and radial coordinate, normalization, trajectory model, collision model, species and gradients, electric-field definition, resolution and tolerance, output conversion — SFINCS for full local parity, MONKES and DKES/NEO-2 for monoenergetic coefficients, yancc for an independent differentiable discretization, KNOSOS for bounce-averaged regimes, BOOTSJ and analytic formulas for the bootstrap current.

### 11.5 Documentation, README and prose

Documentation serves four needs, not the project's history (Diátaxis): getting started, tutorials, how-to guides, theory, reference, validation, development. Roadmaps, campaign diaries and agent logs stay out of user navigation. Each physics page states the question, the ordering, variables and units, the equations, coordinates and normalization, constraints, the derivation or a route to it, discretization and solver, inputs and outputs, domain of validity and failure modes, validation evidence, sources, and the owning source file.

The README, as the owner specified on 2026-09-22 and in the manner of VMEX's: short, self-contained sections, each detailed enough for a user to grasp its purpose, no long prose, and compelling figures — a feature showcase; the case where SFINCS does not converge and DKX does, **with the reason**; the one-factorization, many-solves result; a table of analytic-limit proof tests from section 11.3; and a capability table giving each capability's status. Install and a native quickstart come first; documentation and citation links last.

Prose rules: lead with the result or the user action; define each specialist term before use; never call a feature robust, production, exact or validated without naming the gate that makes it so; prefer measured quantities to adjectives; keep a limitation beside its claim, not on a distant page; one idea per paragraph.

### 11.6 Examples ladder and style contract

At most ten user examples, `examples/NN_name/run.py` with an equivalent `case.toml` where one exists: `01_tokamak_profile`, `02_vmec_stellarator`, `03_boozer_stellarator`, `04_monoenergetic_scan`, `05_ambipolar_profile`, `06_convergence_certificate`, `07_gradients`, `08_vmex_optimization`, `09_phi1_and_impurities`, and a compact, clearly separated `examples/compatibility/` set of SFINCS decks. No example is added before its public API is stable enough to copy. Slot 08 holds a geometry proxy (section 6) until Phase 2d workstream 3 replaces it with the real VMEX boundary optimization.

Each script is a user program in eight visible steps — imports; editable parameters; geometry and species; physics and numerical configuration; run; printed scientific summary with the residual and validity status; saved native result; publication-ready plot. No `argparse`, no `main()`, no `if __name__ == "__main__"`, no hidden helpers that belong in the library, no environment-dependent change to the physics. The docstring states runtime and regime. CI runs the small ones; heavy ones run nightly without mutating the user's case.

### 11.7 Performance: measurement, solver routes, reuse, and memory spent for speed

Every performance claim records the input and model, the unknowns, the requested and achieved true residual, the observable difference from the accuracy reference, and separately import, construction, compilation, factor or preconditioner setup, solve, post-processing, cold and warm wall time, peak process and device memory, iterations and products, what was reused, and the hardware, versions, precision and thread topology. JAX timings synchronize with `block_until_ready()`.

The solver routes of 2026-08-30 remain the frame: **S1** structured block elimination, exact where the Legendre structure is, with multi-right-hand-side and transposed factor reuse and a final true residual; **S2** matrix-free flexible recycled Krylov with physics-aware right preconditioning and a certified adjoint; **S3** the physics-aware coarse preconditioner, admitted only if documented, verified, pin-preserving and faster end to end; **S4** sparse direct, which the 2026-08-30 plan kept to a referee for bounded systems and which since 2026-09-19 is a production route through the assembly, with Ruiz equilibration, returned and reusable factors, and MUMPS as an optional SOLVAX backend — optional, not a normal dependency, which keeps the 2026-08-30 rule; **S5** experimental multigrid or collocation, promoted only on a representative regime matrix.

A solve separates into analysis, construction, factor or preconditioner, solve, certification and observable extraction, and a reusable state serves many right-hand sides, `E_r` scans, root refinement, adjacent surfaces, collisionality scans, optimization iterations and forward-and-adjoint pairs, each reuse recording why it is valid and when it is invalidated. JAX: jit the outer stable computation, keep hot functions pure, keep shapes static across a scan family, parse and choose policy outside jit, avoid closing over large arrays, measure `vmap` memory before using it, shard independent work before sharding a solve, and restrict mixed precision to preconditioners and factors with double-precision refinement. Parallelism order: independent cases, surfaces, scan points, right-hand sides, independent subsystems, then single-solve sharding only when a profile justifies it.

**Memory is spent for speed, deliberately (owner, 2026-09-22).** DKX keeps factors, converged states and operator coefficients alive across repeated solves and across a forward and its adjoint, and chooses `jax.checkpoint` policies to store rather than recompute where the host allows, because an optimizer evaluates hundreds of times and the 2026-09-20 measurements put the gradient's excess cost in recomputation, not in linear algebra. Every such trade reports its peak resident and device memory beside the time it buys, and a memory estimate — not a size constant — decides whether a route fits.

### 11.8 Definition of done for DKX 3

Code: one native high-level API, a permanent SFINCS compatibility namespace, an inert import, a shallow `src/dkx`, no duplicate execution stack, generic numerics in SOLVAX, about 35 production files and 45,000 lines unless justified here. Science: proof tests for every mathematical kernel; conservation, symmetry, entropy, limit and manufactured-solution tests for stable physics; convergence and uncertainty certificates; independent validation by capability; derivative and transposed-solve certification; CPU/GPU and route equivalence; a minimal regression test for every fixed bug. Users: `pip install dkx` and one command give a meaningful result, with progress, physical units, provenance in the output, nine or fewer examples that print, save and plot, and complete tutorials, how-to guides, theory, reference, validation and limitations. Performance: an official laptop and GPU suite, cold, warm and total time separated, bounded memory with a conservative preflight, reuse across scans, roots, profiles and adjoints, and an honest operational scope for hard cases. Distribution: wheel, sdist, installed files and a fresh clone each under 20 MiB; PyPI, conda-forge, DOI and citation metadata; enforced release gates.

## 12. Logbook

Append-only. One row per merged change or killed hypothesis, newest last. Detailed commands and numbers live in the PR or the `docs/experiments/` record the row cites; the row keeps what the next agent needs to know.

| Date | Change | Evidence | Consequence |
| --- | --- | --- | --- |
| 2026-08-30 | *DKX 3 Research-Grade Consolidation and Release Plan* written as the authoritative handoff | plan at `6c4614d7` | consolidation targets of section 11 |
| 2026-09-06 | Reconciled plan (#190) replaced it; #169–#191 merged | section 9 | reduction goals kept, hardest targets softened; drift recorded in 11.1 |
| 2026-09-13 | Independent review; preconditioner cost anatomy; SFINCS's `1d-12` sparsify threshold found to move the HSX current 12–19% | `2026-09-13-*.md` | DKX right, SFINCS threshold wrong for that case; landreman/sfincs#27 opened |
| 2026-09-15 | Release 2.4.0 | CHANGELOG | — |
| 2026-09-18 | Speed-triangle apply by back-substitution, 3.7× cheaper (#248); stall-ladder defaults and warm-started rungs (#250, #251) | `2026-09-18-*.md` | — |
| 2026-09-19 | Production solver program (#253). SFINCS v3 fails on the HSX gap deck (633,604 unknowns) by every route on 36 GiB: direct killed at 22.5 min, GMRES stagnant at 0.9955, `preconditioner_x = 2` killed at 30.6 min | `2026-09-19-sfincs-on-the-gap-deck.md` | the case DKX must win |
| 2026-09-19 | Operator assembled from products (#254; SOLVAX 0.23.0 `compression`); Ruiz equilibration (SOLVAX 0.24.0); PARDISO 9.4e-2 unscaled → 1.2e-10 scaled | #254, #255 | the missing capability was the scaling, not the ordering |
| 2026-09-20 | Direct route on the assembly (#256): 66,004 unknowns in 846 s to `1.3e-14`, a deck the route had refused; its reach is n^2.8 time, n^1.6 memory, so not the gap deck (#257) | #256, #257 | direct for small and medium decks |
| 2026-09-20 | **Killed:** collision-coupling attribution. Retaining the cross-species triangle exactly cost 17% more iterations; the factored splitting reached 5.1e5 error. `Nx` drives the growth, not `Nxi`, tolerance-independent (#258) | `2026-09-19-collision-coupling-is-cross-species.md`, `…-nx-drives-the-iteration-growth.md` | six causes excluded by #260 |
| 2026-09-20 | Assembly 8,800 → 4,800 products by dropping the divisibility rule (#259) | `2026-09-20-assembly-products-and-the-divisibility-rule.md` | structural bound is the angular lattice |
| 2026-09-20 | **Killed:** balanced solve. A diagonal similarity removing 1.5e10 of eigenvector conditioning moved iterations ≤ 1.05× (#260) | `2026-09-20-the-balanced-solve.md` | `Nx` question open; next probe field of values / pseudospectra |
| 2026-09-20 | Factor reuse across right-hand sides and the adjoint (#261): 3 factorizations → 0, sparse adjoint 0.15 of a primal, solve suites 406 → 247 s. `jax.grad` 2.0–2.6× and root 9.72× miss their gates, not closable by reuse | `2026-09-20-one-factorization-many-solves.md` | Phase 2d workstream 2 |
| 2026-09-20 | Release 2.5.0 (#262) | CHANGELOG | — |
| 2026-09-21 | Per-column acceptance (#265), magnetic upwind in assembly (#268), MUMPS opt-in (#267), optimization and convergence evidence (#264, #266, #269), memory units (#271); release 2.6.0 (#275); SOLVAX 0.25.0 with the MUMPS adapter | CHANGELOG | — |
| 2026-09-21 | Limitation, reproducible: sparse transposed solve reaches `3.18e-8` against `1e-10` on `tests/test_operator_assembly.py::DECK` (1,204 unknowns) with `default_rng(0)`, in 2.5.0 and 2.6.0; reported `converged=False` | 2.6.0 CHANGELOG | #265's "not reproduced" tested a different grid |
| 2026-09-22 | Phase 2c: the VMEX → `booz_xform_jax` → DKX chain and the validation matrix (#276); section 0 maintenance convention | #276 | — |
| 2026-09-22 | 2026-08-30 contracts restored (section 11); Phase 2d program started in five workstreams | this revision | results appended here as they land |
| 2026-09-22 | **Correction:** the case where SFINCS fails and DKX converges does not exist yet. On point A DKX's Krylov route reached 2.5e-5 in 20,000 iterations; on the 66,004-unknown grid SFINCS/MUMPS is faster (40 s against 846 s) | #278, `2026-09-19-sfincs-on-the-gap-deck.md` | Phase 2c step 4 corrected; README shows the sparsify-threshold result instead |
| 2026-09-22 | **Correction:** `jax.grad` costs 1.00–1.11× its primal when compiled; the 2.0–2.6× was eager dispatch (13,698 operations for one gradient). "Reverse mode re-executes the forward pass" withdrawn | #279 | first derivative step is making DKX compile its own path |
| 2026-09-22 | **Correction:** a gradient has been carried through VMEX → Boozer → DKX before (`optimize_QA_bootstrap.py`, 7e-3). #281 re-verified it to ≤ 2.4e-3 on three coefficients | #281 | rung 08 is the `QA_optimization.py` mirror, not a first |
| 2026-09-22 | **Open blocker:** DKX and Redl bootstrap currents opposite in sign, equal in magnitude to 0.3%; default fast route's residual 70 | #281 | no DKX bootstrap number is quoted until resolved |
| 2026-09-22 | Literature review: `Nx` wall most likely GCROT restart stagnation (restart 200 vs SFINCS 2,000); `cond(ddx)` ∝ `e^{x_max²}` explained; MONKES-style dense-block Legendre direct about 1 GiB on the gap deck; `Le1`/`Li1` = L₁₁ᵉ, L₁₁ⁱ of Lascas Neto et al., JPP 91 E24 (2025) | `2026-09-22-literature-review.md` | restart test is step 1 of section 13 |
| 2026-09-22 | Spitzer–Härm conductivity reproduced from the full FP operator within 0.36% (Z = 1, 2, 4, 16) | #278 | thermal Onsager test missing |
| 2026-09-22 | 52 dead files deleted; `validation/` shown to be 75/80 live sealed evidence; `production-benchmark-inputs.yml` found broken | #280 | slimming continues by classification |
| 2026-09-22 | Program paused by the owner; drafts #278–#281 hold the work with their handoffs | — | resume from section 13 |
| 2026-09-23 | Program resumed: steps 1–2, 5, 6a, 7, 13–16 run in parallel; #272 and #273 retargeted to `main` after their base merged, #272 rebased (conflict in `docs/performance.rst` resolved by keeping both paragraphs) | #272, #273 | step 17 partly done |
| 2026-09-23 | README/docs/proofs (#278): 8 analytic-limit tests pass — Spitzer–Härm within 0.36%, thermal Onsager `L₁₃ = L₃₁` tightening under refinement (PAS tokamak 2.2e-2 → 2.1e-3, full-FP 2.7e-3 → 6.9e-5), `L₁₂ = L₂₁` to 1e-13, single-species particle flux 1.6e-8 with full FP; README showcase inside the budget; docs brought to 2.6.0 | #278 | steps 13–15 done pending merge |
| 2026-09-23 | **Owner decision:** the README may exceed rule 12's budget; VMEX's README (300 lines, 1,654 words, 18 figures) is the reference scale, and `tests/test_readme_budget.py` is raised to that scale rather than removed | owner | README expansion on `docs/readme-vmex-scale` |
| 2026-09-23 | Slimming: 65 dead files removed from `tools/` and the SFINCS harness, `production-benchmark-inputs.yml` repaired (its script never existed in any commit; pointed at `tools.release.release production-inputs`, stale floor assertion fixed), `validation/README.md` index; examples 194 → 141 files. Kept: 10 benchmark scripts that are the only source of numbers in `performance.rst` | #280, #283 | renaming `sfincs_examples/` to `compatibility/` is blocked by paths in `src/dkx/workflows/scans.py`, `tools/release/release.py`, a figure generator and tests |
| 2026-09-23 | **Restart test (step 5) admitted:** the `Nx` wall is a GCROT restart artifact. At restart ≥ 1,000, `Nx = 16` converges in 357 iterations against 2,788 at restart 200 (2.05× the `Nx = 10` count, against 16×), Krylov time 216 s against 733 s; `(40, 16)` in 390 against 7,167. `Nx = 12` is separate: a residual floor near 1.1e-10, converging in 427 at `tol = 2e-10`. Restart vectors cost 2 × restart × unknowns × 8 bytes (flexible GMRES stores `V` and `Z`), and a longer restart slows every iteration because SOLVAX orthogonalizes against the whole padded basis | #285 | step 6 not needed; memory-aware restart default near 1,000 recommended; SOLVAX: orthogonalize only against the filled basis, drop `Z` for a fixed preconditioner; gap deck `(120, 16)` and the MUMPS analysis still to run |
| 2026-09-23 | **Bootstrap sign (step 1) resolved:** a Boozer-route convention, not a DKX library bug — `psiAHat = |phiedge|/2π · signgs · sign(G + ιI)`, set in `boozer_route_psi_a_hat` with a regression test. The VMEC-file route, and so the W7-X README figure, was already right. The 0.3% match with Redl was coincidence (pitch-angle scattering not converged in pitch, ±10%; Redl invalid on a non-quasisymmetric seed). Residual (step 2) fixed: 6.8e-14 and 1.2e-13. Rung 08 ran once: cost 22.8 → 0.40, FD agreement ≤ 4.2e-3 — but most of the bootstrap drop comes from the aspect-ratio row shrinking |B| from 1.4 to 0.34 T; the kinetic-row-off control is still to run. Four example scripts print the flipped sign on signed values; their squared objectives are unaffected | #281 | control run first; fix the four scripts in a separate PR |
...
One queue across every phase, ordered by what blocks what. Correctness blockers come first, because a fast wrong number is worse than a slow right one; then the cheap experiments that decide which expensive work is needed; then the build-out. Each step names its pull request where one exists, its admission and its kill. When a step lands, append its row to section 12 and strike nothing here — mark it done beside its text.

**A. Correctness blockers — before any bootstrap or optimization number is quoted.**

1. **Resolve the bootstrap-current sign** (#281). DKX gives ⟨j·B⟩ = −1.613e6 A T/m² where Redl gives +1.608e6 on the same surface and profiles, equal in magnitude to 0.3%. Compare DKX's VMEC-file route with its Boozer route on the same equilibrium, and check the handedness and iota-sign convention through `booz_xform_jax`. Also establish whether `examples/optimization/optimize_QA_bootstrap.py` and any shipped claim carry the same sign. Admission: the sign agrees, or the difference is a stated convention converted in one place with a regression test; the magnitude difference from Redl is then the expected pitch-angle-scattering one or is explained. Kill: none — this must be resolved.
2. **Certify the residual in the optimization route** (#281): request the full solve, not the lowest-Legendre fast route whose original-equation residual is 70.
3. **Merge #274** once its author takes it out of draft: failed bootstrap surfaces were converted to zero residuals, so an unsuccessful calculation looked optimal. All 24 checks pass.
4. **Repair the broken `production-benchmark-inputs.yml`**, which calls an untracked script, and the misattributed entry in `docs/figure_provenance.json` (#280 found both).

**B. The cheap experiments that decide the solver work.**

5. **The restart test** (literature finding 1). Rerun the `Nx` ladder of `2026-09-19-nx-drives-the-iteration-growth.md` on the office host at GCROT restart 200, 1,000 and 2,000, logging iterations, the true-residual floor, and the memory the restart vectors cost. Admission: at restart ≥ 1,000 the `Nx = 16` points converge in a small multiple of the `Nx = 10` iteration count. Then the `Nx` wall is a restart artifact, and the default restart becomes a memory-aware choice — memory spent for speed, section 11.7. Kill: iterations still grow with `Nx` at restart 2,000; then go to step 6.
6. **Only if step 5 is killed**, the attribution the literature says is the right one: `‖(A − M)M⁻¹‖`, the field of values of `AM⁻¹`, and the count of outlying eigenvalues against `Nx`. Then the NEO-2 route (factor the local collision part, deflate the few slow modes) and SFINCS's `preconditioner_x_min_L`.
6a. **MUMPS analysis only on the gap deck** (literature addendum): `JOB=1` on the assembled 633,604-unknown matrix, with METIS ordering and with a Legendre-pair-major user ordering, on the office host. It costs minutes and reports the factor entries, flops, and in-core and out-of-core memory, replacing every extrapolation of the direct route's reach. Measure beside it the A4000's float64 against float32 dense LU at `m` near 1e4, since that rate is disputed.
7. **Make DKX compile its own path** (#279). The eager primal is 3.5–4× slower than the compiled one, and compiled, the gradient is already 1.00–1.11× its primal. Compile the structured route's factorization, solve and operator applies inside the implicit solve for callers that do not `jit`. Then measure the recycled Krylov route and the ambipolar root under `jit`, and explain the one anomaly (compiled primal slower with `differentiable=False`, 0.53 s against 0.37 s). Admission: eager primal and gradient within 1.3× of their compiled times, compile cost reported, no primal change beyond roundoff.

**C. The optimization, once A is resolved.**

8. **Rung 08, the VMEX kinetic optimization** (#281): the `QA_optimization.py` mirror with a DKX bootstrap row and a Redl row as objective tuples so VMEX's exact Jacobian carries them, the full solve, a before/after figure, a bounded test and the record. Admission: the objective falls, the final equilibrium converges in VMEX, and every DKX number carries its original-equation residual.
9. **`Le1`/`Li1`** (Lascas Neto et al., JPP 91 E24, 2025): `L₁₁ᵉ` and `L₁₁ⁱ` from `RHSMode = 2`, single-species pitch-angle-scattering runs on the exact structured route. First, a test that pins the mass factor between the paper's eq. 46 and its code. Then add the objective beside the bootstrap row.
10. **The ambipolar root at optimization speed** (literature finding 6): the fixed-ambipolarity derivative folded into one adjoint from the tangent solve already computed at the root, plus predict-then-Newton across optimization steps. Admission: root ≤ 1.5× its first solve.

**D. The claim DKX has to be able to make (Phase 2c steps 3–4).**

11. **Produce the case where SFINCS fails and DKX converges**, which does not exist yet. The routes to it, in order of cost: step 5's restart result on the gap deck; the MONKES-style dense-block Legendre direct solve (about 2×10¹⁴ flops and 1 GiB there, stability to be established against the 3×10⁹ growth factor already measured); MUMPS through DKX's assembly on the office host. Admission: Phase 2c step 4 — at least three decks where DKX completes and a named alternative does not, with the alternative's failure recorded, and at least three where both complete and agree within their bars.
12. **The validation matrix** of Phase 2c step 3, across RHS mode, collision operator, geometry, ambipolar root, `Phi1` and species.
13. **The thermal Onsager-symmetry test under refinement** (#278 found it missing), beside Spitzer–Härm in `tests/test_physics_limits.py`.

**E. Showcase and documentation.**

14. **README showcase** (#278): VMEX-style sections with figures, built from claims that survive section 12's corrections — the sparsify-threshold result where DKX is right and SFINCS's `1d-12` cutoff is not; one factorization, many solves; the proof-test table; the capability table; the gap deck stated as open in both codes. Fit rule 12's budget, or raise it minimally with the old and new numbers stated.
15. **Documentation brought to 2.6.0 and beyond**, Sphinx built with warnings as errors.

**F. Repository and architecture (section 11).**

16. **Slimming continues** (#280): the six print-only `tools/parity` scripts and their CI lines, benchmark scripts cited only in prose (each quoted number checked for a remaining source first), `tools/performance/*`, a short `validation/README.md` index with the one audit command, then a separate examples PR. The `validation/` layout of section 11.4 is a resealing step and gets its own PR.
17. **Rework #263** so its four-item queue lands as supersessions rather than 2,128 deleted lines; **rebase #272 and #273** onto `main`, since their base branch merged.
18. **Split the eight files over 1,800 lines**, starting with `solve.py` (3,418) into the `solvers/` owners of section 11.2, deleting while extracting.
19. **The test tree toward 8–12 modules and CI toward 4–6 shards** (section 11.3), never dropping a scientific assertion.

*Status at the 2026-09-23 pause:* steps 13–15 (#278) and step 16's first round (#280, #283) are done and queued to merge; step 17 is half done (#272 rebased and queued; #273 retargeted, still a draft; #263 not reworked); steps 1–2, 5, 6a and 7 are in flight as drafts. **Next, in order:** read the four drafts' bodies; settle the bootstrap sign (step 1) before anything quotes a DKX bootstrap number; read the restart result (step 5), which decides between a memory-aware restart default and step 6; finish step 7; land the README expansion; then unblock the `compatibility/` rename (four hard-coded paths), rework #263 into supersessions, and split `solve.py` once #279 has merged.

**G. Release.** Cut 2.7.0 when steps 1, 2, 5, 7 and 8 have landed, with the changelog written from the PRs — entries have to be written at release time until a CI check requires them in the PR that makes the change.
