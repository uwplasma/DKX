...
Use PR #91's admitted native pitch-speed ramp evidence in the next bounded
diagnostic. The surface-local result shows that nearly equal active-mode totals
with different speed-local allocation change root topology and selected Er,
while the uniform route reaches a 31.86 GB footprint. Do not run pitch48 or a
uniform whole profile. Investigate a cheaper speed-local allocation/refinement
rule before packaging another phase-space rung. Zeta/speed convergence,
