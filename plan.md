...
| 2026-08-29 | P8.4 joint speed/zeta selected-tail diagnosis | branch `p8/joint-speed-zeta`, inputs `83473f5`, based on merged PR #99 | Completed the smallest fixed-pitch 2x2 speed/zeta matrix at speed 8/10 and zeta 37/45 on the two bounded W7-X surfaces and common sampled field 8.55 kV/m. Speed refinement moves particle/heat flux by as much as 7.55%/6.56%; zeta refinement moves them by 9.32%/9.41%, failing the unchanged 2% gates in every pair. Selected-tail upper bounds range from 7.96% to 9.71% and are non-monotone with zeta, so the metric is retained as conditioning evidence rather than promoted as a convergence oracle. Every accepted residual is below `1.23e-15`; cold/warm walls span 39.80-73.24/33.33-72.97 s and peak RSS remains below 2.29 GB. All four cold/warm scientific results are exact apart from timing. Compact and external-result audits pass with exact input, geometry, and eight-result checksums. The deliberately narrow field window observes no root bracket and no full-profile, performance, full-FP, Phi1, experiment, or independent-code claim is made. | Review and merge this bounded negative-evidence slice. Then run a matched fixed-field YANCC/SFINCS or MONKES reference before increasing the DKX grid. |

Preparatory joint speed/zeta review additionally passes 71 focused
artifact/manifest/planning/package/release tests, scoped Ruff/JSON/diff checks, and a
warning-clean 46-page documentation build. The isolated wheel and sdist are
580891 B and 945361 B and pass `twine check`; all generated build products
remain outside the checkout.

## 22. Current next action

Complete hosted review and merge of the prepared fixed-pitch 2x2 speed/zeta
evidence. Both axes remain unresolved: the matrix reaches 9.32%
particle-flux and 9.41% heat-flux movement while its finite tail bounds are
non-monotone. Do not spend the host envelope on another larger DKX grid. The
next scientific run should be a matched fixed-field YANCC/SFINCS or MONKES
reference with independently converged observables. Continuous branch-event
localization, experiment, full-FP ambipolar comparison, Phi1, and a second
stellarator family remain separate gates.
