...
1. complete the section 7.4 physics proofs that are genuinely missing. A fourth survey (2026-08-31) narrowed the list again, and two of the three items previously called gaps were already proved: moments of manufactured distributions landed in `tests/test_physics.py`, and the Shaing-Callen collisionless bootstrap limit has been covered all along by `tests/test_shaing_callen.py`, including the sqrt(nuPrime) convergence rate. The Spitzer conductivity limit is covered in its *monoenergetic* form (`D33* -> 1`, geometry-free, 3.0e-4) and in its momentum-restoration factor (Sugama rank-1, wide envelope against the exact 1.96). What remains is the *thermal* Lorentz conductivity closed form and the monoenergetic-to-thermal convolution against analytic manufactured coefficients;
...
The GPU lane is live: `~/venvs/dkx-gpu` on the office host runs jax 0.10.2 and reports both CUDA devices, so the 3.10-interpreter blocker recorded earlier is resolved and needs no further decision.
