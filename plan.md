...
Complete local, package, and hosted review of the bounded fixed-field
phase-space/SFINCS certificate, then merge it. Particle and heat flux now have
an admitted grid at this surface and field, but parallel current remains
theta-sensitive through theta 29 and is explicitly refinement-exhausted.
Before a whole-profile rerun, isolate theta-current conditioning and compare
the bootstrap/current conventions and limiting behavior against SFINCS and
STELLOPT's BOOTSJ/PENTA sources. Continuous branch-event localization,
experiment, full-FP ambipolar comparison, Phi1, and a second stellarator
family remain separate gates.
