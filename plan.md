...
| 2026-08-29 | P8.2 automatic ambipolar true-residual recovery | current ambipolar route PR | A production-grid five-surface W7-X standard-configuration PAS/DKES method case exposed a real structured-batch miss at `rN=0.7`, `Er=0`: `8.23448e-13` true residual versus the unchanged `4.4514e-13` target. The all-GMRES alternative was rejected after 346.94 s because it had not completed the first surface and reached 9855287296 B maximum RSS plus an 81547880576 B process-footprint measurement. The bounded automatic policy instead retries only failed fields with one scalar GMRES solve and retains both attempts, requested/executed routes, residuals, acceptance, and reason. The unchanged five-surface case then completed in 299.05 s wall, retained 221 structured attempts plus one GMRES recovery, reduced the failed point to `1.93235e-13`, kept every completed residual below that value, resolved every adaptive hierarchy, and found root counts `[1, 1, 3, 1, 1]` with selected fields `[11.3428, 11.6309, 7.4170, -2.8516, -6.7627]` kV/m. The process measured 14054981632 B maximum RSS and 17754733184 B peak footprint; its round-tripped native NetCDF is 339329 B. Explicit solver methods remain fail-closed. These are solver/workflow admission results, not phase-space convergence, experiment, full-FP ambipolar, or independent-code validation. | Complete focused/source/package/docs review and matched cold/warm measurements before hosted admission; then build the compact checksummed full-profile certificate as its own slice. |

## 22. Current next action

Admit the bounded automatic ambipolar route-recovery slice only after the real
five-surface workload, explicit fail-closed route tests, attempt-level NetCDF
round trip, installed artifacts, documentation, and hosted gates pass without
changing the linear tolerance. Then build a compact checksummed full-profile
certificate retaining all fields, true current residuals, root alternatives,
bounded electric-field refinement, cold/warm runtime, and peak memory. Keep
phase-space convergence, continuous branch-event localization, experiment,
full-FP ambipolar comparison, and a second stellarator family as explicit later
gates.
