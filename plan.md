...
| 2026-08-28 | P8.9 VMEC profile/plot regression | `p8/vmec-profile-plot-regression` | The representative command now consumes VMEC's evaluated `presf` independent of its input coefficient representation, retains `pmass_type` provenance, preserves failed surface records, and gives unavailable physics, parser failure, kinetic solve failure, and no-bracket evidence distinct HDF5 and plotted statuses. Synthetic wout cases exercise `power_series`, `gauss_trunc`, `two_power`, `two_lorentz`, `akima_spline`, `cubic_spline`, `pedestal`, `rational`, and `sum_atan`; standard wout density/temperature limitations are stated explicitly instead of inferred. On the checked vacuum tokamak fixture, the real quick command rendered both Er/bootstrap and SI particle/heat-flux panels, retained two `bracketed_root` surfaces and `power_series` provenance, and explicitly recorded the generic-plasma fallback. The empty-cache M4/24 GiB run measured 32.75 s wall, 2175057920 B peak RSS, a 198 KiB visually inspected PNG, and a 22 KiB HDF5 file. Ruff, 57 focused representative tests, 121 representative/planning/docs/package/source-contract tests, and warning-clean 46-page docs pass. Isolated wheel/sdist artifacts are 569549 B/918675 B, and an out-of-checkout wheel install imported from site-packages and reproduced VMEC profile classification. | Open hosted review; require the full coverage/docs/examples/Python-floor/installed-artifact gates before merge. A standard wout still cannot supply separate density/temperature profiles, and independent converged stellarator profile comparison remains outstanding. |

## 22. Current next action

Complete P8.9 VMEC profile/plot regression across the canonical evaluated
``presf`` contract, including ``sum_atan`` provenance and every checked VMEC
pressure representation. Verify that standard wout density/temperature limits
remain explicit, physical inputs plus completed solves render electric-field
and flux panels, and unavailable physics, parser failure, kinetic solve failure,
and no bracket remain distinct in HDF5 and plots. Run focused/broad tests,
warning-clean docs, isolated artifacts, installed-wheel smoke, and hosted gates
before merge.
