...
Complete local, package, and hosted review of interval-scoped seeded-bracket
promotion. The uniform admitted-grid hierarchy is rejected on the current host:
it completed no surface in 42.5 minutes and reached a 21.88 GB process
footprint. Next use the cheapest already reviewed DKX/SFINCS-informed discovery
grid to identify candidate sign-changing intervals, independently replay every
endpoint, and only then promote those intervals at the admitted transport-flux
grid. Do not describe the promoted interval set as a global all-root search.
