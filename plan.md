...
| Fokker–Planck, iterations run away | The dropped term is the collision operator's speed coupling | Speed coupling retained: the triangle after a stall (#250), then step 2 below | Original residual; same answer as `coarse` where both converge |
| Fokker–Planck, no iteration converges; or many solves of one operator (gradients, transport matrices, root iterations) | The matrix is sparse and can be assembled | Direct, through the assembly (step 1) | Backward error after refinement; assembly checked against the operator |
...
**Step 1, one week: a multifrontal factorization behind the assembly.** The assembly removed the reason the direct route was confined to `max_dense_size`; the factorization is what remains. Put a sparse direct backend in SOLVAX behind the interface `SpluFactorization` already defines: ordering on `A + Aᵀ`, scaling, iterative refinement to a backward-error target, transposed solves from the same factors, and a memory estimate before the factorization rather than a kill during it. Candidates, in order: an optional MUMPS binding; SuperLU with `MMD_AT_PLUS_A` and equilibration; a JAX-native elimination that uses the operator's known tensor structure instead of discovering it. In DKX, replace the size guard of the direct route with a memory guard fed by that estimate, and let `method="auto"` choose it when the estimate fits and an iteration has stalled.
Admission: the 66,004-unknown collaborator deck in at most 2× MUMPS's 40 s and 3.8 GiB, residual at most `1e-12` after refinement, and the transposed solve at the cost of a forward one. Then the 633,604-unknown gap deck within 48 GiB on the office host, which gives point A its first converged `Nx = 16` value.
...
**Step 2, one week: keep the whole collision coupling, without serializing the speeds.** The triangle is exact for `D + U` but solves the speeds in sequence, which costs 2.3× a `coarse` apply on four cores because one speed's slice is `n_species` chains where the default batches `n_species · n_x`. The collision operator is diagonal in `(θ, ζ)` and in `l`, so its whole coupling across speeds and species is one small dense block, `n_species · n_x` square, per `(l, θ, ζ)`. Apply it as a second factor: `M⁻¹ = (D_c + C_off)⁻¹ D_c (S + D_c)⁻¹`, with `S + D_c` the chains already factored, `D_c` the collision diagonal they contain, and `C_off` everything the simplified operator drops. The first factor stays one batched apply; the second is a batched small dense solve that touches no large factor. Both triangles and the species coupling are kept, which the speed triangle cannot do.
Admission: on point B at `Nx = 16`, at most the triangle's 2,599 iterations at an apply no more than 1.2× `coarse`; on NCSX `(25, 37, 61, 8)`, a solve faster than `coarse`'s 45.7 s, which the triangle misses; point A at `Nx = 16` converges.
Kill: fewer than 1.5× fewer iterations than `coarse` on NCSX after one week. The splitting error `S D_c⁻¹ C_off` is then not small where it matters; record it and keep the triangle.
...
Admission: iteration count within 10% of the fully coupled preconditioner of step 2 on the two-species HSX-like deck and on a three-species impurity deck. Kill: more than 1.5× the iterations.
