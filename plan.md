...
| 2026-08-29 | P8.5 native whole-profile certificate | current certificate PR | Added a portable checked TOML and a 20079 B machine-readable certificate for the exact five-surface W7-X standard-configuration PAS/DKES method case on merged #87. The artifact pins DKX `cb1ed04`, SFINCS geometry source `8df5453`, case ID `f284407a...f096bf`, the 1278 B input (`f2bfee1f...602e7`), the 19416676 B Boozer geometry (`81c686e5...b6062`), and both 339225 B native NetCDF results (`199fcf0e...b8d60`, `a3a790bf...0470c`). Matched empty/populated-cache processes measured 301.64/302.09 s wall, 13588021248/14178992128 B maximum RSS, and 17728371400/17770117688 B peak footprint; the 0.15% slower warm run supports no cache-speedup claim. The compact result retains every selected SI particle/heat flux and current, root counts `[1, 1, 3, 1, 1]`, alternatives and branch identities, seven discrete events including six nonsmooth warnings, two refinement rungs per surface, all 222 solver attempts, and the one rejected/recovered pair. Every final bracket is 0.0048828125 kV/m, maximum selected residual is `5.37e-15`, maximum accepted attempt residual is `1.93e-13`, and cold/warm scientific arrays are identical except timing. Self-contained and external-file audits pass. This remains workflow evidence, not phase-space convergence, continuously localized events, experiment, full-FP/Phi1 or independent ambipolar validation, cross-code performance, or a second stellarator family. | Complete manifest/docs/tests/package review and hosted admission, then make phase-space root/observable convergence the next independent slice. |

## 22. Current next action

Admit the compact native whole-profile certificate only after its checked TOML,
compact arithmetic, raw NetCDF/geometry checksum audit, manifest, documentation,
package gates, and hosted producers pass. Then add a separate phase-space
resolution ladder that quantifies movement in every root and selected flux
without conflating electric-field midpoint refinement with kinetic-grid
convergence. Keep continuous branch-event localization, experiment, full-FP
ambipolar comparison, Phi1, and a second stellarator family as explicit later
gates.
