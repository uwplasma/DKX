...
And that collapse is a diagonal scaling. LAPACK balancing, a diagonal similarity that leaves the eigenvalues untouched, takes the two numbers to 9.82e10 and 1.57e11 — seven to eleven orders removed and the `Nx` dependence flattened from 5,700× to 1.6×. This does not contradict the earlier point that a scaling cannot move the spectrum; it cannot, and it does not. It moves the basis that spectrum is expressed in, and that is the quantity convergence depends on here.

So the work is to solve the balanced system: scale the right-hand side, fold the diagonal into the preconditioner, and keep the answer in the original variables. Note this is a change to the solve and not only to the matrix — right-preconditioned GCROT minimizes its residual in the unscaled norm, and the point of balancing is to change that norm. The balancing diagonal also says which rows it corrects, which is the cheapest route to naming the operator term responsible; the standing candidate is the high-speed rows of the light species, where every other measurement in this program has landed.
Admission: at `(Nxi, Nx) = (20, 16)` on the production grid, at most half of `coarse`'s 2,788 iterations, with `FSABjHat` unchanged to the 1.6e-9 the preconditioner arms already agree on, and no loss at `Nx = 10`.
Kill: the balanced solve does not reduce iterations by 1.5× although the assembled operator's eigenvector conditioning falls as measured. Then the gap is between the balanced *matrix* and the balanced *solve*, and the next measurement is the GCROT residual curve in both norms rather than another preconditioner.
