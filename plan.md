...
- Validation for the Fortran-v3-reduced factor-policy split: targeted
  ...
  split.
- Validation for the symbolic frontal policy split: targeted `py_compile`,
  targeted `ruff`, full RHSMode=1 full-assembly tests (`116 passed`), adjacent
  sparse-pattern tests (`132 passed`), `git diff --check`, repository-size
  audit, and Sphinx HTML docs build passed locally. This moved frontal versus
  nested-dissection routing, active-size caps, separator/block limits, dense
  Schur update budgets, admission probes, and ND residual-polish controls into
