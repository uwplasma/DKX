...
| 2026-08-29 | P8.4 native pitch-speed ramp evidence control | PR #91, current branch | Added optional schema-v1 `resolution.pitch_speed_ramp` values 0/1/2 matching the supported SFINCS speed-local pitch truncation rules, while omitting the historical default 1 from canonical content so implicit and explicit defaults preserve every existing case ID. Native execution consumes the control directly without a namelist path and records the rule, all active pitch counts by speed, and their sum in immutable Result/NetCDF metadata. A small analytic kinetic oracle matches the accepted compatibility path at 2e-12 relative tolerance for both uniform option 0 and default option 1. Local review passed 59 focused native/ambipolar/schema/package tests, scoped Ruff/diff checks, a warning-clean 46-page docs build, a 573850 B wheel, and a 931609 B sdist. This is evidence plumbing, not a convergence result. | Run the full hosted gate. If admitted, design a bounded surface-local W7-X comparison that holds the speed-local truncation rule explicit; do not infer convergence or run a whole-profile uniform-pitch escalation without a memory preflight. |

## 22. Current next action

Finish hosted review of PR #91's native pitch-speed ramp control. Then use its
retained speed-local mode counts to design a bounded surface-local W7-X study
that separates maximum pitch order from the ramped truncation. Do not run
pitch48 or a uniform-pitch whole profile blindly: pitch40-to-pitch44 does not
approach the movement gates and pitch44 nearly consumes the 24 GiB process
footprint envelope. Zeta/speed convergence, continuous branch-event
