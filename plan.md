...
| 2026-08-29 | P8.4 bounded selected-tail reconstruction | PR #99 merged at `9eb8edee7f5648332dce01e1e1974e65f6feeccd` | SOLVAX PR #95 merged at `a726310`, then its corrected singular-leading-block selected-tail algorithm and exact selected-head reuse shipped in release PR #97 at `8799a9b` and tag/PyPI `v0.19.0`; all hosted SOLVAX platforms, exhaustive shards, and combined coverage passed. DKX reconstructs the exact final two active Legendre blocks after the zero-padded truncated solve and reports a separately named upper bound whose denominator is the exact union of retained low and tail modes; omitted middle-mode energy proves the bound cannot understate the corresponding full-state ratio. Uniform constrained and ramped independent full/GMRES referees match reconstructed tails from `1e-15` to `1.3e-11` relative, and synthetic tests prove the bound against the exact full-state ratio, including underflow scaling and overlap. The default is zero-cost and preserves existing case IDs. Opt-in workflow evidence replays only each surface's accepted selected field, counts that solve and attempt explicitly, and requires solver-tolerance equivalence plus the unchanged true-residual gate. A higher-zeta review probe exposed harmless batch-shape rounding at `3.76e-11` integrated relative movement and `1.08e-9` in one small speed-bin contribution; the diagnostic now gates complete speed spectra by species-wise vector norm at the requested solve tolerance, preserves the originally accepted observables and residual bit-for-bit, and attaches only the independently reconstructed tail evidence. The bounded W7-X speed-8/pitch-52 probe retained 32 finite ratios with maximum `0.08899451323082261`, no false exact-tail array, and `6.62e-16` maximum accepted residual. Cold/warm runs in one process measured 33.65/29.05 s wall, 32.11/28.85 s solve, and 1.32/1.60 GB recorded peak memory; every scientific array is exact across runs except timing. The compact and external-result audits, 80 focused tests plus the strengthened real replay-preservation regression, scoped Ruff/JSON/diff checks, and warning-clean 46-page docs pass locally. Wheel/sdist artifacts are 580667/945696 B and pass `twine check`; an isolated wheel with published SOLVAX 0.19.0 imported from `site-packages`, retained eight finite analytic bounds at `2.32e-15` maximum residual, and round-tripped a 223606 B NetCDF result. Every substantive hosted producer, combined coverage, docs, installed-wheel, example, external-data, Python-floor, workflow-lint, and required-job gate passed before the authorized squash merge. | Use the high-speed tail evidence to design the smallest joint speed/zeta probe; do not promote a full root/profile campaign. |
| 2026-08-29 | P8.4 joint speed/zeta selected-tail diagnosis | PR #100 merged at `0f11ba2bb0ded34b6708c62543b72c25c6410300` | Completed the smallest fixed-pitch 2x2 speed/zeta matrix at speed 8/10 and zeta 37/45 on the two bounded W7-X surfaces and common sampled field 8.55 kV/m. Speed refinement moves particle/heat flux by as much as 7.55%/6.56%; zeta refinement moves them by 9.32%/9.41%, failing the unchanged 2% gates in every pair. Selected-tail upper bounds range from 7.96% to 9.71% and are non-monotone with zeta, so the metric is retained as conditioning evidence rather than promoted as a convergence oracle. Every accepted residual is below `1.23e-15`; cold/warm walls span 39.80-73.24/33.33-72.97 s and peak RSS remains below 2.29 GB. All four cold/warm scientific results are exact apart from timing. Compact and external-result audits pass with exact input, geometry, and eight-result checksums. The deliberately narrow field window observes no root bracket and no full-profile, performance, full-FP, Phi1, experiment, or independent-code claim is made. Both docs builds, all 13 coverage producers, combined coverage, required aggregation, wheel, examples, external-data, Python-floor, and workflow-lint passed before the authorized squash merge. | Run a matched fixed-field independent referee before increasing the DKX grid. |
| 2026-08-29 | P8.4 native physical-flux conversion and matched SFINCS referee | PR #101 merged at `706748dc5752cc1b2f01c0b4c2950074733da0fe` | Source-level YANCC review first established that fixed-input transport moments require pitch 97/129 and a `1e-9` Krylov tolerance before the independent full-trajectory pitch movement falls below 0.89%; those values are excluded from matched parity because `solve_dke` retains trajectory terms outside the DKX PAS/DKES model. A pinned SFINCS v3 MUMPS solve then matched the exact thermal two-species W7-X surface, 8.55 kV/m field, PAS collisions, DKES ExB drift, Phi1-off policy, and ramp `[6,11,19,30,42,52,52,52]`. It exposed that native execution multiplied psiHat fluxes by `d(psiHat)/d(rHat)`, the inverse of the documented SFINCS `d(rHat)/d(psiHat)` factor. The corrected fresh native result agrees with SFINCS within 0.32% for particle/heat flux and 0.024% for parallel current, with `6.79e-16` DKX primal and `5.77e-15` SFINCS KSP residuals. SFINCS measured 10.48 s and 4.50 GB RSS; DKX measured 10.85 s and 1.04 GB RSS, as provenance rather than a performance-equivalence claim. Historical native validation artifacts are now explicitly marked as retaining valid same-surface relative movements, roots, topology, residuals, and timings while their absolute physical flux magnitudes are superseded. Compact and external-raw audits plus 84 focused native/ambipolar/units/planning/manifest/release/package tests pass locally, as do scoped Ruff/JSON/diff checks and a warning-clean 46-page docs build. The 580999 B wheel and 946421 B sdist pass `twine check`; an out-of-checkout wheel import from `site-packages` reproduces the exact corrected fluxes and `6.79e-16` residual in a 48114 B NetCDF result. Both docs builds, all 13 coverage producers, combined coverage, required aggregation, installed wheel, examples, external data, Python floor, and workflow lint passed before the authorized squash merge. | Regenerate promoted whole-profile absolute-flux evidence only after the relevant phase-space observables are admitted. |
| 2026-08-29 | P8.4 fixed-field phase-space and high-pitch SFINCS referee | PR #102 merged at `8309d744f1a384f60d85bbc66cd5ac82d3f970bb` | A source-guided one-axis ladder holds the W7-X standard `rN=0.5` surface, physical H/e profiles, 8.55 kV/m field, PAS collisions, DKES ExB drift, and Phi1-off policy fixed. Pinned SFINCS pitch 120 to 150 moves particle/heat/current by at most 0.191%; matched DKX at pitch150/zeta37 agrees within 0.269%. DKX then admits fixed-field particle/heat flux at theta15/zeta85/pitch150/speed8: pitch150 to 180 moves them by at most 0.790%, zeta85 to 109 by 1.72%, speed8 to 10 by 0.073%, and theta15 to 19 by 0.436%. Every DKX residual remains below `7.10e-14`; the high SFINCS residual remains below `4.04e-15`. Parallel current is not admitted: theta15 to 19, 19 to 23, and 23 to 29 move it by 7.35%, 7.15%, and 5.46%, while the theta29 pair costs 983.34 s and 4.47 GB RSS. The checked artifact therefore records fixed-field transport-flux admission and `parallel_current_status=refinement_exhausted`, with no whole-profile/root/bootstrap/performance promotion. Compact and external-raw audits, 131 focused artifact/manifest/planning/package/release/policy/figure tests, scoped Ruff/JSON/diff checks, and a warning-clean 46-page docs build pass locally. The 580999 B wheel and 947314 B sdist pass `twine check`. Both docs builds, all 13 coverage producers, combined coverage, required aggregation, installed wheel, examples, external data, Python floor, and workflow lint passed before the authorized squash merge. | Preflight the admitted transport-flux grid before launching a bounded whole-profile root campaign; keep current/bootstrap separate. |
| 2026-08-29 | P8.2/P8.4 native ambipolar launch preflight | PR #103 merged at `ca11f06585687708eed46ff5ac584bcea23aee5e` | Native-case validation exposes the same deterministic adaptive hierarchy and retained-evaluation capacity bound enforced at execution, including per-surface/profile solve counts and retained-evidence bytes, without loading geometry or JAX. The output explicitly labels this a capacity bound rather than a runtime estimate and repeats that finite sampling cannot exclude hidden even-numbered crossings. The checked five-surface W7-X input uses the fixed-field-admitted theta15/zeta85/pitch150/speed8 transport grid, excludes unadmitted bootstrap current from its observables, and reports 33 hierarchy points, at most 1023 evaluations per surface/5115 per profile, and 4746720 retained-evidence bytes. One hundred twenty-two focused schema/CLI/native-ambipolar/planning/package/release/source/docs tests, scoped Ruff/diff checks, and a warning-clean 46-page docs build pass locally. The 581709 B wheel and 948448 B sdist pass `twine check`; an out-of-checkout installed-wheel import reproduces the exact 306-evaluation/190944-byte analytic preflight. Every substantive hosted producer, combined coverage, both docs builds, installed wheel, examples, external data, Python floor, workflow lint, and required aggregation passed before the authorized squash merge. | Run the smallest two-surface admitted-grid root probe before committing to the five-surface campaign. |
| 2026-08-29 | P8.2/P8.4 admitted-grid launch diagnosis and seeded promotion | PR #104 merged at `757a2b9b84078668c7d7131aec992b875d8ff437` | The smallest two-surface theta15/zeta85/pitch150/speed8 uniform hierarchy passed the retained-evidence preflight (33 hierarchy points, at most 825 evaluations per surface/1650 per profile, 1531200 retained bytes) but did not complete one surface in 2551.33 s. The memory guard correctly rejected 87.2 GB reusable dense bands and 29.1 GB Schur factors, then used checkpointed row-on-demand elimination; the stopped process reached 10205478912 B maximum RSS and 21883225584 B peak footprint without writing a Result. The checked record labels this an operational uniform-route no-go, not a numerical failure or no-root result. Native cases can now promote independently reviewed discovery brackets at an expensive grid: every distinct endpoint and bisection point is a real retained kinetic solve, work/evidence is bounded per surface, failed/partial seeds are explicit, and Result/NetCDF/certificate/summary/plot label the scope `explicit_seeded_intervals_only` so unsampled crossings are never excluded. The design adopts PENTA's useful sign-change/search-range discipline but keeps DKX's stricter real-solve evidence instead of interpolated roots. Local review passed 184 focused native/batch/scan/schema/planning/package/release/policy tests, scoped Ruff/JSON/diff checks, and a warning-clean 46-page docs build. The isolated 584462 B wheel and 952733 B sdist passed `twine check`; an out-of-checkout wheel import found both analytic roots at `[-1.796875,-1.84375]` kV/m using 7 retained solves per surface and preserved the interval-only scope through a 212468 B NetCDF round trip. Hosted review initially exposed a real compatibility regression because serializing the new default `search_strategy = "uniform"` changed historical case IDs; commit `84ef68e` restored default omission while retaining semantic IDs for seeded cases. Both docs builds, all 13 coverage producers, combined coverage, examples, external-data, Python-floor, installed-wheel, workflow-lint, and required aggregation then passed before the authorized squash merge. | Independently verify the cheapest discovery-grid bracket endpoints before any admitted-grid root promotion. |
| 2026-08-29 | P8.2/P8.4 W7-X bounded bracket discovery and replay | PR #105 merged at `8c2010477d67c44e3d34a75592e35fbb90d685c7` | The cheapest reviewed two-surface W7-X PAS/DKES grid (theta15/zeta37/pitch36/speed6) retained root topology `[1,3]` in both cold and warm processes, with roots `[11.455078125]` and `[-0.1025390625,0.1806640625,8.4326171875]` kV/m, 41/57 evaluations, and maximum true residual `3.40e-14`. Cold/warm walls were 137.56/132.37 s, maximum RSS 3.85/3.75 GB, and scientific arrays were exact except timing. A separate interval-scoped replay then solved all eight distinct endpoints, retained a strict sign change in every one of the four final brackets, and reproduced the roots and brackets exactly in 12.10 s solve time with 1.10 GB maximum RSS and `8.72e-15` maximum residual. The checked inputs, geometry, raw inputs/logs/NetCDF results, environment, timings, memory, endpoints, and exclusions are checksummed. Compact and external audits, 69 evidence/manifest/release/package/source-policy tests, scoped Ruff/JSON/diff checks, and a warning-clean 46-page docs build passed locally. The isolated 584569 B wheel and 951387 B sdist passed `twine check`. This is bounded low-resolution discovery evidence only: `admitted_grid_promotion_ready=false`, and it does not exclude unsampled crossings. Both docs builds, all 13 coverage producers, combined coverage, examples, external-data, Python-floor, installed-wheel, workflow-lint, and required aggregation passed before the authorized squash merge. | Construct disjoint cross-rung candidate envelopes and replay them at theta15/zeta85/pitch150/speed8. Do not assume the narrow low-resolution brackets remain sign-changing or let one wide interval hide multiple crossings. |
| 2026-08-29 | P8.2/P8.4 W7-X admitted-grid seeded envelopes | branch `p8/w7x-admitted-seeded-envelope`, based on merged PR #105 | Promoted only explicit candidate intervals to the fixed-field-admitted theta15/zeta85/pitch150/speed8 grid. The broad two-surface run completed in 2835.73 s with 11.50 GB maximum RSS/22.66 GB peak footprint, found the inner root at 12.681640625 kV/m, and retained five strictly negative outer-surface endpoint currents from -2 through 11 kV/m as `seeded_bracket_failed`. A targeted `[11,14]` extension then found the moved outer root at 11.533203125 kV/m in 829.51 s with 2.00 GB maximum RSS. The final narrow-bracket cold/warm replay retained topology `[1,1]`, roots `[12.681640625,11.533203125]` kV/m, four real endpoint solves, strict signs, and every scientific array exactly except solve timing; cold/warm solve times were 272.07/258.88 s, recorded peaks 1.49/1.69 GB, and maximum residual `2.99e-14`. Compact and all-external audits, 108 workflow/evidence/manifest/release/package/source-policy tests, scoped Ruff/JSON/diff checks, and a warning-clean 46-page docs build pass locally. The isolated 584569 B wheel and 951711 B sdist pass `twine check`. The result scope remains `explicit_seeded_intervals_only`; lower outer branches were not observed at this grid and unsampled crossings are not excluded. | Complete hosted review. Next extend the admitted roots to the five-surface profile with independently discovered surface-local intervals; do not infer missing surfaces or bootstrap convergence. |

PR #100's joint speed/zeta review additionally passed 100 focused
artifact/manifest/planning/package/release/policy tests locally, scoped
Ruff/JSON/diff checks, and a warning-clean 46-page documentation build. The
isolated wheel and sdist were 580891 B and 945361 B and passed `twine check`;
all generated build products remained outside the checkout.

## 22. Current next action

Complete local, package, and hosted review of the two-surface admitted-grid
seeded certificate. The explicit promoted intervals resolve roots at
`[12.681640625, 11.533203125] kV/m` with exact cold/warm science, while the
lower outer-surface intervals remain explicit failed signs. Next extend to the
five-surface profile using surface-local discovery and narrow replay; do not
infer unmeasured surfaces, hide failed intervals, or describe the interval set
as a global all-root search.
A matched theta19/zeta37 diagnostic already shows DKX/SFINCS agreement within
0.269% for flux and `1.3e-5` relative for current, with both codes moving
current by about 0.96% from theta15; this rules out a simple current
normalization mismatch but does not admit high-zeta profile current. Keep
bootstrap/current convergence separate from the transport-root campaign.
Continuous branch-event localization, experiment, full-FP ambipolar
comparison, Phi1, and a second stellarator family remain separate gates.
