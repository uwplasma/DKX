...
| 2026-08-29 | P8.2/P8.4 W7-X bounded bracket discovery and replay | branch `p8/w7x-seeded-bracket-discovery`, based on merged PR #104 | The cheapest reviewed two-surface W7-X PAS/DKES grid (theta15/zeta37/pitch36/speed6) retained root topology `[1,3]` in both cold and warm processes, with roots `[11.455078125]` and `[-0.1025390625,0.1806640625,8.4326171875]` kV/m, 41/57 evaluations, and maximum true residual `3.40e-14`. Cold/warm walls were 137.56/132.37 s, maximum RSS 3.85/3.75 GB, and scientific arrays were exact except timing. A separate interval-scoped replay then solved all eight distinct endpoints, retained a strict sign change in every one of the four final brackets, and reproduced the roots and brackets exactly in 12.10 s solve time with 1.10 GB maximum RSS and `8.72e-15` maximum residual. The checked inputs, geometry, raw inputs/logs/NetCDF results, environment, timings, memory, endpoints, and exclusions are checksummed. Compact and external audits, 69 evidence/manifest/release/package/source-policy tests, scoped Ruff/JSON/diff checks, and a warning-clean 46-page docs build pass locally. The isolated 584569 B wheel and 951387 B sdist pass `twine check`. This is bounded low-resolution discovery evidence only: `admitted_grid_promotion_ready=false`, and it does not exclude unsampled crossings. | Complete hosted review, then construct disjoint cross-rung candidate envelopes and replay them at theta15/zeta85/pitch150/speed8. Do not assume the narrow low-resolution brackets remain sign-changing or let one wide interval hide multiple crossings. |
...
Complete local, package, and hosted review of the bounded W7-X discovery and
seeded-endpoint replay. The low-resolution `[1,3]` topology and all four strict
endpoint sign changes are independently reproducible, but they do not promote
the admitted grid. Next derive disjoint candidate envelopes from the reviewed
cross-rung root history, then evaluate their endpoints at theta15/zeta85/
pitch150/speed8. Keep intervals narrow enough that one bisection cannot hide
multiple known candidate crossings, and treat moved or failed signs as evidence
rather than widening claims. Do not describe the interval set as a global
all-root search.
