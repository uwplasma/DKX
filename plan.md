...
- Branch `refactor/v3-driver-architecture` remains the single draft-PR branch.
  The latest tranche moves generic sparse-PC GMRES finalization, post-MinRes
  polishing, and dtype-retry helpers into
...
- Largest remaining files after the first sparse package splits are
  `sfincs_jax/problems/profile_response/sparse_pc.py` (`13552` lines),
...
- `profile_response.sparse_pc` currently has 136 top-level functions and
  133 top-level classes. The sparse package split has moved 7 x-block rescue
  ...
  `profile_response/sparse/finalization.py`, while preserving the existing
...
3. Sparse profile-response package split: about 68%. This is the main blocker
...
5. Validation lane: about 88%. Focused sparse/profile-response tests and broad
...
1. Commit and push the sparse finalization split after final local hygiene.
2. Continue the sparse package split with direct-tail/finalization-bundle
   metadata or fortran-reduced/xblock-QI helpers next, choosing whichever
   boundary avoids circular imports and reduces `sparse_pc.py` most cleanly.
