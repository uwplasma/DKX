...
| --- | --- | --- | --- |
| 2026-09-01 | **Open defect found, not fixed:** the Boozer deck route and the converted-case route disagree on `FSABjHat` on Linux only. | Same commit, same equilibrium, both taking the structured direct route to a 1e-15 residual: agreement is 2.6e-15 on macOS/arm64 and 1.75e-4 on Linux/x86-64. Deck `FSABjHat` is 6.640227e-10 on macOS against 6.641136e-10 on Linux, so the deck route alone already differs by 1.4e-4 across platforms. Not solver tolerance. | Split into its own test, xfailed off-darwin with the measured numbers in the reason rather than relaxed to a passing bound -- the flux round-trip holds at 1e-8 on both platforms, so a loosened tolerance would hide a real discrepancy. Diagnose the non-stellarator-symmetric Boozer path next. |
