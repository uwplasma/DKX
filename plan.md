...
| 2026-08-29 | P8.2 automatic ambipolar true-residual recovery | PR pending, `68db134` | A production-grid five-surface W7-X standard-configuration PAS/DKES method case exposed a real structured-batch miss at `rN=0.7`, `Er=0`: `8.23448e-13` true residual versus the unchanged `4.4514e-13` target. The all-GMRES alternative was rejected after 346.94 s because it had not completed the first surface and reached 9855287296 B maximum RSS plus an 81547880576 B process-footprint measurement. The bounded automatic policy instead retries only failed fields with one scalar GMRES solve and retains both attempts, requested/executed routes, residuals, acceptance, and reason; explicit methods remain fail-closed. On the Apple M4/24 GiB host with macOS 26.6.2, Python 3.11.15, JAX 0.9.2, NumPy 2.4.3, and SciPy 1.17.1, matched empty/populated external-cache processes completed in 307.43/295.04 s wall (305.414/292.936 s inside `dkx.run`), measured 14409515008/14681489408 B maximum RSS and 17540823704/17707186672 B peak footprint, retained 221 structured attempts plus one GMRES recovery, reduced the failed point to `1.93235e-13`, and kept every completed residual below that value. Both runs resolved every adaptive hierarchy, found root counts `[1, 1, 3, 1, 1]`, and selected `[11.3428, 11.6309, 7.4170, -2.8516, -6.7627]` kV/m; every scientific/evidence array is identical across runs except timing. The 339329 B NetCDF certificates hash to `52b8d801...bdb7746` and `0dbbd74b...bad906`; the input and W7-X geometry hash to `180b5c33...d6439e` and `81c686e5...b6062`. Final local review passed 112 focused native/source/package tests, 27 route-specific native/batch tests, release gates, scoped Ruff/diff checks, warning-clean 46-page docs, a 573480 B wheel, a 927789 B sdist, and `twine check`. A clean out-of-checkout wheel install under JAX 0.10.2 imported DKX from `site-packages`, reproduced both analytic roots at `2.31e-15` true residual with 28 retained attempts, and round-tripped a 197134 B NetCDF result. These are solver/workflow admission results, not phase-space convergence, experiment, full-FP ambipolar, or independent-code validation. | Run every hosted producer and aggregation gate; if admitted, build the compact checksummed full-profile certificate as its own slice. |

## 22. Current next action

Open the bounded automatic ambipolar route-recovery PR after its real
five-surface workload, explicit fail-closed route tests, attempt-level NetCDF
round trip, installed artifacts, and documentation have passed without changing
the linear tolerance. Admit it only if every hosted producer and aggregation
gate is green. Then build a compact checksummed full-profile certificate
retaining all fields, true current residuals, root alternatives, bounded
electric-field refinement, cold/warm runtime, and peak memory. Keep phase-space
convergence, continuous branch-event localization, experiment, full-FP
ambipolar comparison, and a second stellarator family as explicit later gates.
