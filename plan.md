...
And that accuracy is not what costs the iterations either, which the same probe settles by making it far worse on purpose: `DKX_COARSE_FACTOR_DTYPE=float32` takes the worst chain's backward error from 2.6e-7 to 793, 9.4 decades, and takes GCROT at `(Nxi, Nx) = (20, 16)` from 2,788 iterations to 10,391 — a factor of 3.7. Over the interval that matters, `Nx = 10` to `16`, the float64 backward error moves 0.6 of a decade, from 7.9e-8 to 3.2e-7, so it cannot produce the 16× in iterations; recovering it entirely, 4.3 decades down to the dense LU's 1.4e-11, would by that measured sensitivity buy well under 2×. The `sparse` route agrees: a fill-reducing elimination of the same operator returns *exactly* 2,788 iterations at `(20, 16)`, with the two maps differing by 5.2e-9. So the factorization is exonerated and the growth is in the spectrum of `A M⁻¹`. The step is therefore a measurement before it is an implementation: assemble `A` and `M` on a deck small enough to take both, and read the eigenvalues and the eigenvector conditioning of `A M⁻¹` at `Nx = 10` and `Nx = 16`. Everything cheaper has now been tried and recorded — retaining more coupling, loosening the tolerance, scaling, and a second exact elimination — and none of them is where the growth lives. Only once that says what degrades is there a preconditioner to build.
Admission: a statement of what changes in the spectrum between `Nx = 10` and `Nx = 16`, specific enough to name the operator term responsible, and a candidate whose admission test can be written against it.
Kill: the spectrum is indistinguishable at the two resolutions. Then the growth is in the right-hand side or the Krylov recycling rather than the operator, and the next measurement is the convergence curve itself rather than the spectrum.
