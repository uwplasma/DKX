...
Kill: a stable elimination takes the worst subsystem's backward error to the dense LU's 1.4e-11 and does not reduce iterations by 1.5×. Then the factors' accuracy is not what costs the iterations, the growth is in the eigenvector conditioning instead, and the next move is a spectral measurement of `A M⁻¹` against `Nx` rather than another preconditioner.
