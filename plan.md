...
- The README is long and benchmark-heavy. (The `sfincs-jax` documentation URL is **not** a defect: the maintainer confirms it is the correct link, and it serves a site titled "DKX documentation". `dkx.readthedocs.io` does not exist.)
...
- shorten README. (Documentation identity is settled: `sfincs-jax.readthedocs.io` is the correct link, confirmed by the maintainer, and the Sphinx project name is already `DKX`.)
...
Added 2026-09-02 after several measurements had to be thrown away.

1. **Never measure on a loaded machine.** A sweep saturates every core. The
   same deck measured ``MatLUFactorNum`` at 0.025 s idle and 3.25 s under a
   concurrent sweep -- a factor of 130. An ad-hoc profile taken "just to check"
   while a sweep runs corrupts both its own numbers and the sweep's.
2. **A relative difference needs the magnitude it was taken against.** Without
...
| 2026-09-02 | **Every ad-hoc timing taken during a running sweep is void, including several I nearly reported.** | Load average reached 43.89 on 14 cores. The same deck measured `MatLUFactorNum` at 0.025 s idle and 3.25 s under load, and `SNESSolve` at 0.16 s against 44 s. Both sides of the DKX-versus-SFINCS profile comparison were affected, and the ad-hoc runs were also polluting the sweep's own numbers. | Do not measure while a sweep runs. Re-profile both codes on an idle machine before any performance claim, and spot-check whether the in-flight sweep's timings survived the load it was given. See [[feedback-never-benchmark-a-shared-checkout]]. |
| 2026-09-02 | **Lead, not yet a result: DKX's sparse-direct route may be weak because of its backend, not because direct methods are wrong here.** | PETSc's log shows MUMPS factorizing these operators as a small slice of SFINCS's runtime, while DKX's tier-3 uses scipy SuperLU, which ran 75 minutes on the 66004-unknown deck without finishing. Multifrontal with a good ordering against a solver that is neither. | Measure on an idle machine: scipy SuperLU against MUMPS on an identical matrix. If it holds, DKX's tier-3 becomes a real referee instead of a route that refuses at any useful size, and the `Er = 15` stall may have a deterministic answer instead of needing `Nxi = 180`. |
| 2026-09-02 | **A converged residual does not pin the physics on these operators.** Three separate observations turned out to be one cause. | Dense SVD of the materialized operator: `er_xdot` 3.36e18, `pas_scheme12` 3.00e12, `magdrift` 6.43e7, `fp_cs3` 2.96e7. At 3.4e18 the system is numerically singular in float64 (`cond * eps` ~ 750), and the deck carrying it is the `Er` `xDot` term -- the physics where every anomaly this session appeared. The bound `||dx||/||x|| <= cond * ||r||/||b||` then explains: Boozer `FSABjHat` moving 2.7e-4 across routes at residual 1e-15; a warm-started surface moving 2.46% from a cold one at residual 1e-10; `FSABjHat` changing sign with `Nxi` while every solve reported converged. | `solver.relative_tolerance` is a residual bound and is **not** an accuracy guarantee for outputs. Do not advertise warm restart, and do not promote route or platform agreement, until outputs carry a solution-error estimate rather than a residual. |
| 2026-09-02 | Row/column equilibration is worth 456x on the worst well-posed deck, and does not rescue the singular one. | Ruiz equilibration (20 sweeps): `pas_scheme12` 3.00e12 -> 6.58e9 (456x), `fp_cs3` 2.96e7 -> 1.58e5 (187x), `magdrift` 6.43e7 -> 2.46e6 (26x), `er_xdot` 3.36e18 -> 1.24e18 (3x). Row magnitude spread on `pas_scheme12` is 4.99e12, so most of that conditioning is scaling rather than stiffness. | Equilibrating before the solve is a concrete, bounded improvement to try. It will not fix `er_xdot`, whose conditioning is intrinsic. |
| 2026-09-02 | **Warm start across surfaces was implemented and reverted.** | Threading `x0`, the GCROT recycle pair and the built preconditioner from one surface to the next changed a flux by **2.46%** while `primal_residual < 1e-10` passed and the solve reported converged, breaking agreement between the case route and the namelist route. That is the conditioning row above, not a coding error. | Reverted. The audit stands: warm restart exists in `er.py` (x0 + recycle + precond), `phi1.py` (recycle only) and `monoenergetic.py` (x0 only), and is absent from `execution.py`, `workflows/scan.py`, `workflows/converge.py`, `workflows/optimization.py`, `run.py` and `batch.py`. Closing that gap needs the error estimate first. |
| 2026-09-02 | A user's missing `Er` folders are under-resolution in pitch angle, not a solver defect. | Their deck at `Er=15`, varying only `Nxi`: 20 failed after 6182 s; 30, 40, 60, 80 converged in 406, 436, 360, 359 s -- more resolution ran *faster*, because the better-conditioned operator needs far fewer iterations. But `FSABjHat` was +5.9e-3, +3.0e-3, -3.4e-3, -4.1e-3 across those four: it changes sign, so none of them is resolved. Sparse direct is not the alternative: materialization is ~160 s at n=66004 but SuperLU ran 75 minutes without finishing. | #160 rewrites the stall message, which had told users to *reduce* `Nxi`. A resolution study at `Nxi` 120/180 and `Nzeta` 35/75 is running to find where the deck actually resolves. |
| 2026-09-02 | The block-Krylov lever is real on GPU and mostly not worth building. | Coarse-preconditioner apply against right-hand-side width, A4000: k=1, 4, 8, 16 all 8.0 ms, so 16 columns are free (16x per column). M3 Max CPU: 1.5, 2.3, 2.8, 3.9 ms (6.1x per column at k=16). But Arnoldi is sequential, so no block method can use free width for a *single* right-hand side; DKX's only genuine multi-RHS case is RHSMode=2/3 at 2-3 columns, and the axes with real width (`Er`, surfaces) are already vmapped in `batched_er_scan`. | Do not implement block GMRES in SOLVAX for this. Revisit only if a workload appears with many simultaneous right-hand sides. |
| 2026-09-01 | **The Boozer round-trip's parallel-current bound was never supportable.** | Measured on one machine, varying only solver route and tolerance with the physics fixed: `direct` gives 6.642796e-10 at 1e-10, 1e-11 and 1e-12; `iterative` gives 6.640995e-10 at 1e-10 and 1e-11 and 6.642796e-10 at 1e-12 -- a spread of 2.7e-4 against an asserted 1e-8. Across platforms the deck route gives 6.640227e-10 (macOS/arm64) and 6.641136e-10 (Linux/x86-64). First read as a platform defect; the single-machine sweep showed it is not. `FSABjHat` is a small, poorly determined component of this tiny deck's state, and a 1e-15 residual measured in the whole-state norm does not pin it. | Split into its own test and xfailed with the measurements in the reason. Not relaxed to a passing bound, which would assert nothing. Closing it needs a deck that resolves the parallel current, or a reference converged past the moment's own sensitivity. The neighbouring flux round-trip still holds at 1e-8 on both platforms and stays unconditional. |
| 2026-09-01 | Stripped the one `Co-authored-by: Claude` trailer from main's history and closed #129 as superseded. | It entered via the squash-merge of #129, whose branch commits carried it; GitHub collected it into #130's message and listed an assistant as a repository contributor. Rebuilt that commit and replayed the 26 after it, trees byte-identical (same tree hash, empty `git diff`). | #157 adds the CI job that rejects the trailer on a PR before it can be squashed. |
| 2026-09-01 | Closed the open bootstrap comparison: the two files are the same equilibrium at different scale. | `wout_LandremanPaul2021_QA_beta2p5_bootstrap.nc` is the published QA deck scaled down 10x in length -- `Aminor_p` 0.17045 vs 1.7045 m, `Rmajor_p` 1.0227 vs 10.227, identical `volavgB` and `presf(0)`. At fixed B, `j ~ B/(mu0 L)`, so its `jdotb` is 10x the published one: the 9.4-9.9x ratio that looked like a defect. The published wout matches the paper's stored SFINCS curve to 0.5-2% in the core. | Nothing outstanding; the ratio is geometry, not physics. |
| 2026-09-01 | DKX reproduces the paper's SFINCS bootstrap current on the published QA equilibrium. | Published deck with the paper's profiles (`ne = 2.38e20(1-s^5)`, `Te = Ti = 9.45 keV (1-s)`): 6.1% RMS against the stored `<J.B>` over `0.15 <= s <= 0.85`, `Er = 0`, 25x15x32x8, residual 7.9e-14, 98 s on one A4000. | Not a converged study -- `Er = 0` is assumed, not known to match the paper's runs. Promote to a validation artifact only after an ambipolar rerun and a resolution ladder. |
| 2026-09-01 | `dkx run` now writes the output file a case declares. | `[output].file` was parsed, validated, resolved and recorded on the Result, then not written unless `--out` was given. Found when the 130 s benchmark above produced no file with exit status zero. `run_case()` keeps its library contract. | #158. |
| 2026-09-01 | README shows the cross-code agreement it never mentioned. | SFINCS Fortran v3 at 2.7e-10 to 1.4e-08 on matched full-Fokker-Planck decks (3.2e-03 for W7-X through physical flux units); MONKES and YANCC inside the 6% gate on all four Beidler coefficients for DSHAPE, NCSX and W7-X EIM. The figure reads every value from `validation/*.json` at build time and raises on a missing artifact. | #158. |
| 2026-09-01 | Main's CI was red on three tests; all fixed. | Banned words `currently` (docs/cli.rst, introduced by #156) and `older` (examples/README.md), plus the Boozer conversion test, which resolved its equilibrium only through the `tests/ref` search entry that exists when `repository_root()` finds a checkout -- absent under the non-editable install the coverage job uses on purpose, so it passed from every source tree and failed only in CI. | Swept the other 566 tests touching `REF`/`repository_root` with `repository_root()` stubbed to None: that test was the only one affected. |
...
| 2026-09-01 | The on-axis plasma is no longer a hardcoded temperature. It is scaled from the equilibrium's own pressure against **published reactor profiles**, with the temperature following from `p = 2nT`, and pinned from the command line rather than an environment variable. | The anchor is the mean of the QA and QH kinetic profiles in Landreman, Buller and Drevlak (arXiv:2205.02914) -- the study VMEX benchmarks its self-consistent bootstrap current against: n(0) = 2.38e20 at 9.45 keV and 2.20e20 at 10.0 keV, both near p(0) = 7.1e5 Pa, with `p = e(n_e T_e + n_i T_i)` exactly. `n(0) = n_ref (p(0)/p_ref)^(1/2)`; the exponent barely matters near the anchor (1/2, 2/3 and 1 give 9.77, 9.75 and 9.71 keV) and one half is the symmetric choice that degrades gracefully away from it. Pinning the published density returns the published temperature to 0.1%. | The user's equilibrium has p(0) = 7.2098e5 Pa, within **0.04%** of the published QA reference, because it is that configuration. The old hardcode put it at 1.1e21 m^-3 and 2 keV against the paper's 2.38e20 and 9.45 keV -- a factor 4.7 in temperature, which is what suppressed the bootstrap current. The `Er` bracket is scaled by the same temperature: it was tuned at 2 keV, the ambipolar field goes roughly like `T/(e L)`, and unscaled it sat entirely inside the ion root of the hotter plasma, which is why every surface reported "no bracketed root". Scaled, all five bracket, and the run is *faster* (51.6 s against 66.2 s) because the extension probes are no longer needed. | **Left open and reported rather than concluded.** With the corrected plasma, DKX's radial `<j.B>` is now several times *larger* than the equilibrium's stored `jdotb`, where before it was 2.5x smaller -- so temperature alone does not reconcile the two. A separate check found the wout's own `jdotb` is **9.4-9.9x** the paper's stored SFINCS drift-kinetic curve at four of five radii, which would say the equilibrium's current is not the drift-kinetic one; but I could not confirm that this `beta2p5_bootstrap` file is the equilibrium that curve was computed for, so it is evidence, not a conclusion. The roots also come out non-monotonic in radius, which a monotonic profile should not give. |
| 2026-09-01 | The second half of the same user report: four of five radial-scan surfaces quoted a bootstrap current at the bracket **edge** rather than at an ambipolar root. All five now find real roots, for 6% more wall time. | Printing `J_r` against `E_r` showed exactly what was wrong: at `r/a >= 0.40` the radial current is positive across the whole bracket and *decreasing* toward the negative edge -- 1.17e-8 at -12 kV/m against 1.4e-7 at +2. The roots were genuinely outside, and the search reported "no bracketed root" and then quoted `<j.B>` at -12 anyway. | The scan now extends past the negative edge, but only on evidence: no sign change yet, a finite current at the edge, and that current smaller in magnitude than at its neighbour, i.e. actually heading for zero. This is deliberately not "extend whenever no root was found". The -12 floor exists because on a reference equilibrium a failed solve once manufactured a sign change and the root finder reported the fake crossing -- and a failed solve does not approach zero monotonically, so the predicate keeps that case out. Bounded at -24 kV/m. | All five surfaces now bracket a root: -9.914, -12.875, -14.605, -15.960, -17.494 kV/m, deepening monotonically with radius as the trend predicted, at 70.1 s against 66.2 s. One implementation trap worth recording: probing single fields desynchronised `er` from `scan.moments`, which every root interpolation indexes, and surfaced as `IndexError: index 10 is out of bounds for axis 0 with size 10`. The extension therefore re-scans once over the full extended list rather than appending to one array and not the other. |
| 2026-09-01 | A user reported `dkx wout_...bootstrap.nc` giving a bootstrap current ~2.5x smaller than the VMEX equilibrium it was optimized for. **It is neither a convergence failure nor a bug: the plasma is under-determined by the input.** | Convergence was checked first and ruled out. Refining the profile solve on that equilibrium gives `<j.B>/sqrt(<B^2>)` of -0.2161 at the shipped 13x19x13x4, then -0.2076, -0.2097, -0.2098 at 15x23x16x5, 21x31x24x6 and 25x39x40x7 -- a 3% move from the default, with the last two agreeing to 0.05%. More resolution cannot close a factor of 2.5, so the answer to "can it be a bit slower but more accurate" is that accuracy is not what is missing here. | A VMEC equilibrium fixes the **pressure** and nothing else. DKX split it with a hardcoded `DEFAULT_T_AXIS_KEV = 2.0` and `T ~ p^(1/3)`, which for this file means n = 9.28e20 m^-3 at 1.82 keV -- a dense, cold plasma whose collisionality `n/T^2` strongly suppresses the bootstrap current. Measured at fixed pressure and fixed resolution, `<j.B>/sqrt(<B^2>)` is -0.208, -0.499, -0.787, -1.130 for an assumed axis temperature of 2, 4, 8 and 16 keV: **a factor of 5.4**. The reported 2.5x gap corresponds to about 4-5 keV, so it is entirely accounted for by the assumption. | Two things follow and are done. The provenance phrase said "p(s) from the equilibrium", which reads as though the whole plasma came from the file; it now names the assumed `T(0)` and how to change it. And `DKX_T_AXIS_KEV` makes the assumption settable, so a bootstrap current can be compared like-for-like against the optimizer that produced the equilibrium. Not concluded: whether VMEX's own `jdotb` is self-consistent. That cannot be judged until both sides use the same `(n, T)`, which is now possible. Separately, four of the five radial-scan surfaces reported `no_bracketed_root` and then quoted `<j.B>` at the bracket edge `Er = -12 kV/m`, which is not an ambipolar state and should not be presented as one -- that is a distinct reporting defect, not yet fixed. |
| 2026-09-01 | Preconditioner reuse across a **wide `Er` bracket does not work**, and finding that out uncovered a latent crash in the residual-recovery retry that could never have run. | `batched_solve` was extended to decide the route with `tier1_available` -- the same predicate `solve` uses, so the build is never speculative -- and build one coarse preconditioner outside the `lax.map` for every element. On the Fokker-Planck ambipolar deck a point far from where the pair was built failed its true-residual check. The +-15% collisionality reuse measured earlier does **not** generalise to a +-8 kV/m `Er` sweep: the coarse surrogate's ExB block varies far more across a bracket than the collision operator does across a collisionality step. The batch change is reverted. | What the failure exposed is worth more than the change. Tripping the residual check entered the automatic true-residual recovery, which crashed with "assignment destination is read-only". `np.asarray` over a JAX buffer returns a **read-only view**, and the retry assigns into `legendre_tail` element-wise, so that path could never have completed. Its sibling arrays escape only by accident -- they are scaled by a unit factor on the way out, which copies. Now `np.array`, which copies. | The reuse that does hold is the narrow kind already shipped: `SolveResult.precond` threaded to a nearby operator, neutral on the structured-direct route and safe across a collisionality step. Reuse across a bracket needs a refresh policy keyed on the iteration count, not a single build, and that is the shape any future attempt should take. |
| 2026-09-01 | `SolveResult` now carries the preconditioner its solve built, which is what makes reuse correct rather than speculative. The `Er`-scan wiring reverted yesterday is redone on that basis. | The earlier attempt failed because a caller cannot know the route before solving: building whenever `method="auto"` meant a structured-direct point paid for a preconditioner it never used, and measured slower. Returning what the solve *actually* built removes the guess -- a direct route hands back `None` and the next point builds nothing. Verified per route: `method="gmres"` returns a pair, `method="direct"` returns `None`. | Measured on both routes now: the PAS ambipolar deck (structured direct) is 6.18/5.99 s against a 6.16/6.19 s baseline, i.e. neutral, which is the requirement -- the previous version was slower here. The Fokker-Planck deck (recycled Krylov) is 13.37/13.40 s against 13.44/13.45 s. | The flagship native `ambipolar_profile` workflow is still untouched and still builds 16 coarse preconditioners per run, confirmed by instrumenting it again after the change. The reason is now understood precisely: `batch.solve_one` runs under `jax.lax.map`, so those 16 are 16 separate **calls to `batched_er_scan`** from the search's rounds, not 16 builds inside one traced map. Reuse there means threading a preconditioner across those calls in `workflows/ambipolar_native.py`, which is a deeper change than a parameter and is left for its own slice rather than rushed. |
| 2026-09-01 | The FGMRES restart default is **left alone**, against the GPU profile's recommendation, because the finding does not transfer to CPU. Recorded so it is not "optimised" later. | On GPU the profile measured `restart` 30 -> 100 giving 118 -> 83 iterations and -25% wall time, and listed raising it as opportunity #4. Measured on CPU on the Sugama deck the iteration reduction reproduces -- 100 -> 61 -- but the wall time does not: 0.305 s at the default 30 against 0.566 s at 100, with 60 also at 0.568 s. The default is the fastest setting on CPU by a factor of 1.9. | The mechanism explains both. On GPU the preconditioner apply is 93.75% of device time, so anything that cuts iterations cuts time almost proportionally. On CPU the apply does not dominate, and the extra orthogonalisation a longer Arnoldi basis costs per iteration outweighs having fewer of them. | The general lesson for Phase G: a Krylov tuning knob measured on one backend is not a default for both. Anything raised from this profile has to be re-measured on the other platform before it changes a shipped default, and `restart` is the case where that check changed the answer. |
| 2026-09-01 | The three defects the GPU profiling turned up are fixed. One of them was an operating-rule-11 violation hiding in plain sight. | `DKX_COARSE_FACTOR_DTYPE=float32` was **silently inert** on the dense-band coarse route -- the profile measured a bit-identical residual with it set, which is the proof. Its own docstring promises the knob is honoured and explains why float32 is opt-in rather than default, so the code contradicted its documentation. The dense route now threads `factor_dtype` into the jitted assembler, casting the bands only after the pins and the invertibility floor are computed at full precision, since those are scaled against band magnitudes. Measured: float64 gives 100 iterations at 5.2231e-15, float32 gives 101 at 5.3629e-15 -- different, where it used to be bit-identical -- and the answer is unchanged against the sparse-direct referee to 7.8e-11. | `recycle_dim=0` died inside jax indexing with "index is out of bounds for axis 1 with size 0", a traceback naming neither the parameter nor DKX. solvax's GCROT scatters the recycled subspace into an `(n, k)` array, so `k = 0` cannot work and plain restarted FGMRES is not reachable through this route. It is now refused by name, with the alternatives stated. | `preconditioner="sparse"` under `JAX_PLATFORMS=cuda` failed at solve time with a JAX-internal message: it factorizes on the host through `jax.pure_callback` and no CPU backend exists. The requirement is now checked at build time and the error says to unset `JAX_PLATFORMS` or use `cuda,cpu`. |
| 2026-09-01 | Wiring the reusable preconditioner into the `Er` scan was attempted, measured, found to be a pessimization, and **reverted**. Recorded so it is not retried the same way. | The `precond=` hook from PR #147 works: passing a pair skips the build entirely, 0 builds against 4 over four solves. But threading it into `er.radial_current` behind `method in (None, "auto", "gmres")` builds a coarse preconditioner *before knowing the route*, and an `auto` case that lands on the structured-direct route never uses it. Measured on the shipped ambipolar deck: 6.31/6.17 s with the change against 6.16/6.19 s without -- flat to slightly worse, which is the wasted build. | Two things have to be true before this is worth doing again. `solve()` should **return** the preconditioner it built, so a caller reuses what the route actually chose instead of guessing ahead of it. And the target is the wrong module: the native `ambipolar_profile` workflow goes through `dkx.batch.batched_er_scan`, not `er.py` -- instrumented at 16 coarse builds per run, untouched by the `er.py` change. `er.py` serves the SFINCS-compatibility `find_ambipolar_er` path, which is what the GPU profile measured at 17 builds for 17 solves. | A provenance correction from the profiling agent also lands here: its numbers are pinned to `23abed65`, three commits behind, so its claim that `solve()` "already has a `precond=` parameter" was read from a newer tree -- at its baseline the parameter did not exist. Its compile-sensitive figures (the ambipolar first-solve 69.7 s, the cold-solve column, the per-point rebuild accounting) predate PRs #146 and #147 and should not be quoted against current main. The op-level tables, the 16-columns result, the memory law and the CPU/GPU section are unaffected. |
| 2026-09-01 | GPU profiling lands, and its top memory finding is fixed: the coarse-preconditioner build now runs as one jitted computation. Peak **4.3x lower**, measured on GPU. | The profile's headline is that the preconditioner **apply** is 93.75% of recycled-Krylov device time -- 87.2 ms per iteration against 0.088 ms for the physics matvec, a ratio near 990:1 -- and that it is latency-bound at 1.9% of FP64 peak because each apply issues ~188 batched triangular solves with **one right-hand column**. The decisive probe: 16 columns cost the same 85.0 ms as 1, at identical memory. That is the largest remaining lever and is not yet taken. | The memory finding was actionable immediately. The dense-band build ran eagerly, so every transpose, stack, `max(abs())` and add was a separate live buffer: peak reached ~18x one band where 4 suffice. Assembling and factoring under one module-level `jax.jit` lets XLA reuse the buffers. Measured same-deck on an idle A4000: **0.0384 GB to 0.0089 GB**, and faster on CPU too (0.122 s to 0.099 s), with the solve unchanged at 100 iterations and agreeing with the sparse-direct referee to 1.7e-10. Jitted at module level rather than as a local closure, so repeated builds hit the cache -- the same trap fixed in `moments.py` earlier today. | **One agent claim corrected by direct measurement.** The report said DKX's default CPU configuration is 36 threads and 4.5-16.6x too slow. It is not: `runtime.configure()` already clamps `NPROC` to `min(8, cpu_count)`, verified as 8 on the 36-core host. The 36-thread numbers were a forced override, not the default. What survives is a documentation problem rather than a code one: `docs/performance.rst` states its CPU production baseline was taken "under ~34-thread load", which is neither the default nor the best configuration, so the same-host GPU/CPU ratio is calibrated against a pessimistic CPU number. Flagged rather than rewritten -- the production deck needs the external equilibrium archive, so I could not reproduce the figure myself, and the agent disclosed machine contention during its runs. |
| 2026-09-01 | Phase G: a solve can now be handed a preconditioner instead of building one. A five-point sweep runs **1.70x faster** with iteration counts unchanged. | `build_tier2_preconditioner` was called unconditionally inside `_solve_tier2`, and `solve()` took a preconditioner *name*, so a caller had no way to pass one in. Measured on the Sugama `collisionOperator=3` deck the build is **39% of a single call**, and the `Er`, monoenergetic and batched scans each paid it per point. `solve(..., precond=pair)` reuses a pair from `build_tier2_preconditioner`. | Reuse is safe by construction rather than by tolerance: the preconditioner never changes the converged answer, only the iteration count. Measured across a +-15% collisionality sweep -- wider than a scan step -- solutions agreed to **7.3e-14** and iterations went 100/100/113/105/115 rebuilt against 90/100/105/113/109 reused, i.e. no degradation. This is the same property SFINCS relies on to precondition with a deliberately different, cheaper matrix, and SFINCS does **not** reuse across `Er`, so this puts DKX ahead of the reference rather than level with it. | The escalation path deliberately does not take a prebuilt pair: it is searching over preconditioner *kinds*, so pinning one would defeat the search. Threading it there was tried, produced a latent `NameError` on a path that only runs when tier 2 stalls, and was removed with the reason recorded in the source. |
| 2026-09-01 | Phase G, first profiling result: **DKX recompiled 57 XLA modules on every warm solve.** Fixing it halves the warm solve, 0.985 s to 0.490 s on the shipped tokamak deck. | A `jax.profiler` trace of a warm solve came back with a million events dominated not by XLA execution but by *tracing* -- 173k `isinstance`, 15k `safe_map`, 14k `core.typeof`, 12k `partial_eval`. Counting `lower_jaxpr_to_module` confirmed it: 341 lowerings on the first call, then **57 on every subsequent identical call**, steady state rather than warmup, with MLIR lowering taking 0.68 s of a 0.93 s solve. | The cause is small and general. `moments._sum_x` and `_sum_tz_sx` take a `strict=True` branch -- the Fortran loop order, which exists for bit-parity -- built from a `lax.fori_loop` whose body is a **closure defined inside the caller**. A new Python object every call means a new dispatch-cache key, so XLA lowers the identical module again: 42 of the 57 came from one line. Hoisting both loops to module-level `jax.jit` builds the closure once. Verified bit-identical, and the Fortran parity suites confirm it. | Two regressions of my own surfaced while verifying, both mine and both now fixed. Adding `physics.coulomb_logarithm` changed every deterministic `case_id`, which invalidated the sealed validation artifacts -- five audits failed with "input case ID mismatch". `to_dict` already had this exact pattern for four earlier fields, each commented "preserves every existing schema-v1 case ID"; the new field now follows it and is omitted at its default. And the `dkx validate` preflight refused any case carrying a `[scan]` table, which is precisely what `dkx scan` exists to run. The remaining 12 lowerings per solve are in `solve.py` and are the next target. |
| 2026-09-01 | `dkx validate` now means "this will run", not "this parses". The case schema is deliberately wider than the executor, and nothing closed the gap until the user had already paid for a solve setup. | Three fields advertise enum values execution refuses: `magnetic_drifts = "full"`, `run.workflow = "monoenergetic"` and `"transport_matrix"`, and `phi1 = "kinetic"`/`"full"`. Each passes `Case.from_file` and the JSON Schema, then fails inside `run_case`. `validate` now calls `execution._validate_native_slice`, the executor's own preflight, so the refusal arrives at the cheap command instead of the expensive one. | This was found three separate times before it was recognised as one defect: by the `dkx schema` template being unrunnable, by the convert survey, and by the example ladder needing four rungs without a `case.toml`. The pattern is worth naming -- **a schema field the executor rejects is no more usable than a missing one** -- because the fix is not to narrow the enums. The wider schema is right: a case should be able to name tangential drifts or a monoenergetic workflow before the executor implements them. What was wrong is that nothing told the user which half they were in. | A test asserts the preflight still accepts every `case.toml` the repository ships. A guard that rejects the project's own examples is a broken guard, and those five are the ones a reader meets first. |
| 2026-09-01 | Phase F: the nine-rung example ladder of section 9.1 replaces the overlapping folders. Every rung runs in CI, byte-for-byte what a reader runs. | `01_tokamak_profile` through `09_phi1_and_impurities`, each a `run.py` plus a `case.toml` where the native route supports one. The rungs carry **no `DKX_CI` branch at all** -- section 9.2 forbids a CI flag that changes the scientific case -- so each is intrinsically sized to seconds and CI executes exactly what a reader executes. Twenty-four tests, about 130 s. Where a rung has a `case.toml`, the script builds the same case in Python and asserts the two share a `case_id`, so the documented command line provably solves what the script solves. | The four rungs **without** a `case.toml` each record why, and two of those reasons are the same defect found earlier with `magnetic_drifts`: **the JSON schema advertises values the executor refuses.** `run.workflow = "monoenergetic"` validates and is then rejected by `execution._validate_native_slice`; `physics.phi1` accepts `kinetic` and `full` and the executor takes only `off`. A schema field the executor rejects is no more usable than a missing one. | Six older example trees survive despite being pedagogically redundant, and the reason is load-bearing rather than laziness: `src/dkx/workflows/optimization.py` **spawns `examples/optimization/*.py` by argv at runtime**, and ten test files plus `docs/figure_provenance.json` reference the others. Retiring them is a separate change that has to start in `src/`, not in `examples/`. |
| 2026-09-01 | A second plot family, and `dkx plot` now dispatches on what a result holds rather than on what the user knows about it. | `dkx scan` wrote Results that nothing could display: they carry a leading `case` dimension and no `r_N`, so the profile panel refused them. `plot_scan_summary` draws the observables against the scan axis, and `--kind auto` (the default) picks the panel from the arrays present. A user should not have to know which panel their result supports before they can look at it. | The axis label comes from `metadata["scan_axes"]`, not from the array name. The array name has had its dots flattened -- `axis_electric_field_value_kV_m` -- so deriving the label from it renders "electric field value kV m". Section 10.2 asks for physical coordinates rather than mangled internal key names, and the scan already records the real schema path. | A failed scan point is drawn as a marked gap rather than omitted, for the same reason a failed ambipolar evaluation is: omitting it joins the curve across the hole, and a scan missing its middle point should not look like a smooth trend. A multi-axis scan is refused rather than projected onto one abscissa, which would overplot unrelated points. |
| 2026-09-01 | The first of section 10.1's plot families lands: radial current against electric field, with every evaluation, bracket, root type and selected branch. `dkx plot RESULT --kind search`. | This family earns priority over the other ten because DKX's admitted ambipolar results are scoped to explicit sampled intervals, and that claim is only honest if a reader can see where the scan actually looked. The result stores `radial_current_A_m2` at the accepted answer only, so the curve along the search is assembled as `J_r = sum_s Z_s e Gamma_s` from the per-evaluation species fluxes -- verified against the stored root current, which it reproduces exactly (-0.4603003 on the shipped ambipolar deck). | Two section 10.3 contract points are load-bearing rather than decorative. An evaluation that produced no number is drawn on a rug at the axis floor rather than dropped or plotted at zero: a failed solve rendered as `J_r = 0` would draw a crossing that never happened, a fake root in the one figure a reader consults to decide whether a root is real. And a surface with no admitted root says so on the panel, with the reason a null result is not proof, instead of showing an empty axis that reads as "nothing happened". | Refusals are by name: a profile Result asked for `--kind search` is told it needs `ambipolar_profile`, a result without species charges is refused rather than assuming +1, and a SFINCS HDF5 file asked for `--kind search` is refused rather than silently handed the summary panel it did not ask for. |
| 2026-09-01 | The documentation landing page was still SFINCS-first, and writing its replacement found that the case template `dkx schema` prints cannot be run at all. | `docs/index.rst` opened with `run_profile(Path("input.namelist"))` -- exactly the stale quickstart Phase F's acceptance forbids -- while the README had been native-first since PR #118. It now leads with a self-contained `Case.from_mapping` snippet that names no equilibrium file, and a test executes it verbatim, the same guard the README quickstart has. | Verifying that quickstart rather than assuming it exposed two defects. `dkx schema --format toml > case.toml && dkx run case.toml` failed twice over: the template's `magnetic_drifts = "full"` is a value `run_case` **refuses**, and it was the schema *default*, so any minimal case omitting the field was rejected too. The default is now `dkes`, the route execution actually implements, with `full` kept in the enum for when it lands. The template also promised `transport_matrix` and `monoenergetic` workflows that `run_case` does not accept. | The template still cannot run unedited, and that is correct rather than a defect: it names a VMEC equilibrium the reader must supply. It now says so in its own header instead of leaving the reader to discover it, and the landing page calls it a reference rather than a starting file. The lesson worth keeping: a documented snippet that nobody executes is wrong within a few releases, so both quickstarts are now executed by tests. |
| 2026-09-01 | Phase F opens with a command-line reference. Four of the eleven commands -- `doctor`, `roots`, `convert` and the `sfincs` group -- were documented on **no page at all**, having been added faster than the docs followed. | `docs/cli.rst` covers all eleven plus the compatibility group, organised by what a user is trying to do rather than alphabetically, and states the exit-status contract: 0 success, 1 the command ran and the answer is no (a scan point failed, a case is not converged, two results differ), 2 the command could not run at all. It documents the behaviours that are not visible from `--help`: that `compare` dispatches on extension because NetCDF4 *is* HDF5, that timings are reported but do not decide its verdict, that `scan` resume keys on `case_id` rather than position, and that zero admitted roots is not proof that none exist. | The documentation-identity task turns out to be **already satisfied**, and this row previously got that half wrong. `README.md` and `pyproject.toml` point at `sfincs-jax.readthedocs.io`, which reads as stale naming; it is not. That URL is live, serves a site titled "DKX documentation", and the maintainer confirms it is the correct link. `dkx.readthedocs.io` returns 404, so "fixing" the repository links would have broken working ones, and no Read the Docs rename is wanted either. The Sphinx project name is already `DKX`. Nothing to do. |
