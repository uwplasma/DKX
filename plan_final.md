...
- Latest audited code-change head: `563d9dac`; last green CI evidence: `96fb2677`.
...
- Non-package Python volume is still too high: `tests/` has 318 Python files,
  `examples/` has 122 Python files, and `scripts/` has 22 Python files.
...
| Examples/tests/scripts cleanup | Active | 68% | Examples are <=10 curated workflows plus Fortran-v3 references; tests are smaller, organized, and >=95% coverage; scripts are removed or promoted; benchmarks are gone. |
...
Overall readiness under this stricter core-slim goal is about 74-78%.
