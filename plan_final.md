...
| --- | --- | --- |
| Whole package | 202 Python files, 165,517 package lines | `<=190` Python files and fewer package lines than this checkpoint for review-ready. Stretch target: `<=175` files if compatibility shims can be deleted safely. |
...
| `problems/transport_matrix` | 25 files including `parallel/`; 14,983 lines; `policies.py` now owns the former solve-policy and preconditioner-dispatch relays; many remaining files are 50-600 line shards | `<=18` files for review-ready. Stretch target: `<=14` files including `parallel/`; `parallel/ <=3` files. |
...
2. Done: merge policy/dispatch shards:
   ...
   `problems/transport_matrix/policies.py`, and the relays have been deleted.
