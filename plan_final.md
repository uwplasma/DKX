...
- Latest audited code-change head: `dd463b57`; last green CI evidence: `96fb2677`.
...
- Non-package Python volume is still too high: `tests/` has 313 Python files,
  `examples/` has 122 Python files, and `scripts/` has 14 Python files.
...
| Examples/tests/scripts cleanup | Active | 74% | Examples are <=10 curated workflows plus Fortran-v3 references; tests are smaller, organized, and >=95% coverage; scripts are removed or promoted; benchmarks are gone. |
...
Overall readiness under this stricter core-slim goal is about 77-81%.
