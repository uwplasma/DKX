...
- Latest audited branch head: `932a8c6f`; last green CI evidence: `96fb2677`.
...
- Non-package Python volume is still too high: `tests/` has 327 Python files,
  `examples/` has 122 Python files, and `scripts/` has 33 Python files.
...
| Examples/tests/scripts cleanup | Active | 55% | Examples are <=10 curated workflows plus Fortran-v3 references; tests are smaller, organized, and >=95% coverage; scripts are removed or promoted; benchmarks are gone. |
...
Overall readiness under this stricter core-slim goal is about 66-70%.
