...
def _resolve_report_path(path_str: str, *, suite_root: Path) -> Path:
    path = Path(path_str)
    if path.is_absolute():
        return path
...
def audit_suite_output_keys(*, suite_root: Path) -> list[CaseKeyCoverage]:
    ...
    for row in rows:
        fortran_h5 = _resolve_report_path(str(row["fortran_h5"]), suite_root=Path(suite_root))
        jax_h5 = _resolve_report_path(str(row["jax_h5"]), suite_root=Path(suite_root))
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"cases={len(coverage)} missing_total={total_missing} extra_total={total_extra}")
