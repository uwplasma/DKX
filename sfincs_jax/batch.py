...
Single ``sfincs_jax`` solves are CPU/GPU parity on the direct tier; the GPU win
comes from **batching** many solves that share a discretization but differ in
their physics leaves.  Because :class:`sfincs_jax.drift_kinetic.KineticOperator`
...
comes from the tier-1 memory model in :mod:`sfincs_jax.solve`
(:func:`sfincs_jax.solve.tier1_peak_memory_bytes`), the memory budget from the
...
* reuse :func:`sfincs_jax.solve.solve` and the operator read-only (no new solver
...
class BatchedSolveResult:
    ...
    Attributes:
        ...
        moments: the per-element RHSMode-1 moment table
            (:func:`sfincs_jax.run.profile_moments_from_operator`), every entry
...
def solve_footprint_bytes(op: KineticOperator) -> float:
    ...
    Reuses :func:`sfincs_jax.solve.tier1_peak_memory_bytes` — the full-band
...
def batched_solve(
    ...
    *,
    ...
    with :func:`sfincs_jax.solve.solve` and reduced to the RHSMode-1 moment
    table (:func:`sfincs_jax.run.profile_moments_from_operator`).  The map is a
    ...
    Args:
        ...
        batch_leaves: mapping ``field name -> batched value`` for the leaves
            ...
            :class:`~sfincs_jax.drift_kinetic.KineticOperator` leaves (no
            discretization leaf; see :data:`_DISCRETIZATION_FIELDS`).
        solve_method: forwarded to :func:`sfincs_jax.solve.solve` (``"auto"``
        ...
        ntv_kernel_tz: optional NTV geometric kernel forwarded to the moment
            table (see :func:`sfincs_jax.run.profile_moments_from_operator`).
...
def batched_er_scan(
    ...
    *,
    ...
    :func:`sfincs_jax.er.operator_at_er`); every other leaf — geometry, species,
    ...
    Args:
        problem: a prepared :class:`sfincs_jax.er.ErProblem`
            (:func:`sfincs_jax.er.prepare`) — carries the base operator with the
    ...
    *,
    ...
    Args:
        operators: the per-surface
            :class:`~sfincs_jax.drift_kinetic.KineticOperator` objects (e.g. one
