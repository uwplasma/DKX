...
from .outputs import formats as _output_formats
...
def _solver_trace_memory_estimate(
    *,
    ...
    Prefer the full-system size for explicit RHSMode=1 paths because sparse-PC and
    sparse-host solvers operate on the full v3 state vector. If only an active
    size is known, use it as a lower-bound estimate rather than leaving the trace
    empty.
    """
    ...
    estimate = estimate_linear_solve_memory(
        ...
        csr_nnz=csr_nnz,
        preconditioner_nbytes=_metadata_int(solver_metadata, "sparse_pc_factor_nbytes_estimate"),
    ...
    return {
        ...
        "csr_operator_nbytes": None if estimate.csr_operator_nbytes is None else int(estimate.csr_operator_nbytes),
        ...
        "csr_total_nbytes": None if estimate.csr_total_nbytes is None else int(estimate.csr_total_nbytes),
        "dense_per_device_nbytes": int(estimate.dense_per_device_nbytes),
        "csr_per_device_nbytes": (
            None if estimate.csr_per_device_nbytes is None else int(estimate.csr_per_device_nbytes)
        ),
    }
