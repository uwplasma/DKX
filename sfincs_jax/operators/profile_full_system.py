...
def _try_build_structured_rhs1_full_csr_operator_bundle(
    *,
    ...
    """Build a no-probe RHSMode=1 CSR bundle for supported full systems.

    This runtime/non-autodiff path adapts the analytic full-CSR matrix owned by
    this module into the ``SparseOperatorBundle`` contract used by sparse-PC
    solver paths. Unsupported or over-budget systems return ``None`` so callers
    can fall back to matrix-free or pattern-probed sparse assembly.
    """
    ...
    if active_indices is not None:
        ...
        if bool(projects_active_subset) and not read_bool_env(
            ...
            if int(op.total_size) > int(max_project_full_size):
                ...
                return None
    # The direct-tail path avoids the conservative product-pattern estimate
    # used by sparse probing; enforce the memory budget on the actual active
    # CSR matrix after assembly instead.
