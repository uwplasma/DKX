...
def _transport_sparse_direct_rescue_allowed(**_kwargs: object) -> bool:
    """The transport sparse-direct rescue path was removed; never admitted."""
    return False


def _transport_sparse_direct_first_attempt_allowed(**_kwargs: object) -> bool:
    """The transport sparse-direct first attempt was removed; never admitted."""
    return False
...
def _transport_sparse_direct_rescue_first(**_kwargs: object) -> bool:
    """Sparse-direct rescue-first mode was removed with the sparse families."""
    return False
...
def _sparse_jax_builder_removed(**_kwargs: object):
    """The env-only ``sparse_jax`` transport preconditioner was removed."""
    raise NotImplementedError(
        "the sparse_jax transport preconditioner was removed with the legacy sparse families"
    )
...
def solve_v3_transport_matrix_linear_gmres(
    *,
    ...
    transport_precond_builders = TransportPreconditionerDispatchBuilders(
        ...
        sparse_jax_builder=_sparse_jax_builder_removed,
...
class TransportSparseDirectContext:
    """Placeholder for the removed transport sparse-direct rescue path.

    The host sparse-direct factorization families were deleted with the legacy
    sparse owners; the retained legacy transport route relies on the tzfft /
    sxblock / collision preconditioners plus the SciPy history rescue.  The
    context keeps its call surface so the driver structure is unchanged, but
    it never admits a pattern and its ``solve`` cannot be reached because the
    sparse-direct first-attempt/rescue policies are statically disabled.
    """

    op: Any
    emit: EmitFn | None

    def pattern_for_solve(self, *, n: int, active_indices_np: np.ndarray | None) -> None:
        del n, active_indices_np
        return None

    def solve(self, **_kwargs: Any) -> GMRESSolveResult:
        raise RuntimeError(
            "transport sparse-direct rescue was removed with the legacy sparse solver families"
        )


def transport_sparse_direct_context_from_env(*, op: Any, emit: EmitFn | None, **_legacy: Any) -> TransportSparseDirectContext:
    """Create the (disabled) transport sparse-direct context."""
    return TransportSparseDirectContext(op=op, emit=emit)
