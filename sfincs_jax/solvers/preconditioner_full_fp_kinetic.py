...
def _species_cache_key(op: V3FullSystemOperator, kind: str) -> tuple[object, ...]:
    return _rhsmode1_precond_cache_key(op, kind)


def _species_regularization_from_env() -> np.float64:
...
def build_rhs1_species_block_preconditioner(
    *,
    ...
    """Build a full RHSMode=1 kinetic block preconditioner per species."""

    cache_key = _species_cache_key(op, "species_block")
    nxi_for_x = np.asarray(op.fblock.collisionless.n_xi_for_x, dtype=np.int32)
    local_per_species = int(np.sum(nxi_for_x))
    block_size_hint = int(local_per_species * int(op.n_theta) * int(op.n_zeta))
    dtype = _precond_dtype(block_size_hint * block_size_hint)
    ...
    if cached is None:
        ...
        reg = _species_regularization_from_env()
                ...
                for il in range(max_l):
                    for it in range(n_theta):
                        for iz in range(n_zeta):
                            idx_map[species, k] = int(
                                ((((species * n_x + ix) * n_l + il) * n_theta + it) * n_zeta + iz)
                            )
                            k += 1
            rep_idx = idx_map[species, :]
        ...
        extra_idx_jnp, extra_inv_jnp = _species_extra_inverse(
            op=op,
            total_size=total_size,
            dtype=dtype,
            reg=reg,
...
def build_rhs1_species_xblock_preconditioner(
    *,
    ...
    """Build one species/x/L factor shared over each angular grid point."""

    cache_key = _species_cache_key(op, "sxblock")
    nxi_for_x = np.asarray(op.fblock.collisionless.n_xi_for_x, dtype=np.int32)
    local_per_species = int(np.sum(nxi_for_x))
    block_size = int(int(op.n_species) * local_per_species)
    dtype = _precond_dtype(block_size * block_size)
    ...
    if cached is None:
        ...
        for species in range(n_species):
            for ix in range(n_x):
                max_l = int(nxi_for_x[ix])
                for il in range(max_l):
                    f_idx = ((((species * n_x + ix) * n_l + il) * n_theta + 0) * n_zeta + 0)
