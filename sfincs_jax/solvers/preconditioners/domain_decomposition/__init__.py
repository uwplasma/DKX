...
def _dd_core_patch_ranges(
    n: int, block: int, overlap: int
...
def _rhs1_dd_coarse_level_count(*, n_dev: int) -> int:
    ...
    n_dev = max(1, int(n_dev))
    coarse_levels_env = os.environ.get(
...
def _rhs1_dd_coarse_block_sizes(
    *, n: int, block: int, overlap: int, levels: int
) -> tuple[int, ...]:
...
def _build_axis_line_preconditioner(
    *,
    ...
    if cached is None:
        ...
        block_inv = np.zeros(
            (*idx_map.shape[:2], line_size, line_size), dtype=np.float64
        )
...
def build_rhs1_theta_schwarz_preconditioner(
    *,
        ...
        for s in range(n_species):
            for iz in range(n_zeta):
                for core_start, core_end, patch_start, patch_end in ranges:
                    ...
                    chunk_cols = precond_chunk_cols(
                        total_size, int(patch_idx_np.shape[0])
                    )
                    y_sub = matvec_submatrix_v3_unsharded(
                        op_pc,
    ...
    coarse_env = (
        os.environ.get("SFINCS_JAX_RHSMODE1_SCHWARZ_COARSE", "").strip().lower()
    )
    coarse_auto = coarse_env not in {"0", "false", "no", "off"}
    coarse_steps_env = os.environ.get(
    ...
    try:
        ...
        coarse_steps = 1
    coarse_damp_env = os.environ.get(
    ...
    *,
            ...
            for ix in range(n_x):
                for il in range(int(nxi_for_x[ix])):
                    for iz in range(n_zeta):
                        ...
                        chunk_cols = precond_chunk_cols(
                            total_size, int(rep_idx.shape[0])
                        )
                        y_sub = matvec_submatrix_v3_unsharded(
                            op_pc,
                            col_idx=rep_idx,
    ...
    def _apply_full(r_full: jnp.ndarray) -> jnp.ndarray:
        ...
        if block_inv.shape[0] > 0:
            ...
            z_full = z_full.at[block_idx.reshape((-1,))].set(
                z_loc.reshape((-1,)), unique_indices=True
            )
        if extra_inv_jnp is not None:
            r_extra = r_full[extra_idx_jnp]
            z_extra = extra_inv_jnp @ r_extra
