...
ApplyOperatorFn = Callable[[object, jnp.ndarray], jnp.ndarray]
...
def build_rhs1_preconditioner_from_kind(
    *,
    ...
    if rhs1_precond_kind == "theta_line":
        return builders.theta_line_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    ...
    if rhs1_precond_kind == "theta_line_xdiag":
        precond = builders.theta_line_xdiag_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    ...
    if rhs1_precond_kind == "species_block":
        return builders.species_block_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "sxblock":
        return builders.sxblock_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "sxblock_tz":
        return builders.sxblock_tz_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "xblock_tz":
        return builders.xblock_tz_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    ...
    if rhs1_precond_kind == "theta_zeta":
        return builders.theta_zeta_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "xmg":
        return builders.xmg_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "pas_lite":
        return builders.pas_lite_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "pas_hybrid":
        return builders.pas_hybrid_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "pas_schur":
        return builders.pas_schur_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "pas_tz":
        return builders.pas_tz_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "pas_tzfft":
        return builders.pas_tzfft_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "pas_tokamak_theta":
        return builders.pas_tokamak_theta_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "pas_ilu":
        return builders.pas_ilu_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
    if rhs1_precond_kind == "zeta_line":
        return builders.zeta_line_builder(op=op, reduce_full=reduce_full, expand_reduced=expand_reduced)
...
def _constraint_scheme1_source_basis(op: object) -> list[jnp.ndarray]:
...
def matvec_submatrix_v3_unsharded(
    ...
    *,
    ...
    from sfincs_jax.operators.profile_system import apply_v3_full_system_operator  # noqa: PLC0415
