...
| `validity.py` | Local-validity diagnostics for radially-local drift-kinetic results: the finite-orbit-width parameter `delta_FOW = w_b/L` (radial locality), the E x B resonance ratios `k_ExB = omega_E/nu_eff` (the `1/nu -> sqrt(nu)` boundary) and `k_res = omega_E/omega_d` (the superbanana-plateau resonance), and the collisionality-regime classifier (Pfirsch-Schlueter / plateau / banana / `1/nu` / `sqrt(nu)` / superbanana-plateau) from `nu_star` and `EStar` (Hinton & Hazeltine, Rev. Mod. Phys. 48, 239 (1976); K.C. Shaing, Phys. Fluids 27, 1567 (1984); D.-I. Ho & R.M. Kulsrud, Phys. Fluids 30, 442 (1987); Beidler et al., Nucl. Fusion 51, 076001 (2011)). Differentiable scalar ratios; host-side pass/marginal/fail flags. |
...
| `impurity.py` | Classical impurity transport: the algebraic (Pfirsch-Schlueter-independent) particle/heat fluxes reduced to the single geometry scalar `<|grad psiHat|^2/BHat^2>` (the geometry-collapsed form of `moments.classical_fluxes`), a bulk-plus-impurity `SpeciesSet` builder, the classical diffusion coefficient, and the ion-temperature screening diagnostic (`-Z` density peaking and the `1/2`-in-the-collisional-limit screening coefficient; Braginskii, Rev. Plasma Phys. 1 (1965); Hinton & Hazeltine, Rev. Mod. Phys. 48, 239 (1976); Helander & Sigmar, CUP (2002)). Differentiable, `vmap`-over-charge-state. |
| `api.py`, `cli.py`, `__main__.py` | Thin public surface over the canonical modules, including `validate` and `schema` for native cases. |
...
| `bootstrap.py` | `KineticBootstrapMismatch` (traced) and `KineticBootstrapCurrent` (host): the kinetic `<j.B>` as `vmex.optimize` objective terms, plus an overlay plot. |
...
outputs, XLA profiles, or large equilibrium files inside `dkx/`.
