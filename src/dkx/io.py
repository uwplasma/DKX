...
This module owns the read side of the ``sfincsOutput`` formats plus the
generic dict-to-file serializers used by benchmarks and workflow summaries.
End-to-end output files are produced by the canonical pipeline
(:mod:`dkx.run` + :mod:`dkx.writer`); equilibrium-path helpers
are re-exported from :mod:`dkx.input_compat` for compatibility with
existing imports.
"""
...
if TYPE_CHECKING:  # pragma: no cover - import cycle guard for type checkers only.
    from .solver_trace import SolverTrace
...
def output_file_format(path: Path) -> str:
    ...
    if suffix == ".npz":
        return "npz"
    raise ValueError(
        f"Unsupported dkx output suffix {suffix!r}. "
    ...
    *,
    ...
    with h5py.File(path, "w") as f:
        ...
        if solver_trace is not None:
            from .solver_trace import write_solver_trace_h5  # noqa: PLC0415
...
def write_sfincs_netcdf(
    *,
    ...
    with Dataset(path, "w", format="NETCDF4") as ds:
        ds.setncattr("dkx_format", "netcdf")
        if solver_trace is not None:
            ds.setncattr("dkx_solver_trace_json", solver_trace.to_json())
        for key, value in data.items():
            ...
            var.setncattr("dkx_original_dtype", original_dtype)
            if arr.ndim == 0:
                var[...] = arr.reshape(()).item()
            else:
                var[...] = arr
        ds.setncattr("dkx_original_names_json", json.dumps(original_names, sort_keys=True))
...
def write_sfincs_npz(
    *,
    ...
    for key, value in data.items():
        ...
        payload[str(key)] = arr
    if solver_trace is not None:
        payload["dkx_solver_trace_json"] = np.asarray(solver_trace.to_json())
...
def read_sfincs_output_file(path: Path) -> dict[str, Any]:
    ...
    if fmt == "netcdf":
        ...
        with Dataset(path, "r") as ds:
            mapping_raw = getattr(ds, "dkx_original_names_json", "{}")
            ...
            for name, var in ds.variables.items():
                ...
                original_dtype = getattr(var, "dkx_original_dtype", "")
