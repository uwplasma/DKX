...
from sfincs_jax.operators.profile_electric_field import ErXDotV3Operator, apply_er_xdot_v3_offdiag2
...
def test_er_xdot_offdiag2_matvec_matches_fortran_matrix() -> None:
    """Parity test for the Er xDot term (|ΔL|=2) against a Fortran v3 Jacobian matrix."""
    here = Path(__file__).parent
    input_path = here / "ref" / "er_xdot_1species_tiny.input.namelist"
    mat_path = here / "ref" / "er_xdot_1species_tiny.whichMatrix_1.petscbin"
    ...
    y_jax = np.asarray(apply_er_xdot_v3_offdiag2(op, jnp.asarray(f)))

    # Fortran reference: restrict A@x to |ΔL|=2 and same (theta,zeta) within the F block.
    ...
    for row in range(n_f):
        ...
        for c, v in zip(cols.tolist(), vals.tolist()):
            ...
            if (s_c, it_c, iz_c) != (s_r, it_r, iz_r):
                continue
            if abs(l_c - l_r) != 2:
                continue
            # allow ix_c != ix_r (x coupling)
