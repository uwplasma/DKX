...
def _species_set(case: dict, dn=None, dt=None) -> species.SpeciesSet:
    s = len(case["z"])
    return species.SpeciesSet(
...
def test_defaults_match_fortran_globalvariables() -> None:
    ...
    assert constants.DEFAULT_NU_PRIME == 1.0
    assert constants.DEFAULT_E_STAR == 0.0
    # globalVariables.F90 line 97 (adiabatic defaults; also outputs/writer.py).
    ad = species.AdiabaticSpecies()
...
def test_radial_conversions_match_old_transport_helper(psi_a_hat: float, a_hat: float, r_n: float) -> None:
    ...
    new = constants.RadialCoordinates(psi_a_hat=psi_a_hat, a_hat=a_hat, r_n=r_n)
...
def test_gradient_variants_match_writer_arithmetic(case_name: str) -> None:
    ...
    radial = constants.RadialCoordinates(psi_a_hat=-0.384935, a_hat=0.5109, r_n=0.5)
...
def test_namelist_species_match_old_writer_path(species_block: str) -> None:
    ...
    species_group = nml.group("speciesParameters")
    geom_params = nml.group("geometryParameters")

    psi_a_hat, a_hat = scheme4_radial_constants()
    r_n = 0.5  # v3 forces rN=0.5 for geometryScheme=4
    radial = constants.RadialCoordinates(psi_a_hat=psi_a_hat, a_hat=a_hat, r_n=r_n)
    new = species.species_set_from_namelist(nml, radial=radial)
    ...
    coord_old = infer_species_input_radial_coordinate_for_gradients(
        geom_params=geom_params, species_params=species_group, default=4
    )
    assert (
        species.infer_gradient_coordinate(geom_params=geom_params, species_params=species_group)
        == coord_old
    )
    ...
    def _old_arr(key: str, default: float) -> np.ndarray:
        v = species_group.get(key, default)
...
def test_fortran_example_namelist_matches_full_system_operator() -> None:
    ...
    assert float(op.alpha) == constants.DEFAULT_ALPHA
    # No adiabatic species in this example.
    assert species.adiabatic_species_from_namelist(nml) is None
...
def test_rhs_mode3_nu_n_and_phi_gradient_match_writer_formulas() -> None:
    ...
    assert (
        constants.nu_n_from_nu_prime(nu_prime=nu_prime, b0_over_bbar=b0, g_hat=g_hat, i_hat=i_hat, iota=iota)
        == nu_n_old
    )
    assert (
        constants.d_phi_hat_d_psi_hat_from_e_star(
            e_star=e_star, alpha=alpha, delta=delta, iota=iota, b0_over_bbar=b0, g_hat=g_hat
        )
        == dphi_old
    )
    # Round trip.
    assert constants.nu_prime_from_nu_n(
...
def test_adiabatic_species_from_namelist_defaults_and_overrides() -> None:
    ...
    ad = species.adiabatic_species_from_namelist(nml)
    ...
    ad2 = species.adiabatic_species_from_namelist(nml2)
