...
import sfincs_jax.problems.transport_solve as iteration_stats
from sfincs_jax.problems.transport_solve import emit_transport_ksp_iteration_stats
...
def test_transport_iteration_stats_emits_bicgstab_history(monkeypatch) -> None:
    captured = {}

    def fake_bicgstab(**kwargs):
        captured.update(kwargs)
        return jnp.zeros((2,)), 0.0, [1.0, 0.4]
    ...
    assert captured["precondition_side"] == "left"
    assert emitted == [(0, "whichRHS=5 ksp_iterations=2 solver=bicgstab")]


def test_transport_iteration_stats_ignores_unknown_solver_kind(monkeypatch) -> None:
    calls: list[str] = []
    monkeypatch.setattr(
        iteration_stats,
        "gmres_solve_with_history_scipy",
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: calls.append("bicgstab"),
    )
    emitted: list[str] = []
