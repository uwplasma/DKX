...
_STATUS_SCRIPT = _REPO / "examples" / "optimization" / "vmex_workflow_status.py"
_PIPELINE_SCRIPT = _REPO / "examples" / "autodiff" / "vmex_to_boozer_sfincs_pipeline.py"


def test_vmex_workflow_status_scaffold_is_skip_safe(tmp_path: Path) -> None:
    ...
    assert payload["workflow"] == "vmex_to_boozer_sfincs_geometry_proxy"
    assert payload["status"] in {"ready", "skipped"}
    assert set(payload["optional_backends"]) == {"vmex", "booz_xform_jax"}
...
def test_vmex_boozer_proxy_gate_passes_or_skips_cleanly(tmp_path: Path) -> None:
    if importlib.util.find_spec("vmex") is None:
        pytest.skip("vmex is not installed")
